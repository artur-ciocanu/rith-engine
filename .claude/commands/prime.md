---
description: Prime agent with full Rith Engine codebase understanding
---

# Prime: Load Rith Engine Project Context

## Objective

Build comprehensive understanding of the Rith Engine monorepo before beginning any work. This orients
...
Read the dependency chain (no @rith/* deps → most isolated):

- `packages/paths/src/index.ts` — path resolution + Pino logger (zero deps)
- `packages/git/src/index.ts` — git ops (depends only on @rith/paths)
- `packages/isolation/src/index.ts` — worktree isolation (depends on @rith/git + @rith/paths)
- `packages/workflows/src/index.ts` — workflow engine (depends on @rith/git + @rith/paths)
...
### Project Overview
- Rith Engine: Remote agentic coding platform (Slack, Telegram, GitHub, Discord, Web)

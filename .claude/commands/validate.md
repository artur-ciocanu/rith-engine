---
description: Run Rith Engine's full validation suite with per-level reporting
---

# Validate: Comprehensive Rith Engine Validation

## Objective

Run all four validation levels for the Rith Engine monorepo and report pass/fail with actionable
...
- Package boundary violations (e.g., `@rith/workflows` importing from `@rith/core`)
...
- `@rith/core` — 7 separate `bun test` batches (mock.module pollution prevention)
- `@rith/workflows` — 5 batches
- `@rith/adapters` — 3 batches
- `@rith/isolation` — 3 batches
...
```bash
bun --filter @rith/workflows test
```

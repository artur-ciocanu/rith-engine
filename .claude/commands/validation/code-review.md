...
Perform a thorough technical code review on recently changed files, checking for bugs, security issues, and adherence to Rith Engine's documented conventions.
...
**Rith Engine-Specific Conventions**
...
- `@rith/git` and `@rith/paths` must not import from `@rith/core`
- `@rith/workflows` injects deps via narrow interfaces, not direct core imports

...
1. `@Rith Engine /command-invoke prime` - Analyze codebase
2. `@Rith Engine /command-invoke plan-feature` - Create implementation plan
3. `@Rith Engine /command-invoke execute` - Implement changes and create PR
4. (Phase 9) `@Rith Engine /command-invoke review-pr` - Review the created PR
...
**Updated for:**
- Bun runtime (not npm)
- Rith Engine paths (`~/.rith/workspaces/owner/repo`)
...
if [ -n "$RITH_HOME" ]; then
  WORK_DIR="${RITH_HOME}/workspaces"
else
  WORK_DIR="${HOME}/.rith/workspaces"
fi
...
rm -rf "${HOME}/.rith/worktrees"/remote-coding-test-*
...
# Create .rith/commands directory with test commands
mkdir -p .rith/commands

# Copy commands from main project
cp "${PROJECT_ROOT}/.rith/commands/prime.md" .rith/commands/ 2>/dev/null || \
cp "${PROJECT_ROOT}/.claude/commands/core_piv_loop/prime.md" .rith/commands/prime.md 2>/dev/null || \
cat > .rith/commands/prime.md << 'CMDEOF'
# Prime Command
...
cp "${PROJECT_ROOT}/.rith/commands/plan.md" .rith/commands/plan-feature.md 2>/dev/null || \
cp "${PROJECT_ROOT}/.claude/commands/core_piv_loop/plan-feature.md" .rith/commands/plan-feature.md 2>/dev/null || \
cat > .rith/commands/plan-feature.md << 'CMDEOF'
...
cp "${PROJECT_ROOT}/.rith/commands/execute.md" .rith/commands/ 2>/dev/null || \
cat > .rith/commands/execute.md << 'CMDEOF'
# Execute Command
...
# Commit
git add .
git commit -m "Initial test repository setup with rith commands"
...
echo "$CWD_MSG" | grep -q "rith\|workspaces" && echo "✅ /getcwd works" || echo "❌ /getcwd failed"
```
...
ORIGINAL_CWD=$(curl -s http://localhost:3000/test/messages/test-clone | jq -r '.messages[-1].message' | grep -o '/[^ ]*rith[^ ]*' | head -1)

# Test setcwd with valid path
curl -X POST http://localhost:3000/test/message \
  -H "Content-Type: application/json" \
  -d "{\"conversationId\":\"test-clone\",\"message\":\"/setcwd /.rith/workspaces/${GITHUB_USERNAME}/${TEST_REPO_NAME}\"}"
...
# Create issue WITHOUT @Rith Engine mention - workflow starts with comments
...
# This is the first @Rith Engine mention - triggers worktree creation
gh issue comment ${ISSUE_NUMBER} --body "@Rith Engine /command-invoke prime"
...
gh issue comment ${ISSUE_NUMBER} --body "@Rith Engine /command-invoke plan-feature \"Add a Validation section to README.md after the Purpose section, explaining how to run tests and validate the platform\""
...
gh issue comment ${ISSUE_NUMBER} --body "@Rith Engine /command-invoke execute"
...
gh pr comment ${PR_NUMBER} --body "@Rith Engine /command-invoke review-pr"
...
rm -rf "${HOME}/.rith/worktrees/${GITHUB_USERNAME}/${TEST_REPO_NAME}"

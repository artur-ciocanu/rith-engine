...
> **Purpose**: Complete reference for understanding and building platform adapters in Rith Engine.
...
4. Self-loop prevention: ignores comments containing `<!-- rith-bot-response -->`
...
**Sending:** `octokit.rest.issues.createComment` with retry logic (3 attempts, exponential backoff). Appends `<!-- rith-bot-response -->` marker to every comment.

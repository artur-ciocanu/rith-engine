...
> **Purpose**: End-to-end flow traces across the entire Rith Engine system with file:line references.
    ...
    → lockManager.acquireLock(conversationId, handler) (conversation-lock.ts:59)
      → handleMessage(platform, conversationId, text) (orchestrator-agent.ts:383)
        ...
        ALL OTHER MESSAGES (including unknown slash commands):
          ...
          → cwd = getRith EngineWorkspacesPath() (orchestrator-agent.ts:458)
...
## 2. Workflow Execution: `/workflow run rith-fix-github-issue #42`
...
```
  ...
  → getWorktreePath() — ~/.rith/workspaces/{owner}/{repo}/worktrees/{branch}
  ...
  → copyConfiguredFiles() — .rith/ + config.worktree.copyFiles
```
...
import * as sessionDb from '@rith/core/db/sessions';
...
Layer 1: Code defaults (config-loader.ts:165)
  → botName: 'Rith Engine', assistant: 'claude', concurrency.maxConversations: 10

Layer 2: Global config (~/.rith/config.yaml)
  → loadGlobalConfig() — cached after first load
  → Overrides: botName, defaultAssistant, assistants.*, streaming modes

Layer 3: Repo config ({repoPath}/.rith/config.yaml)

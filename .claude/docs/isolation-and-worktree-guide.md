...
> **Purpose**: Complete reference for how Rith Engine creates, manages, and cleans up git worktrees for parallel development.
...
**2. Sync workspace**
- Calls `syncWorkspace(repoPath, baseBranch)` from `@rith/git/repo.ts`
...
- Default: always copies `.rith/` directory
- User config: additional paths from `.rith/config.yaml` `worktree.copyFiles`
...
- **Project-scoped** (repo under `~/.rith/workspaces/owner/repo/`):
  - Base: `~/.rith/workspaces/owner/repo/worktrees/`

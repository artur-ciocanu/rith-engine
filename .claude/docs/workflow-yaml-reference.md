...
> **Purpose**: Complete specification of every field, option, and type in Rith Engine's workflow system.
...
Workflows are YAML files discovered from `.rith/workflows/` (recursively) plus bundled defaults. Each must have exactly one execution mode: `steps:`, `loop:` + `prompt:`, or `nodes:`. Parsed by `parseWorkflow()` in `packages/workflows/src/loader.ts:448`.
...
- **Example**: `name: rith-fix-github-issue-dag`
...
### `provider` (optional)
- **Type**: `'claude'` | `'codex'`
- **Default**: falls back to `.rith/config.yaml` assistants default (Claude)
...
- **Default**: from `.rith/config.yaml` `assistants.codex.modelReasoningEffort`
...
steps:
  - command: rith-plan
  - command: rith-implement
    clearContext: true
  - parallel:
      - command: rith-review-code
      - command: rith-review-tests
```
...
4. Opt-out: `defaults.loadDefaultWorkflows: false` in `.rith/config.yaml`
...
|------|------|-------------|
| `rith-feature-development.yaml` | `steps:` | Simple two-step sequential |
| `rith-plan-to-pr.yaml` | `steps:` | 11 steps with parallel review block |
| `rith-ralph-fresh.yaml` | `loop:` | `fresh_context: true`, `<promise>COMPLETE</promise>` |
| `rith-smart-pr-review.yaml` | `nodes:` | `output_format`, `when:`, `trigger_rule: one_success` |
| `rith-validate-pr.yaml` | `nodes:` | `idle_timeout: 1800000`, bash nodes, `trigger_rule: all_done` |
| `rith-fix-github-issue-dag.yaml` | `nodes:` | Full lifecycle with all DAG features |

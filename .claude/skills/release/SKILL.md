...
Creates a release by comparing dev to main, generating changelog entries from commits, bumping the version, and creating a PR. After the tag is pushed and the release workflow finishes building binaries, updates the Homebrew formula with the real SHA256 values from the published `checksums.txt`, syncs the `coleam00/homebrew-rith` tap, and verifies the end-to-end install path via `/test-release`.

> **⚠️ CRITICAL — Homebrew formula SHAs cannot be known until after the release workflow builds binaries.**
>
> The `version` field in `homebrew/rith.rb` and the `sha256` fields must be updated **atomically**. Never update one without the other.
>
...
> 4. Update `homebrew/rith.rb` with the new version AND the new SHAs in a single commit
> 5. Sync to the `coleam00/homebrew-rith/Formula/rith.rb` tap repo
>
...
if [ -f scripts/build-binaries.sh ] && [ -f packages/cli/src/cli.ts ]; then
  ...
  if ! "$TMP_BINARY" --help > /tmp/rith-preflight.log 2>&1; then
    echo "ERROR: compiled binary crashed at startup"
    cat /tmp/rith-preflight.log
    echo ""
  ...
  if grep -qE "Expected CommonJS module|TypeError:|ReferenceError:|SyntaxError:" /tmp/rith-preflight.log; then
...
> open-PR sanity. If the merge produces a `homebrew/rith.rb` conflict during a
> recovery flow, resolve with `git checkout origin/main -- homebrew/rith.rb`
...
> Step 11 (tap sync to `coleam00/homebrew-rith`) is always manual. Check the Actions tab
...
WORKFLOW_FAILED=0
for i in {1..30}; do
  ASSET_COUNT=$(gh release view "vx.y.z" --repo artur-ciocanu/rith-engine --json assets --jq '.assets | length')
  # Expect 7 assets: 5 binaries (darwin-arm64, darwin-x64, linux-arm64, linux-x64, windows-x64.exe) + rith-web.tar.gz + checksums.txt
...
```bash
TMP_DIR=$(mktemp -d)
gh release download "vx.y.z" --repo artur-ciocanu/rith-engine --pattern "checksums.txt" --dir "$TMP_DIR"

DARWIN_ARM64_SHA=$(awk '/rith-darwin-arm64$/ {print $1}' "$TMP_DIR/checksums.txt")
DARWIN_X64_SHA=$(awk '/rith-darwin-x64$/ {print $1}' "$TMP_DIR/checksums.txt")
LINUX_ARM64_SHA=$(awk '/rith-linux-arm64$/ {print $1}' "$TMP_DIR/checksums.txt")
LINUX_X64_SHA=$(awk '/rith-linux-x64$/ {print $1}' "$TMP_DIR/checksums.txt")
...
**Update `homebrew/rith.rb` in the main repo atomically with version AND SHAs:**
...
```bash
cat > homebrew/rith.rb << EOF
# Homebrew formula for Rith Engine CLI
# To install: brew install coleam00/rith/rith
#
# This formula downloads pre-built binaries from GitHub releases.
# For development, see: https://github.com/artur-ciocanu/rith-engine

class Rith Engine < Formula
  desc "Remote agentic coding platform - control AI assistants from anywhere"
  homepage "https://github.com/artur-ciocanu/rith-engine"
  version "x.y.z"
  license "MIT"

  on_macos do
    on_arm do
      url "https://github.com/artur-ciocanu/rith-engine/releases/download/v#{version}/rith-darwin-arm64"
      sha256 "${DARWIN_ARM64_SHA}"
    end
    on_intel do
      url "https://github.com/artur-ciocanu/rith-engine/releases/download/v#{version}/rith-darwin-x64"
  ...
  on_linux do
    on_arm do
      url "https://github.com/artur-ciocanu/rith-engine/releases/download/v#{version}/rith-linux-arm64"
      sha256 "${LINUX_ARM64_SHA}"
    end
    on_intel do
      url "https://github.com/artur-ciocanu/rith-engine/releases/download/v#{version}/rith-linux-x64"
  ...
  def install
    ...
    bin.install binary_name => "rith"
  end

  test do
    # Basic version check - rith version should exit with 0 on success
    assert_match version.to_s, shell_output("#{bin}/rith version")
  end
end
EOF
```
...
The `coleam00/homebrew-rith` repository hosts the actual tap formula that Homebrew reads when users run `brew tap coleam00/rith && brew install coleam00/rith/rith`. The file `artur-ciocanu/rith-engine/homebrew/rith.rb` is the source-of-truth template; the file `coleam00/homebrew-rith/Formula/rith.rb` is what users actually install from. These must be kept in sync.

```bash
TAP_DIR=$(mktemp -d)
git clone git@github.com:coleam00/homebrew-rith.git "$TAP_DIR"
cp homebrew/rith.rb "$TAP_DIR/Formula/rith.rb"

cd "$TAP_DIR"
if git diff --quiet; then
  echo "Tap formula already matches — no sync needed"
else
  git add Formula/rith.rb
...
If the `git clone` fails with a permissions error, the user running the release skill does not have push access to `coleam00/homebrew-rith`. Ask them to request push access from the repo owner, or to perform the sync manually via the GitHub web UI. Do not skip this step silently — the release is not complete until the tap is synced.
...
This will:
- Install via `brew tap coleam00/rith && brew install coleam00/rith/rith`
...
```bash
gh api repos/artur-ciocanu/rith-engine/releases/latest --jq '.tag_name'
...
- **NEVER update `homebrew/rith.rb` version field without also updating the `sha256` values**. They must move together atomically. The correct SHAs only exist after the release workflow finishes building binaries — see Step 10. Updating the version field alone produces a stale formula that looks valid but causes checksum mismatches on install.
- **NEVER skip Step 11 (tap sync).** The `artur-ciocanu/rith-engine/homebrew/rith.rb` file is only a template; users install from `coleam00/homebrew-rith/Formula/rith.rb`. If you update one without the other, users get stale or wrong data.

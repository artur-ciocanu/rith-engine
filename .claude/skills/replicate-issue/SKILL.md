...
description: |
  Replicate and validate a GitHub issue by spinning up Rith Engine, analyzing the issue,
  ...
  Capability: Checks out main, pulls latest, starts Rith Engine, reads the GitHub issue,
  then uses agent-browser to systematically test every symptom and produce a findings report.
  NOT for: Fixing issues (use /rith or /exp-piv-loop:fix-issue), general UI testing (use /validate-ui).
...
Systematically reproduce and validate a GitHub issue against the live Rith Engine application.
...
### 0.2 Kill Existing Rith Engine Processes

Free up ports 3090 (backend) and 5173 (frontend) so Rith Engine starts cleanly.
...
# Trigger a workflow (rith-assist is a good general-purpose one)
curl -s -X POST http://localhost:3090/api/workflows/rith-assist/run \
...
# Stop Rith Engine (optional — leave running if user wants to continue testing)

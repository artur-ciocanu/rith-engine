...
name: rith-dev
description: |
  The PRIMARY development workflow for the Rith Engine project (remote-coding-agent).
  Use this skill instead of any PRP skills when working on Rith Engine code.
  ...
  IMPLEMENT   — "implement the plan", "execute .claude/rith/plans/auth.plan.md",
  ...
  NOT for: Running Rith Engine CLI workflows in worktrees (use /rith instead).
...
- **Artifacts**: !`ls .claude/rith/ 2>/dev/null || echo "none yet"`
- **Active plans**: !`ls .claude/rith/plans/*.plan.md 2>/dev/null | head -5 || echo "none"`
...
All artifacts go to `.claude/rith/`. Create subdirectories as needed on first use.

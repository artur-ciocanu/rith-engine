...
- Files committed
- Suggest next step: `/rith-dev pr` to create a pull request

### Examples

```
/rith-dev commit                          # All changes
/rith-dev commit typescript files         # *.ts only
/rith-dev commit except package-lock      # Exclude specific
/rith-dev commit only the new files       # Untracked only
/rith-dev commit staged                   # Already-staged only
/rith-dev commit the auth refactor        # Context-based targeting
```

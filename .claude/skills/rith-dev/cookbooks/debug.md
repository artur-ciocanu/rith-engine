...
**If from a GitHub issue**: Save to `.claude/rith/issues/issue-{number}.md`
**Otherwise**: Save to `.claude/rith/debug/{date}-{slug}.md`
...
**Next steps**:
- For complex fixes: `/rith-dev plan` (create a plan from the RCA)
- For simple fixes: `/rith-dev implement` (implement directly)

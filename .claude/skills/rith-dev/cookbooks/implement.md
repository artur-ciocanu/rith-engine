...
2. **If no path**: Look for the most recent `.plan.md` in `.claude/rith/plans/` (excluding `completed/`)
3. **If no plans found**: Tell the user and suggest `/rith-dev plan`
...
Save to `.claude/rith/reports/{slug}-report.md` using the plan's slug.
...
```bash
mkdir -p .claude/rith/plans/completed
mv {plan-path} .claude/rith/plans/completed/
```
...
To continue: `/rith-dev plan {prd-path}`
```

**Next steps**:
- To commit: `/rith-dev commit`
- To create PR: `/rith-dev pr`
- To review: `/rith-dev review`

...
- {For bugs}: `/rith-dev debug #{number}` to investigate
- {For features}: `/rith-dev prd` to write requirements, or `/rith-dev plan` to design
```
...
```
/rith-dev issue                              # Use recent conversation context
/rith-dev report this to gh                  # Same — infer from context
/rith-dev log this bug in github             # Bug from context
/rith-dev create a feature request for X     # Feature with description
/rith-dev gh issue: streaming breaks on large responses  # Bug with description
```

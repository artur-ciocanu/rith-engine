...
If `--follow-up` and an existing research file on this topic exists in `.claude/rith/research/`:
...
Save to `.claude/rith/research/{date}-{slug}.md`.
...
**Next steps**:
- To dig deeper: `/rith-dev research --follow-up {topic}`
- For strategic/external research: `/rith-dev investigate {topic}`
- To write requirements: `/rith-dev prd {topic}`

...
Check `.claude/rith/` for artifacts related to this work:

```bash
ls .claude/rith/reports/ 2>/dev/null
ls .claude/rith/plans/completed/ 2>/dev/null
ls .claude/rith/issues/ 2>/dev/null
ls .claude/rith/debug/ 2>/dev/null
```
...
Save to `.claude/rith/reviews/{date}-pr-{number}.md` (or `{date}-{slug}.md` for local changes).
...
gh pr review {NUMBER} --approve --body-file .claude/rith/reviews/{filename}.md

# If --request-changes OR high issues:
gh pr review {NUMBER} --request-changes --body-file .claude/rith/reviews/{filename}.md

# Otherwise just comment:
gh pr comment {NUMBER} --body-file .claude/rith/reviews/{filename}.md
```
...
Suggest: `/rith-dev review {pr-number}` to self-review before requesting human review.

...
Steps to install and configure the Rith Engine CLI. Always run these first.

## 1. Install Dependencies

```bash
cd <rith-repo> && bun install
```
...
```bash
cd <rith-repo>/packages/cli && bun link
```

This makes `rith` available as a global command from any directory.
...
- **Database**: SQLite auto-creates at `~/.rith/rith.db` — no setup needed for CLI-only use.
- **Config**: `~/.rith/config.yaml` is auto-created on first run with sensible defaults. Per-repo config can be added at `<repo>/.rith/config.yaml` to override the AI assistant or configure command folders. Neither file needs manual creation for basic usage.

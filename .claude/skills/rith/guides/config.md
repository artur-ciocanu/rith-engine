# Rith Engine Configuration Guide

Interactive guide for viewing and modifying Rith Engine configuration. Use this when the user wants to change, view, or understand their config — at any point, not just during initial setup.
...
Options:
  1. "Repo config" (Recommended) — Settings for a specific repository (.rith/config.yaml in the repo)
  2. "Global config" — User-wide settings (~/.rith/config.yaml)
  3. "Both" — Review and modify both configs
```

## Step 2: Load Current Config

### For Global Config (~/.rith/config.yaml)

Read the file:
```bash
cat ~/.rith/config.yaml
```

If it doesn't exist, tell the user it will be auto-created on first Rith Engine run with defaults. Offer to create it now.

### For Repo Config (<repo>/.rith/config.yaml)

First determine the target repo. If the current working directory is a git repo (and not the Rith Engine source repo), use it. Otherwise ask:
...
Then read the file:
```bash
cat <target-repo>/.rith/config.yaml
```

If it doesn't exist, tell the user: "No repo config found. I can create one — or Rith Engine will use defaults." Offer to create it.
...
Format example:
```
Current repo config (.rith/config.yaml):
...
Options:
  1. "Bot name" — Display name shown in messages (current: {value}, default: Rith Engine)
...
After writing, confirm: "Updated {path}. Changes take effect on the next Rith Engine invocation (no restart needed for CLI)."
...
|--------|------|---------|-------------|
| `botName` | string | `Rith Engine` | Bot display name shown in messages |
...
| `paths.workspaces` | string | `~/.rith/workspaces` | Directory for cloned repositories |
| `paths.worktrees` | string | `~/.rith/worktrees` | Directory for git worktrees |
| `concurrency.maxConversations` | number | `10` | Maximum concurrent AI conversations |

### Repo Config (<repo>/.rith/config.yaml)
...
2. **Repo config** (`.rith/config.yaml` in repo) — project-specific
3. **Global config** (`~/.rith/config.yaml`) — user-wide preferences
...
| `RITH_HOME` | Base directory for all Rith Engine paths |

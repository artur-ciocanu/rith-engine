...
```
<repoRoot>/.rith/commands/     # 1. Repo-scoped (wins)
├── my-command.md                #    Custom command for this repo
├── rith-assist.md             #    Overrides the bundled rith-assist
└── triage/                      #    Subfolders allowed, 1 level deep
    └── review.md                #    Resolves as 'review', not 'triage/review'

~/.rith/commands/              # 2. Home-scoped (user-level, shared across all repos)
├── review-checklist.md          #    Personal helper available in every repo
└── pr-style-guide.md

<bundled defaults>                # 3. Shipped with Rith Engine (rith-assist, etc.)
```
...
When a workflow references `command: my-command`, Rith Engine searches in this order:

1. `<repoRoot>/.rith/commands/my-command.md` (repo scope)
2. `~/.rith/commands/my-command.md` (home scope — shared across every repo on the machine)
3. Bundled defaults (shipped with Rith Engine)

First match wins. To override a bundled command, drop a file with the same name at either scope. To override a home-scoped command for a specific repo, drop a file with the same name in that repo's `.rith/commands/`.

> **Web UI note**: Home-scoped commands appear in the workflow builder's node palette under a dedicated "Global (~/.rith/commands/)" section, distinct from project and bundled entries.

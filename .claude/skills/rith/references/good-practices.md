...
Rith Engine's orchestrator routes user intent to workflows by description. Write descriptions that make routing obvious.
...
- Mention what it does NOT do: "Does not create a PR — use `rith-plan-to-pr` for that".
...
```bash
rith validate workflows <name>     # YAML + DAG structure + resource refs
```
...
1. Run once against a trivial input (`rith workflow run my-workflow --branch test/sanity "hello"`)
2. Check the run log at `~/.rith/workspaces/<owner>/<repo>/logs/<run-id>.jsonl`
3. Check artifacts at `~/.rith/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/`
...
Ten sibling `prompt:` nodes in one layer all depending on one upstream is a $N/run cost bomb and a latency trap. If the work is parallel and similar, use the `agents:` inline sub-agent map-reduce pattern with a cheap model per item and a single stronger reducer. See `references/dag-advanced.md` and the [Inline sub-agents section on github.com/artur-ciocanu/rith-engine](https://github.com/artur-ciocanu/rith-engine/guides/authoring-workflows/#inline-sub-agents) for a worked example.

### ❌ Hardcoding secrets in YAML or MCP configs

Use `$ENV_VAR` expansion in MCP configs and the `env:` block in `.rith/config.yaml` (or Web UI Settings → Projects → Env Vars). See `references/repo-init.md` §Per-Project Env Injection.
...
- id: analyze
  ...
  mcp: .rith/mcp/postgres.json
  allowed_tools: []          # OOPS — disables EVERYTHING, including MCP tools

# FIXED — Rith Engine auto-adds mcp__<server>__* wildcards when mcp: is set,

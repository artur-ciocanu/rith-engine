...
```
~/.rith/workspaces/<owner>/<repo>/logs/<run-id>.jsonl
```
...
Find the run ID from `rith workflow status` (most recent run). Then:
...
```
~/.rith/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/
```
...
```bash
ls ~/.rith/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/
cat ~/.rith/workspaces/<owner>/<repo>/artifacts/runs/<run-id>/issues/issue-42.md
```
...
A node references `$BASE_BRANCH` in its prompt, but neither git auto-detection nor `worktree.baseBranch` in `.rith/config.yaml` produced a branch.

**Fix:**
1. Set `worktree.baseBranch: main` (or `dev`, or whatever) in `.rith/config.yaml`.
2. Or pass `--from <branch>` on `rith workflow run`.
...
Compiled-binary builds of Rith Engine no longer embed Claude Code / Codex — you install them separately and Rith Engine resolves the binary via env var or config.

**Fix (Claude):**
- Install: `curl -fsSL https://claude.ai/install.sh | bash` (or `npm install -g @anthropic-ai/claude-code`)
- Set `CLAUDE_BIN_PATH=/path/to/claude` in `~/.rith/.env`, OR
- Set `assistants.claude.claudeBinaryPath: /absolute/path` in `.rith/config.yaml`
...
See [github.com/artur-ciocanu/rith-engine/getting-started/installation/](https://github.com/artur-ciocanu/rith-engine/getting-started/installation/) for full platform-specific install paths.

### Workflow shows `running` for a long time but nothing happens

Three possibilities:

1. **The AI is actually working.** Check `~/.rith/workspaces/<owner>/<repo>/logs/<run-id>.jsonl` — if you see recent `tool` or `assistant` events in the tail, it's fine. Wait.
2. **The server crashed and left an orphan row.** Server startup no longer auto-fails orphaned `running` rows (per the "No Autonomous Lifecycle Mutation" rule — `CLAUDE.md`). Transition it manually:
   ...
   - CLI: `rith workflow abandon <run-id>` — marks the DB row cancelled without killing any subprocess. Right tool for orphans since the subprocess is already gone
   - Chat (Slack / Telegram / Web): `/workflow cancel <run-id>` — actively terminates the subprocess. Use for a still-live run that needs to be interrupted (there is no `rith workflow cancel` CLI subcommand)
...
```bash
rith workflow run my-workflow "original message"
# → "Resuming workflow — skipping N already-completed node(s)"
```

Use `--resume` only when you want to force-reuse the same worktree from a specific failed run. Use `rith workflow resume <run-id>` to force a specific run ID.
...
User-level Claude plugin MCPs (e.g. `telegram`, `notion`) inherited from `~/.claude/` fail to connect in the headless subprocess. This is normal — they're not configured for Rith Engine's worktree context. Rith Engine filters these to debug logs (`dag.mcp_plugin_connection_suppressed`) and surfaces only workflow-configured MCP failures.
...
rith workflow status --json | jq '.runs[]'
...
rith validate commands my-command

# Dump the last 50 lines of a workflow's log
tail -n 50 ~/.rith/workspaces/<owner>/<repo>/logs/<run-id>.jsonl | jq .

# Increase log verbosity (workflow run)
rith workflow run my-workflow --verbose "..."
...
1. Run `rith version` and note the version.
2. Run `rith validate workflows <name>` and capture the output.
...
5. File an issue at https://github.com/artur-ciocanu/rith-engine/issues with version, validate output, log tail, and the YAML.

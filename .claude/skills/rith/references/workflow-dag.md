...
Rith Engine workflows use a DAG (Directed Acyclic Graph) format: nodes with explicit dependency edges. Independent nodes run in parallel, conditions enable routing, and data flows between nodes via `$nodeId.output`. This is the only workflow format — there are no other workflow types.
...
|-------|------|-------------|
| `name` | string (required) | Workflow identifier (used in `rith workflow run <name>`) |
| `description` | string (required) | Human-readable summary. Used for routing; see [Workflow Description Best Practices](https://github.com/artur-ciocanu/rith-engine/guides/authoring-workflows/#workflow-description-best-practices) |
| `provider` | string | AI provider (e.g. `claude`, `codex`, `pi`). Default: from `.rith/config.yaml` |
...
Other worktree config (`baseBranch`, `copyFiles`, `initSubmodules`, `path`) lives in `.rith/config.yaml`, not the workflow YAML — see `references/repo-init.md`.
...
| `sandbox` | object | OS-level filesystem/network restrictions. Nested `network` / `filesystem` sub-objects — see [github.com/artur-ciocanu/rith-engine/guides/authoring-workflows/#claude-sdk-advanced-options](https://github.com/artur-ciocanu/rith-engine/guides/authoring-workflows/#claude-sdk-advanced-options) for the full schema. Layers on top of worktree isolation |
...
### Command Node
Runs a command file from `.rith/commands/`:
```yaml
- id: investigate
  command: investigate-issue         # Loads .rith/commands/investigate-issue.md
```
...
**Named script from `.rith/scripts/`:**
```yaml
- id: analyze
  script: analyze-metrics           # Resolves .rith/scripts/analyze-metrics.py
...
- **Named script resolution**: `<cwd>/.rith/scripts/` (wins) → `~/.rith/scripts/`. 1-level subfolder grouping allowed. Extension determines runtime (`.ts`/`.js` → `bun`, `.py` → `uv`) and MUST match the declared `runtime:`
- **Dispatch**:
...
```bash
rith workflow run my-workflow --resume
```
...
4. User runs `rith workflow approve <run-id> "<their feedback>"` (or replies naturally in chat platforms).
...
rith workflow approve <run-id> --comment "looks good"
rith workflow reject <run-id>
rith workflow reject <run-id> --reason "plan needs more test coverage"
...
When a cancel node runs, Rith Engine:
...
Use `--json` for machine-readable output. Use `rith validate commands <name>` to validate individual command files.
...
- Named scripts must exist in `.rith/scripts/` or `~/.rith/scripts/` with extension matching declared runtime

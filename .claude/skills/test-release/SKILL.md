...
name: test-release
description: |
  Verify a released rith binary works end-to-end via a specific install path.
...
Automated smoke test for a released rith binary. Covers three install paths:
...
Every path installs the binary, runs a fixed smoke test suite, and cleans up. The dev `bun link` binary is never touched and remains the default `rith` on PATH throughout.
...
OUTFILE=dist/test-rith-darwin-arm64 \
bash scripts/build-binaries.sh

# Verify the binary — use the path from the mode you built:
#   multi-target → ./dist/binaries/rith-darwin-arm64
#   single-target → the OUTFILE you passed above
./dist/test-rith-darwin-arm64 version
# Expected: Rith Engine CLI v0.3.1, Build: binary, Git commit: abc12345
```
...
```bash
gh release list --repo artur-ciocanu/rith-engine --limit 1 --json tagName --jq '.[0].tagName'
```
...
About to test:
  ...
  Cleanup:  will uninstall after tests (brew uninstall + untap)
            If `rith-stable` symlink is detected in Phase 2, it will be
...
```bash
gh release view v<version> --repo artur-ciocanu/rith-engine --json tagName,assets --jq '{tag: .tagName, assetCount: (.assets | length)}'
```
...
4. **Detect persistent `rith-stable` install (brew path only).** If the user has renamed a prior brew install to `rith-stable` (the dual-homebrew pattern — see `~/.config/fish/functions/brew-upgrade-rith.fish`), Phase 5's `brew uninstall` will wipe it. Capture the state so Phase 5b can restore it:

```bash
RITH_STABLE_WAS_INSTALLED=""
if [ -L /opt/homebrew/bin/rith-stable ] || [ -L /usr/local/bin/rith-stable ]; then
  RITH_STABLE_WAS_INSTALLED="yes"
  echo "Detected persistent rith-stable — will restore after Phase 5 uninstall."
fi
```

Export `RITH_STABLE_WAS_INSTALLED` into the environment used by Phase 5b. Only applies to the `brew` path — `curl-mac` and `curl-vps` don't go through brew and don't disturb `rith-stable`.
...
```bash
brew tap coleam00/rith
brew install coleam00/rith/rith
BINARY="$(brew --prefix coleam00/rith/rith)/bin/rith"
```
...
```bash
INSTALL_DIR=/tmp/rith-test-release-$(date +%s)
mkdir -p "$INSTALL_DIR"
INSTALL_DIR="$INSTALL_DIR" curl -fsSL https://raw.githubusercontent.com/artur-ciocanu/rith-engine/main/scripts/install.sh | bash
BINARY="$INSTALL_DIR/rith"
```
...
```bash
ssh <target> 'curl -fsSL https://raw.githubusercontent.com/artur-ciocanu/rith-engine/main/scripts/install.sh | bash'
```

Determine where the binary landed — `install.sh` uses `/usr/local/bin/rith` by default, or falls back to `$HOME/.local/bin/rith` if `/usr/local/bin` is not writable:

```bash
ssh <target> 'command -v rith'
```
...
Run these in order against `$BINARY` (or `ssh <target> $REMOTE_BINARY` for curl-vps). **Always use the full binary path, never the `rith` on PATH**, so there is no ambiguity about which binary is under test.
...
- Exit code 0
- Output contains `Rith Engine CLI v<expected-version>`
...
```bash
TESTREPO=/tmp/rith-test-repo-$(date +%s)
mkdir -p "$TESTREPO"
cd "$TESTREPO"
git init -q
...
- Exit code 0
- Output lists at least 20 bundled workflows (rith-assist, rith-fix-github-issue, rith-comprehensive-pr-review, etc.)
...
```bash
"$BINARY" workflow run assist "say hello and nothing else" 2>&1 | tee /tmp/rith-test-assist.log
```
...
- `Credit balance is too low` → auth is pointing at an exhausted API key (check `CLAUDE_USE_GLOBAL_AUTH` and `~/.rith/.env`)
...
```bash
(unset CLAUDE_BIN_PATH; "$BINARY" workflow run assist "hello" 2>&1 | tee /tmp/rith-test-no-path.log)
```

**Pass criteria (when no `~/.rith/config.yaml` configures `claudeBinaryPath`):**
...
If you *do* have `claudeBinaryPath` set globally, skip this test or temporarily rename `~/.rith/config.yaml`.
...
```bash
LEAKREPO=/tmp/rith-test-leak-$(date +%s)
mkdir -p "$LEAKREPO"
cd "$LEAKREPO"
git init -q
git commit -q --allow-empty -m init
printf 'ANTHROPIC_API_KEY=sk-ant-test-fake\n' > .env
"$BINARY" workflow run assist "hello" 2>&1 | tee /tmp/rith-test-leak.log
```
...
```bash
brew uninstall coleam00/rith/rith
brew untap coleam00/rith
```
...
```bash
which -a rith
# should show only the ~/.bun/bin/rith path, not a brew path

rith version | head -1
# should match the dev version captured in Phase 2
```

**Restore `rith-stable` if it existed before the test** (dual-homebrew pattern — see Phase 2 item 4):

```bash
if [ -n "$RITH_STABLE_WAS_INSTALLED" ]; then
  echo "Restoring rith-stable (detected before test)..."
  brew tap coleam00/rith
  brew install coleam00/rith/rith
  BREW_BIN="$(brew --prefix)/bin"
  if [ -e "$BREW_BIN/rith" ]; then
    mv "$BREW_BIN/rith" "$BREW_BIN/rith-stable"
    echo "rith-stable restored: $(rith-stable version 2>/dev/null | head -1)"
  else
    echo "WARNING: brew install succeeded but $BREW_BIN/rith missing — check formula"
  fi
fi
```

> **Note on the restored version**: this reinstalls from whatever the tap currently ships, which is typically the release you just tested (so `rith-stable` ends up at the newly-tested version). That's usually what the operator wants — you just verified the new release works, and you want `rith-stable` pointed at it. If you were testing an older version for back-version QA, the restored `rith-stable` will be the *current* tap formula, not the pre-test version. For that rare case, the operator should re-run `brew-upgrade-rith` manually after the test.
...
```bash
ssh <target> "sudo rm -f /usr/local/bin/rith || rm -f \$HOME/.local/bin/rith"
```
...
```
Test Release Report — rith v0.3.1 via brew
────────────────────────────────────────────
Tested at:    2026-04-08 15:42 UTC
Binary SHA:   e62eb73547b3740d56f242859b434a91d3830360a0d18f14de383da0fd7a0be6
Binary path:  /opt/homebrew/Cellar/rith/0.3.1/bin/rith
Dev binary:   /Users/rasmus/.bun/bin/rith → ../install/.../cli.ts (unchanged)
...
```
Test Release Report — rith v0.3.1 via curl-vps
────────────────────────────────────────────────
Tested at:    2026-04-08 15:42 UTC
Binary SHA:   0cf83e15e6af228e3c3473467ca30fa7525b6d7069818d85f97a115ea703d708
Binary path:  user@vps:/usr/local/bin/rith
Dev binary:   /Users/rasmus/.bun/bin/rith (unchanged)

  [PASS]  Test 1  version reports 0.3.1, Build: binary
  [FAIL]  Test 2  workflow list returned 0 workflows

    Command:  rith workflow list
    Exit:     0
    Output:
      Discovering workflows in: /tmp/rith-test-repo-1712590923
      Found 0 workflow(s):
...
- `homebrew/rith.rb` — Homebrew tap formula (updated per release)

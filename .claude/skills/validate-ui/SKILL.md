...
name: validate-ui
description: |
  Comprehensive end-to-end validation of the Rith Engine Web UI using browser automation and codebase review.
  Use when: User wants to validate, test, or audit the Rith Engine web interface, find UI/UX bugs,
  test workflow management, verify parallel agent orchestration, or run comprehensive browser-based E2E tests.
  Triggers: "validate ui", "test the ui", "e2e test", "browser test", "validate rith",
            "test rith ui", "ui audit", "ux review", "comprehensive test", "validate everything".
  Capability: Starts Rith Engine, runs exhaustive browser automation tests via agent-browser CLI,
...
# Rith Engine Web UI — Comprehensive E2E Validation

Run exhaustive end-to-end browser automation tests and codebase review of the Rith Engine Web UI.
The goal: determine whether Rith Engine is doing the best it possibly can to solve the problem of
...
### 0.1 Kill Old Rith Engine Processes

```bash
# Kill any running Rith Engine dev servers (backend + frontend)
...
### 0.3 Start Rith Engine Backend + Frontend

Start both services. Backend must be up before frontend SSE connections work.

```bash
# From the repo root: /path/to/rith

# Start backend (port 3090)
cd /path/to/rith && bun run dev:server &
...
cd /path/to/rith && bun run dev:web &
...
Take screenshots at each major test point: `agent-browser screenshot /tmp/rith-test-{name}.png`
...
- Click `+` again
- Fill in a local path (e.g., `/path/to/rith`)
...
- Click on a workflow card (e.g., `rith-assist`)
...
- Verify: workflow dispatch status message appears (e.g., "Dispatching workflow: rith-assist (background)")
...
- Run a workflow with parallel agents (e.g., `rith-comprehensive-pr-review` has 5 parallel agents)
...
**5.8 Loop Iterations**
- Run a loop workflow (e.g., `rith-test-loop` or `rith-ralph-fresh`)
...
```markdown
# Rith Engine Web UI Validation Report

**Date**: {date}
**Tester**: Claude Code (agent-browser + codebase review)
**Rith Engine Version**: {git commit hash}
**Screenshots**: /tmp/rith-test-*.png
...
> **Is Rith Engine currently doing the best it possibly can to solve the problem of managing a lot of agents in parallel and executing custom workflows with full visibility?**
...
- Take screenshots liberally — save to `/tmp/rith-test-{section}-{name}.png`

---
description: Start Rith Engine from the feature branch, use agent-browser to verify the fix works correctly
argument-hint: (none - reads from artifacts)
---

# E2E Testing: Feature Branch (Verify Fix)

Start Rith Engine from the **feature branch** (this worktree) and use browser automation to verify that the bug is fixed and the UI/UX is correct. Take screenshots as evidence.
...
- `taskkill //F //IM node.exe` or `taskkill //F //IM bun.exe` — this kills Claude Code, the Rith Engine server, and all other workflows
...
## Phase 2: Start Rith Engine on Feature Branch
...
cd "$WORKTREE_PATH" && PORT=$BACKEND_PORT bun run --filter @rith/server dev > "$ARTIFACTS_DIR/.e2e-feature-backend.log" 2>&1 &
BACKEND_PID=$!
...
```bash
# 1. Open the Rith Engine UI (ALWAYS use --session)
FRONTEND_PORT=$(cat $ARTIFACTS_DIR/.frontend-port | tr -d '\n')
...
### 4.2 Stop Feature Branch Rith Engine (Cross-Platform)
...
- **RITH_STARTED**: Backend and frontend running on feature branch code
...
- **RITH_STOPPED**: Processes killed, ports freed — **VERIFY ports are free before finishing**

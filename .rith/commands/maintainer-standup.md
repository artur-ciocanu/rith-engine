...
2. **State JSON block** — delimited by `RITH_STATE_JSON_BEGIN` and `RITH_STATE_JSON_END`, each on its own line, with valid JSON between them.
...
- Do NOT use markdown code fences around the `RITH_STATE_JSON_BEGIN`/`RITH_STATE_JSON_END` markers — the markers must be plain lines.
...
The `direction` markdown defines what Rith Engine IS / IS NOT. The `profile` markdown describes the maintainer's role, scope, and current focus. Both inform the triage:
...
- [ ] State block uses the exact `RITH_STATE_JSON_BEGIN` / `RITH_STATE_JSON_END` markers, each on its own line.
...
Output the brief markdown then the delimited state block — nothing else. The persist node writes the brief markdown (everything before `RITH_STATE_JSON_BEGIN`) to `.rith/maintainer-standup/briefs/<date>.md` and the state JSON (between the markers) to `.rith/maintainer-standup/state.json`. Do not write files yourself.

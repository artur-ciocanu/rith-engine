...
Daily morning briefing for Rith Engine maintainers. Pulls latest `dev`, fetches all open PRs and assigned issues, classifies them **P1–P4** against `direction.md`, and surfaces progress versus the previous run (merged, closed, what you shipped).

## Files in this folder

| File | Committed? | Purpose |
|------|:---:|---------|
| `direction.md` | ✓ | Project north-star — what Rith Engine IS / IS NOT. **Shared by all maintainers.** Drives PR triage and polite-decline classification. |
| `README.md` | ✓ | This file. |
...
1. Copy the template:
   ```bash
   cp .rith/maintainer-standup/profile.md.example .rith/maintainer-standup/profile.md
   ```
2. Edit `profile.md`:
...
3. Run it:
   ```bash
   rith workflow run maintainer-standup ""
   ```
...
`direction.md` is the source of truth for "what Rith Engine is / isn't" during PR triage. Add a clause when a triage decision needs justification (so the next maintainer can reach the same conclusion). When declining a PR, cite the clause inline (e.g., `direction.md §single-developer-tool`).
...
The output structure is defined in `.rith/commands/maintainer-standup.md`. Adjust the Phase 3 template if you want different sections or a different P-tier scheme. The synthesizer's `output_format` schema lives in `.rith/workflows/maintainer-standup.yaml`.

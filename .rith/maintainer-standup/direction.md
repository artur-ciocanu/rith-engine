...
- **Workflow-driven.** Reproducible AI execution chains defined as YAML DAGs in `.rith/workflows/`. Workflows run in isolated git worktrees by default.
- **Type-safe.** Strict TypeScript everywhere. No `any` without justification.
- **Composable.** Scripts in `.rith/scripts/`, commands in `.rith/commands/`, workflows compose them.
...
- **Not a general-purpose chat UI.** Adapters are conversation surfaces for _workflow execution_, not standalone chat experiences.
- **Not a replacement for the AI coding agent itself.** Rith Engine orchestrates Claude Code / Codex / Pi — it doesn't reimplement them.
...
- **Not a workflow marketplace.** Bundled workflows are reference patterns; Rith Engine is not aiming to be a hub for third-party workflow distribution.

## Community providers

Rith Engine ships built-in providers for Claude (`@anthropic-ai/claude-agent-sdk`) and Codex (`@openai/codex-sdk`). Pi (`@mariozechner/pi-coding-agent`) is the reference community provider and sets the pattern others should follow.
...
- The contributor and the community maintain the provider. Rith Engine maintainers do not own upstream-SDK breaks for community providers.
...
- Add a "What Rith Engine IS" or "is NOT" line when a PR triage forces a direction call.

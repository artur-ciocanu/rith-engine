 ...
 * .rith/maintainer-standup/reviewed-prs.json with `{ reviewed_at,
...
if (!existsSync(profilePath)) {
  console.error(
    'No profile.md found — run from repo root, with .rith/maintainer-standup/profile.md present.'
  );
  process.exit(1);
}
...
const allComments = JSON.parse(
  execFileSync(
    'gh',
    ['api', `repos/${owner}/${repo}/issues/comments?since=${since}&per_page=100`, '--paginate'],

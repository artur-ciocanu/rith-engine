 ...
 *
 * Reads gh_handle from .rith/maintainer-standup/profile.md frontmatter.
 *
...
let ghHandle = '';
const profilePath = resolve(process.cwd(), '.rith/maintainer-standup/profile.md');
...
let lastRunAt = '';
const statePath = resolve(process.cwd(), '.rith/maintainer-standup/state.json');
...
if (ghHandle) {
  reviewRequested = parseJson<unknown[]>(
    exec('gh', [
      ...
      'list',
      '--search',
      `is:open is:pr review-requested:${ghHandle}`,
      '--json',
  ...
  authoredByMe = parseJson<unknown[]>(
    exec('gh', [
      ...
      'open',
      '--json',
      'number,title,createdAt,updatedAt,reviewDecision,mergeStateStatus',
    ]),
    []
  );
  issuesAssigned = parseJson<unknown[]>(
    exec('gh', [
      ...
      'open',
      '--json',
      'number,title,labels,createdAt,updatedAt,author',
    ]),
    []
  );
}
...
const recentUnlabeledIssues = parseJson<unknown[]>(
  exec('gh', [
    ...
    'open',
    '--search',
    `no:label created:>${sevenDaysAgoStr}`,
    '--json',
...
const recentlyClosedPrs = parseJson<unknown[]>(
  exec('gh', [
    ...
    `closed:>${sinceDate}`,
    '--json',
    'number,title,author,closedAt,mergedAt,state',
    '--limit',
    '50',
  ]),
  []

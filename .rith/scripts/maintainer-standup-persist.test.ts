...
async function runPersist(stdin: string) {
  ...
  try {
    const proc = Bun.spawn(['bun', 'run', join(import.meta.dir, 'maintainer-standup-persist.ts')], {
      cwd,
...
describe('maintainer-standup-persist', () => {
  ...
  test('marker substring in brief prose before real marker — not confused', async () => {
    const input = [
      ...
      'PR #1676 — fix(scripts): handle duplicate RITH_STATE_JSON_BEGIN blocks in persist — merged ✓',
      'RITH_STATE_JSON_BEGIN',
      '{"version": 4}',
      'RITH_STATE_JSON_END',
    ].join('\n');
  ...
  test('marker substring inside state JSON string value — not confused', async () => {
    ...
    const stateJson = JSON.stringify({
      ...
      observed_prs: [
        {
          number: 1676,
          title: 'fix(scripts): handle duplicate RITH_STATE_JSON_BEGIN blocks in persist',
        },
      ],
    });
    const input = [
  ...
  test('duplicate BEGIN blocks AND marker in prose — last complete pair wins', async () => {
    const input = [
      '# Maintainer Standup — 2026-05-15',
      'Merged PR #1676 which fixes RITH_STATE_JSON_BEGIN duplicate blocks.',
      'RITH_STATE_JSON_BEGIN',
      '{"truncated": true, "partial',
      '',
      'RITH_STATE_JSON_BEGIN',

 ...
 * state.json to .rith/maintainer-standup/. Handles two formats:
 *
...
const BEGIN_RE = /^RITH_STATE_JSON_BEGIN$/gm;
const END_RE = /^RITH_STATE_JSON_END$/gm;
...
if (beginMatches.length > 0 && endMatches.length > 0) {
  ...
  if (beginsBeforeEnd.length > 0) {
    ...
    try {
      ...
      if (beginMatches.length > 1) {
        process.stderr.write(
          `WARN: ${beginMatches.length} RITH_STATE_JSON_BEGIN markers found; used the last complete pair.\n`
        );
      }
    } catch (err) {
    ...
    process.stderr.write(
      `WARN: RITH_STATE_JSON_BEGIN/END markers found but all BEGIN markers appear after the last END; skipping delimiter extraction.\n`
    );
  }
}
...
if (state === null) {
  ...
  if (firstBrace !== -1) {
    ...
    try {
      ...
      process.stderr.write(`JSON-wrapper fallback parse failed: ${(err as Error).message}\n`);
    }
  }
}
...
const baseDir = resolve(process.cwd(), '.rith/maintainer-standup');
...
try {
  ...
  process.stderr.write(`PERSIST FAILED: could not write output files: ${(err as Error).message}\n`);
  process.exit(1);
}
...
process.stdout.write(
  JSON.stringify({
    date,
    source,
    state_path: '.rith/maintainer-standup/state.json',
    brief_path: `.rith/maintainer-standup/briefs/${date}.md`,
  }) + '\n'
);

#!/usr/bin/env bun
/**
 * Validates all .yaml files in $ARTIFACTS_DIR/source/ against the Rith Engine workflow schema.
...
// .rith/scripts/ doesn't reliably honor the @rith/workflows/loader subpath
...
// ship non-workflow YAML alongside the workflow (brand metadata, Rith Engine
...
if (workflowFiles.length === 0) {
  console.log(
    JSON.stringify({ valid: true, files: [], note: 'no workflow yaml files (no top-level nodes:)' })
  );
  process.exit(0);
}

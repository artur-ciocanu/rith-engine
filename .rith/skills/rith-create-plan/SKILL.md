---
name: rith-create-plan
description: |
  Creates comprehensive feature implementation plans through systematic codebase
  exploration, pattern extraction, and strategic research. Transforms feature
  descriptions or PRD files into battle-tested implementation plans.
  Use when creating a new implementation plan for a feature, enhancement, refactor,
  or bug fix.
metadata:
  author: rith-engine
  version: '1.0'
...
> **Note**: For plan confirmation procedures (verifying plan research is still valid before implementation), see [`references/plan-confirmation.md`](references/plan-confirmation.md).
...
Use the plan output template from [`assets/plan-template.md`](assets/plan-template.md) to write to `$ARTIFACTS_DIR/plan.md`.

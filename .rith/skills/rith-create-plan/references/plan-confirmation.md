...
This is a reference file for the `rith-create-plan` skill. It contains the procedure for verifying that a plan's research is still valid before implementation begins.
...
Continue to `rith-implement-tasks` to execute the plan.

{If STOP:}
Revise the plan to address blockers, then re-run `rith-plan-setup`.
```
...
Proceed to `rith-implement-tasks` to execute the plan.
```
...
3. **Re-run planning** with `/rith-plan` to generate a fresh plan

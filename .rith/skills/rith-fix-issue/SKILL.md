...
name: rith-fix-issue
description: |
  Implements a fix from an investigation artifact with code changes, validation,
  commit, and optional PR creation with self-review.
  Use when an investigation artifact exists and needs to be executed as a fix,
  with or without PR creation.
metadata:
  author: rith-engine
  version: '1.0'
---
...
> **If a PR is needed after fixing, continue to [Phase 8: PR - Create Pull Request](#phase-8-pr---create-pull-request)** to push, create a PR linked to the issue, write a full implementation report with PR details, and report to the user.
...
## Phase 8: WRITE - Implementation Report (No-PR Path)

> **If a PR is needed after fixing, skip this phase and continue to [Phase 8: PR - Create Pull Request](#phase-8-pr---create-pull-request) below.**
...
## Phase 9: OUTPUT - Report to User (No-PR Path)
...
# PR Creation Phase (Continue here if a PR is needed)

> **If a PR is needed after fixing, continue from Phase 7 to the phases below.**
> These phases replace the No-PR Path phases above: push, create a PR, write
> the full implementation report with PR details, and report with the PR link.
...
## Phase 9: WRITE - Implementation Report (PR Path)
...
## Phase 10: OUTPUT - Report to User (PR Path)
...
- **READY_FOR_PR**: Workflow continues to PR creation (no-PR path)
- **PR_CREATED**: PR exists and linked to issue (PR path)
- **READY_FOR_REVIEW**: Workflow continues to comprehensive review (PR path)

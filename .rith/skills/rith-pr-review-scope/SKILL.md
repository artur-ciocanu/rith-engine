---
name: rith-pr-review-scope
description: |
  Verifies a PR is in a reviewable state, gathers context for parallel review
  agents, and creates the scope manifest. Use when a PR review workflow needs
  to assess reviewability and prepare artifacts before launching review agents.
metadata:
  author: rith-engine
  version: '1.0'
---
...
The PR number has been resolved and written to `$ARTIFACTS_DIR/.pr-number` by the bash setup step. Read it:
...
```bash
# Option 1: Plan-based workflow (rith-plan-to-merge)
ls -t $ARTIFACTS_DIR/../runs/*/plan-context.md 2>/dev/null | head -1

# Option 2: Issue-based workflow (rith-fix-github-issue)
...
## Phase 4: SCOPE MANIFEST - Document Review Context
...
Then re-request the review: `@rith review this PR`

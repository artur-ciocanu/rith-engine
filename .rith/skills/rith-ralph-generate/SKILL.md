---
name: rith-ralph-generate
description: |
  Autonomously generates Ralph PRD files (prd.md and prd.json) from a feature idea or
  existing PRD through systematic codebase exploration and analysis. Use when a new
  feature needs a product requirements document with sized user stories.
metadata:
  author: rith-engine
  version: '1.0'
---
...
| `.rith/ralph/{slug}/prd.md` exists | Existing PRD in ralph dir | Generate prd.json alongside it             |
...
```bash
mkdir -p .rith/ralph/{slug}
```

### 5.3 Write prd.md

**Output path**: `.rith/ralph/{slug}/prd.md`
...
**Output path**: `.rith/ralph/{slug}/prd.json`
...
```bash
git add .rith/ralph/{slug}/
git commit -m "docs: add Ralph PRD for {feature name}"
```

**PHASE_5_CHECKPOINT:**

- [ ] `.rith/ralph/{slug}/prd.md` written with all sections
- [ ] `.rith/ralph/{slug}/prd.json` written with properly sized stories
...
- **FILES_WRITTEN**: Both prd.md and prd.json exist in `.rith/ralph/{slug}/`

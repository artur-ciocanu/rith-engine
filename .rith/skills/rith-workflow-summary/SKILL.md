---
name: rith-workflow-summary
description: |
  Produces a structured completion report for multi-step workflows. Summarizes
  what was built, decisions made, and verification results. Use when a workflow
  reaches its final node and needs to produce a summary for the user.
metadata:
  author: rith-engine
  version: '1.0'
---
...
**Reply with**: `@rith do quick wins` to auto-fix these.
...
**Reply with**: `@rith create follow-up issues` to create these.
...
**Reply with**: `@rith update docs` to send a docs agent.
...
1. **Review the PR**: {url}
2. **Quick wins**: Reply `@rith do quick wins` on PR (or skip)
3. **Create issues**: Reply `@rith create follow-up issues` (or skip)
4. **Merge when ready**

...
Thank you for your interest in contributing to Rith Engine!
...
Share your Rith Engine workflows with the community by adding an entry to the marketplace registry at [`packages/docs-web/src/data/marketplace.ts`](packages/docs-web/src/data/marketplace.ts).
...
3. Fork Rith Engine and add an entry to `packages/docs-web/src/data/marketplace.ts`
...
├── commands/          # → installed to .rith/commands/
│   └── helper.md
├── scripts/           # → installed to .rith/scripts/
│   └── analyze.ts
└── skills/            # → installed to .rith/skills/
    └── my-skill/
```
...
| `rithVersionCompat` | Semver range (e.g. `>=0.3.0`) |
...
Open an [issue](https://github.com/artur-ciocanu/rith-engine/issues) or start a [discussion](https://github.com/artur-ciocanu/rith-engine/discussions).

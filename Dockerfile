# =============================================================================
# Rith Engine - Remote Agentic Coding Platform
...
# OCI Labels for GHCR
LABEL org.opencontainers.image.source="https://github.com/artur-ciocanu/rith-engine"
...
# Create Rith Engine directories
RUN mkdir -p /.rith/workspaces /.rith/worktrees \
    && chown -R appuser:appuser /.rith
...
# Configure git to trust Rith Engine directories (as appuser)
RUN gosu appuser git config --global --add safe.directory '/.rith/workspaces' && \
    gosu appuser git config --global --add safe.directory '/.rith/workspaces/*' && \
    gosu appuser git config --global --add safe.directory '/.rith/worktrees' && \
    gosu appuser git config --global --add safe.directory '/.rith/worktrees/*'

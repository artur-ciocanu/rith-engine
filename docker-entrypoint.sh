...
mkdir -p /.rith/workspaces /.rith/worktrees

# Determine if we need to use gosu for privilege dropping
if [ "$(id -u)" = "0" ]; then
  # Running as root: fix volume permissions, then drop to appuser
  if ! chown -Rh appuser:appuser /.rith 2>/dev/null; then
    echo "ERROR: Failed to fix ownership of /.rith — volume may be read-only or mounted with incompatible options" >&2
    exit 1
  fi
  # /home/appuser is persisted to a named volume (or bind-mounted via
  # /home/appuser is persisted to a named volume (or bind-mounted via
  # RITH_USER_HOME) so Claude/Codex/Pi config, ~/.gitconfig, shell history,
  # and other user-specific state survive rebuilds. On bind mounts, host UIDs
  # don't map to appuser (1001), so fix ownership the same way we do /.rith.
...
# Warn if vars known to be ignored inside the container were set via env_file: .env.
# These leak in but have no effect (RITH_HOME is overridden to /.rith by source;
# RITH_DATA is a host-side compose substitution token, never read by the container).
if [ -n "${RITH_HOME:-}" ]; then
  echo "[rith] RITH_HOME=${RITH_HOME} ignored in Docker (container home is fixed at /.rith)" >&2
fi
if [ -n "${RITH_DATA:-}" ]; then
  echo "[rith] RITH_DATA=${RITH_DATA} is a host-side compose token; not read inside the container" >&2
fi

# Register all git repositories under /.rith as safe directories.
...
# every boot.
find /.rith -name ".git" -prune -print 2>/dev/null | while IFS= read -r git_dir; do

...
Chat adapters connect Rith Engine to messaging platforms (Slack, Telegram, Discord, etc.) via polling or WebSocket.

## Interface

Implement `IPlatformAdapter` from `@rith/core`:
...
```typescript
import { MyAdapter } from '@rith/adapters/community/chat/my-adapter';
...
Check `packages/adapters/package.json` to see which test files share a batch. If your test mocks the same modules (e.g., `@rith/paths`) with different exports, split it into a separate batch:

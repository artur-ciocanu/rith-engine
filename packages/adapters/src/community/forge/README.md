...
Forge adapters connect Rith Engine to code hosting platforms (GitHub, GitLab, etc.) via webhooks.
...
```typescript
import { MyForgeAdapter } from '@rith/adapters/community/forge/my-forge';
...
Never add your test to an existing batch that mocks the same modules differently (e.g., `@rith/paths`, `@rith/git`).

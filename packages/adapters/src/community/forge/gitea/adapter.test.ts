...
// Mock @rith/git to avoid real git operations in tests
...
import { ConversationLockManager } from '@rith/core';
...
describe('GiteaAdapter', () => {
  ...
  describe('bot mention detection', () => {
    ...
    test('should detect mention when it is the entire message', () => {
      ...
      expect(hasMention.call(adapterWithMention, '@Rith')).toBe(true);
      expect(hasMention.call(adapterWithMention, '@RITH')).toBe(true);
      expect(hasMention.call(adapterWithMention, '@rith')).toBe(true);
    });
  ...
  describe('self-filtering', () => {
    ...
    function createSelfFilterAdapter(botMention = 'rith'): GiteaAdapter {
    ...
    test('should ignore comments from the bot itself', async () => {
      ...
      const payload = createCommentPayload('@rith fix this', 'rith');
    ...
    test('should handle case-insensitive username matching', async () => {
      const adapter = createSelfFilterAdapter('Rith Engine'); // Mixed case config
      const payload = createCommentPayload('@rith test', 'rith'); // Lowercase author
    ...
    test('should NOT filter comments from real users', async () => {
      const adapter = createSelfFilterAdapter();
      const payload = createCommentPayload('@rith please help', 'user123');
    ...
    test('should process comments without bot marker from same user', async () => {
      ...
      const payload = createCommentPayload('@rith fix this', 'Wirasm');
    ...
    test('should handle missing comment.user gracefully', async () => {
      const adapter = createSelfFilterAdapter();
      const payload = createCommentPayload('@rith help', undefined);
  ...
  describe('conversationId format', () => {
    ...
    test('postComment appends bot marker to outgoing comments', async () => {
      ...
      expect(body).toContain('<!-- rith-bot-response -->');

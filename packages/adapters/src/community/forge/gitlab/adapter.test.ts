...
// Mock @rith/paths to suppress noisy logger output during tests
const mockLogger = {
...
mock.module('@rith/paths', () => ({
  ...
  getRithWorkspacesPath: mock(() => '/tmp/test-workspaces'),
  getCommandFolderSearchPaths: mock(() => ['.rith/commands', '.claude/commands']),
  ...
  logRithPaths: mock(() => undefined),
  validateAppDefaultsPaths: mock(async () => undefined),
}));

// Mock @rith/core/db modules to throw immediately (avoid DB connection hangs in tests)
...
const { ConversationLockManager } = await import('@rith/core');
...
function createNotePayload(overrides?: {
  ...
  const base: Record<string, unknown> = {
    ...
    object_attributes: {
      ...
      note: overrides?.note ?? '@rith hello',
      noteable_id: 100,
    },
  };
...
describe('GitLabAdapter', () => {
  ...
  describe('self-filtering', () => {
    ...
    test('ignores comments from bot account', async () => {
      const adapter = createAdapter({ botMention: 'rith' });
      const payload = createNotePayload({
        note: '@rith something',
        username: 'rith',
      });
  ...
  describe('authorization', () => {
    test('rejects unauthorized users when whitelist is set', async () => {
      ...
      const adapter = createAdapter();
      const payload = createNotePayload({ username: 'mallory', note: '@rith hello' });
      await adapter.handleWebhook(payload, 'test-secret');
      expect(mockHandleMessage).not.toHaveBeenCalled();
    });
  });
  ...
  describe('mention patterns', () => {
    test('detects mention at end of string', async () => {
      const adapter = createAdapter();
      const payload = createNotePayload({ note: 'help me @rith' });
      await adapter.handleWebhook(payload, 'test-secret');
    ...
    test('detects mention with comma separator', async () => {
      const adapter = createAdapter();
      const payload = createNotePayload({ note: '@rith, please help' });
      await adapter.handleWebhook(payload, 'test-secret');
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.objectContaining({ conversationId: 'mygroup/myproject#1' }),
        'gitlab.webhook_setup_failed'
    ...
    test('case-insensitive mention detection', async () => {
      const adapter = createAdapter();
      const payload = createNotePayload({ note: '@RITH ENGINE help' });
      await adapter.handleWebhook(payload, 'test-secret');
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.objectContaining({ conversationId: 'mygroup/myproject#1' }),
        'gitlab.webhook_setup_failed'
  ...
  describe('stripMention', () => {
    ...
    test('strips mention followed by space', () => {
      expect(strip('@rith hello')).toBe('hello');
    });

    test('strips mention followed by comma', () => {
      expect(strip('@rith, please help')).toBe('please help');
    });

    test('strips mention at end of string', () => {
      expect(strip('help me @rith')).toBe('help me');
    });

    test('does NOT strip partial-prefix username (@rithbot)', () => {
      // Regression: old regex `@rith[\s,:;]*` would strip '@rith' from '@rithbot',
      // yielding 'bot hello'. New regex requires a separator or end-of-string.
      expect(strip('@rithbot hello')).toBe('@rithbot hello');
    });
  });
  ...
  describe('sendMessage', () => {
    ...
    test('posts to correct issue notes API endpoint', async () => {
      const adapter = createAdapter();
      await adapter.sendMessage('mygroup/myproject#42', 'Hello from Rith Engine');
    ...
    test('appends bot response marker to outgoing comments', async () => {
      ...
      expect(body.body).toContain('<!-- rith-bot-response -->');

...
// Mock @rith/git for ensureRepoReady integration tests
...
describe('GitHubAdapter', () => {
  ...
  describe('bot mention detection', () => {
    ...
    test('should detect mention when it is the entire message', () => {
      const adapterWithMention = new GitHubAdapter('token', 'secret', mockLockManager, 'Rith');
      const hasMention = (
        adapterWithMention as unknown as { hasMention: (text: string) => boolean }
      ).hasMention;
  ...
  describe('self-filtering', () => {
     ...
     */
    function createSelfFilterAdapter(botMention = 'rith'): GitHubAdapter {
    ...
    test('should handle missing comment.user gracefully', async () => {
      const adapter = createSelfFilterAdapter();
      const payload = createCommentPayload('@rith help', undefined); // No user field

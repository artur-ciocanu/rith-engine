...
import { createLogger, getRunArtifactsPath, parseOwnerRepo } from '@rith/paths';
...
const DEFAULT_WORKFLOW = 'rith-assist';
  ...
  if (!env) {
    throw new Error(
      `No active worktree found for branch '${branch}'.\n` +
        "Run 'rith isolation list' to see available worktrees."
    );
  }
...
async function resolveArtifactsDir(
  ...
  const fallback = join(workingPath, '.rith', 'artifacts', 'runs', runId);
  try {
    await stat(fallback);
    return fallback;
  } catch {

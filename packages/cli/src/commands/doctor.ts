 ...
 *
 * Also invoked from the end of `rith setup`; the setup wizard discards the
...
export async function checkClaudeBinary(
  ...
  if (!path) {
    return {
      label,
      status: 'fail',
      message: 'CLAUDE_BIN_PATH is not set. Run `rith setup` to configure.',
    };
  }
  try {
...
export async function checkDatabase(
  ...
  // import. Falls back to the lazy `@rith/core` import in production.
  ...
  try {
    ...
    // `rith setup` when the real fix is a binary rebuild.
...
async function defaultLoadDatabaseDeps(): Promise<DatabaseDeps> {
  // Lazy import so doctor doesn't pull in the full @rith/core graph just to
  // print --help or run a different check.
  const { pool, getDatabaseType } = await import('@rith/core');
...
export async function checkBundledDefaults(): Promise<CheckResult> {
  ...
  try {
    const { BUNDLED_COMMANDS, BUNDLED_WORKFLOWS } = await import('@rith/workflows/defaults');

...
import { createLogger, getWebDistDir, BUNDLED_IS_BINARY, BUNDLED_VERSION } from '@rith/paths';

const log = createLogger('cli.serve');

const GITHUB_REPO = 'artur-ciocanu/rith-engine';
...
export async function serveCommand(opts: ServeOptions): Promise<number> {
  ...
  if (!BUNDLED_IS_BINARY) {
    console.error('Error: `rith serve` is for compiled binaries only.');
  ...
  try {
    const { startServer } = await import('@rith/server');
    await startServer({
...
async function downloadWebDist(version: string, targetDir: string): Promise<void> {
  const tarballUrl = `https://github.com/${GITHUB_REPO}/releases/download/v${version}/rith-web.tar.gz`;
  ...
  const expectedHash = parseChecksum(checksumsText, 'rith-web.tar.gz');

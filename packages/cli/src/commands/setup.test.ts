...
const TEST_DIR = join(tmpdir(), 'rith-setup-test-' + Date.now());
...
describe('setup command', () => {
  ...
  describe('checkExistingConfig', () => {
    it('should return null when no .env file exists', () => {
      // Mock RITH_HOME to point to non-existent directory
      const originalHome = process.env.RITH_HOME;
      process.env.RITH_HOME = join(TEST_DIR, 'nonexistent');
    ...
    it('should detect existing configuration values', () => {
      const envDir = join(TEST_DIR, '.rith');
...
`.trim()
    ...
    it('detects existing Pi configuration from a Pi API key env var', () => {
      const envDir = join(TEST_DIR, '.rith-pi');
      mkdirSync(envDir, { recursive: true });
      const envPath = join(envDir, '.env');
  ...
  describe('copyRithSkill', () => {
    it('should create skill files in target directory', async () => {
      const target = join(TEST_DIR, 'skill-target');
      mkdirSync(target, { recursive: true });

      await copyRithSkill(target);

      expect(existsSync(join(target, '.claude', 'skills', 'rith', 'SKILL.md'))).toBe(true);
      expect(existsSync(join(target, '.claude', 'skills', 'rith', 'guides', 'setup.md'))).toBe(
        true
      );
      expect(
        existsSync(join(target, '.claude', 'skills', 'rith', 'references', 'workflow-dag.md'))
      ).toBe(true);
      expect(
        existsSync(join(target, '.claude', 'skills', 'rith', 'examples', 'dag-workflow.yaml'))
      ).toBe(true);
    });
    ...
    it('should write non-empty content to skill files', async () => {
      ...
      const content = readFileSync(join(target, '.claude', 'skills', 'rith', 'SKILL.md'), 'utf-8');
      expect(content.length).toBeGreaterThan(0);
      expect(content).toContain('rith');
    });

    it('should overwrite existing skill files', async () => {
      const target = join(TEST_DIR, 'skill-target-overwrite');
      const skillDir = join(target, '.claude', 'skills', 'rith');
    ...
    it('should create skill files even when target directory does not exist', async () => {
      ...
      // Do NOT pre-create target — copyRithSkill must handle it
  ...
  describe('bootstrapProjectConfig', () => {
    it('creates .rith/config.yaml when it does not exist', () => {
      ...
      expect(result.path).toBe(join(target, '.rith', 'config.yaml'));
      ...
      expect(content).toContain('Project-scoped Rith Engine config');
      expect(content).toContain('github.com/artur-ciocanu/rith-engine/reference/configuration');
    });

    it('creates the .rith directory if missing (idempotent on parent)', () => {
      const target = join(TEST_DIR, 'bootstrap-no-rith-dir');
      mkdirSync(target, { recursive: true });
      // Do NOT pre-create .rith — bootstrap must create it

      const result = bootstrapProjectConfig(target);

      expect(result.state).toBe('created');
      expect(existsSync(join(target, '.rith'))).toBe(true);
    });

    it('is idempotent — leaves an existing config untouched', () => {
      const target = join(TEST_DIR, 'bootstrap-existing');
      const rithDir = join(target, '.rith');
      mkdirSync(rithDir, { recursive: true });
      const userContent = '# my custom config\nassistants:\n  claude:\n    model: opus\n';
      writeFileSync(join(rithDir, 'config.yaml'), userContent);

      const result = bootstrapProjectConfig(target);

      expect(result.state).toBe('existed');
      const after = readFileSync(join(rithDir, 'config.yaml'), 'utf-8');
 ...
 *   - Default write targets ~/.rith/.env (home scope) with merge preserving
 *     existing non-empty values.
 *   - --scope project writes to <repo>/.rith/.env.
...
describe('writeScopedEnv (#1303)', () => {
  const ROOT = join(tmpdir(), 'rith-write-scoped-env-test-' + Date.now());
  const HOME_DIR = join(ROOT, 'rith-home');
  ...
  beforeEach(() => {
    ...
    process.env.RITH_HOME = HOME_DIR;
  });
  ...
  it('--force overwrites wholesale but writes a timestamped backup', () => {
    ...
    expect(result.backupPath).not.toBeNull();
    expect(result.backupPath).toMatch(/\.rith-backup-\d{4}-\d{2}-\d{2}T/);
  ...
  it('--scope project writes to <repo>/.rith/.env, creating the directory', () => {
    expect(existsSync(join(REPO_DIR, '.rith'))).toBe(false);
    const result = writeScopedEnv('FOO=bar\n', {
      scope: 'project',
      repoPath: REPO_DIR,
      force: false,
    });
    expect(result.targetPath).toBe(join(REPO_DIR, '.rith', '.env'));
  ...
  it('resolveScopedEnvPath returns the rith-owned path for each scope', () => {
    expect(resolveScopedEnvPath('home', REPO_DIR)).toBe(join(HOME_DIR, '.env'));
    expect(resolveScopedEnvPath('project', REPO_DIR)).toBe(join(REPO_DIR, '.rith', '.env'));
  });
...
describe('writeHomePiModelConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'rith-pi-config-'));
    originalHome = process.env.RITH_HOME;
    process.env.RITH_HOME = tmpDir;
  });

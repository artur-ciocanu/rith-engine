/**
 * Setup command - Interactive CLI wizard for Rith Engine credential configuration
 *
 ...
 *
 * Writes configuration to one rith-owned env file, chosen by --scope:
 *   - 'home'    (default)  → ~/.rith/.env
 *   - 'project'            → <repo>/.rith/.env
 *
...
import { copyRithSkill } from './skill';
import { homedir } from 'os';
import { randomBytes } from 'crypto';
import { spawn, execSync, spawnSync, type ChildProcess } from 'child_process';
import { execFileAsync } from '@rith/git';
import { getRegisteredProviders } from '@rith/providers';
import {
  getRithEnvPath as pathsGetRithEnvPath,
  getRepoRithEnvPath as pathsGetRepoRithEnvPath,
  getRithHome as pathsGetRithHome,
  createLogger,
} from '@rith/paths';
...
interface SetupConfig {
  ai: {
     ...
     *  in ~/.rith/.env. Required in compiled Rith Engine binaries; harmless in dev. */
    claudeBinaryPath?: string;
    codex: boolean;
    codexTokens?: CodexTokens;
    pi: boolean;
    /** e.g. 'anthropic/claude-haiku-4-5' — written to ~/.rith/config.yaml */
    piModel?: string;
...
/**
 * Get the Rith Engine home directory (typically ~/.rith)
 */
function getRithHome(): string {
 ...
 *
 * Compiled Rith Engine binaries need an explicit path because the Claude Agent
...
/**
 * Check for existing configuration at the selected scope's rith-owned env
...
export function checkExistingConfig(envPath?: string): ExistingConfig | null {
  const path = envPath ?? join(getRithHome(), '.env');
 ...
 * can re-run setup or hand-edit `.env` and `~/.rith/config.yaml`.
 */
 ...
 * `@rith/providers` so this should always pass, but catching broken compiled
 * builds at setup time is preferable to a silent runtime failure.
 *
 * The `loader` parameter is injected in tests so we don't need
 * `mock.module()` on `@rith/providers` (which would pollute other tests).
 */
export async function checkPiModule(
  loader: () => Promise<unknown> = () => import('@rith/providers')
...
async function collectClaudeBinaryPath(): Promise<string | undefined> {
  ...
  note(
    'Compiled Rith Engine binaries need CLAUDE_BIN_PATH set to the Claude Code executable.\n' +
  ...
  if (existingAuth) {
    ...
    note(
      'Codex requires authentication tokens.\n\n' +
        ...
        'You can skip Codex setup now and run `rith setup` again later.',
      'Codex Auth'
    );
  }
...
After installing Node.js, run 'rith setup' again.`,
          'Node.js Not Found'
        );
...
After upgrading, run 'rith setup' again.`,
  ...
  if (!hasClaude && !hasCodex && !hasPi) {
    log.warning('No AI assistant selected. You can add one later by running `rith setup` again.');
    return {
      claude: false,
      codex: false,
      pi: false,
  ...
  if (hasPi) {
    ...
    if (!piCheck.ok) {
      ...
      if (!continueWithoutPi) {
        cancel('Please check your Rith Engine installation and run setup again.');
        process.exit(0);
      }
      hasPi = false;
      piModel = undefined;
...
async function collectPlatforms(): Promise<SetupConfig['platforms']> {
  const platforms = await multiselect({
    message:
      'Which chat adapters do you want to connect? (all optional — Rith Engine works as CLI + skill without any)\n(↑↓ navigate, space select, enter confirm)',
    options: [
...
async function collectGitHubConfig(): Promise<GitHubConfig> {
  ...
  const customMention = await confirm({
    message: 'Do you want to set a custom @mention name? (Default: rith)',
  });
...
async function collectTelegramConfig(): Promise<TelegramConfig> {
  ...
  note(
    'Telegram Bot Setup\n\n' +
      ...
      '2. Send /newbot\n' +
      '3. Choose a display name (e.g., "My Rith Engine Bot")\n' +
  ...
  if (!allowedUserIds?.trim()) {
    log.warning(
      'No allowlist set — your Telegram bot will accept messages from ANYONE.\n' +
        'Add TELEGRAM_ALLOWED_USER_IDS to ~/.rith/.env after setup to restrict access.'
    );
  }
...
async function collectBotDisplayName(): Promise<string> {
  const customName = await confirm({
    message: 'Do you want to set a custom bot display name? (Default: Rith Engine)',
  });
...
export function generateEnvContent(config: SetupConfig): string {
  ...
  // Bot Display Name
  if (config.botDisplayName !== 'Rith Engine') {
  ...
  // them in sync. Writing a fixed PORT here risked a mismatch if ~/.rith/.env leaks a PORT that
 ...
 * so Docker (`/.rith`), the `RITH_HOME` override, and the "undefined"
...
export function resolveScopedEnvPath(scope: 'home' | 'project', repoPath: string): string {
  if (scope === 'project') return pathsGetRepoRithEnvPath(repoPath);
  return pathsGetRithEnvPath();
}

/**
 * Result of attempting to bootstrap project-scoped Rith Engine config.
 *  - `created`: `.rith/config.yaml` did not exist; we wrote a starter.
...
/**
 * Create `<projectPath>/.rith/config.yaml` with a commented-out template if
 ...
 * users' trees and Rith Engine's loaders handle their absence cleanly.
 */
export function bootstrapProjectConfig(projectPath: string): BootstrapProjectConfigResult {
  const rithDir = join(projectPath, '.rith');
  const configPath = join(rithDir, 'config.yaml');
  try {
    mkdirSync(rithDir, { recursive: true });
    ...
    writeFileSync(
      configPath,
      [
        '# Project-scoped Rith Engine config',
        '# Inherits defaults from ~/.rith/config.yaml.',
        '# Reference: https://github.com/artur-ciocanu/rith-engine/reference/configuration/',
        '#',
        '# Examples:',
        '#   assistants:',
        '#     claude:',
...
/**
 * Write the Pi model ref to `~/.rith/config.yaml` so Pi knows which backend
...
export function writeHomePiModelConfig(model: string): void {
  // Use the paths-package version of getRithHome so Docker (/.rith) is
  // handled correctly — the local getRithHome() always returns ~/.rith.
  const home = pathsGetRithHome();
...
function spawnWindowsTerminal(repoPath: string): SpawnResult {
  ...
  if (
    trySpawn('wt.exe', ['-d', repoPath, 'cmd', '/k', 'rith setup'], {
      detached: true,
      stdio: 'ignore',
    })
  ) {
  ...
  if (
    trySpawn('cmd.exe', ['/c', 'start', '""', '/D', repoPath, 'cmd', '/k', 'rith setup'], {
      detached: true,
      stdio: 'ignore',
    })
  ) {
  ...
  return { success: false, error: 'Could not open terminal. Please run `rith setup` manually.' };
}
...
function spawnMacTerminal(repoPath: string): SpawnResult {
  ...
  const script = `tell application "Terminal" to do script "cd '${escapedPath}' && rith setup"`;
...
function spawnLinuxTerminal(repoPath: string): SpawnResult {
  const setupCmd = 'rith setup; exec bash';
  ...
  return {
    success: false,
    error: 'Could not find a terminal emulator. Please run `rith setup` manually.',
  };
}

/**
 * Spawn a new terminal window with rith setup
 */
...
export async function setupCommand(options: SetupOptions): Promise<void> {
  ...
  if (options.spawn) {
    ...
    if (result.success) {
      ...
      console.log('');
      console.log(`    cd ${options.repoPath} && rith setup`);
      console.log('');
      console.log(
  ...
  // rith does NOT manage it — avoids confusion for users upgrading from
  ...
  if (existsSync(legacyRepoEnv)) {
    log.info(
      `Note: ${legacyRepoEnv} exists but is not managed by rith.\n` +
        ...
        '      Put rith env vars in ~/.rith/.env (home scope) or ' +
        `${join(options.repoPath, '.rith', '.env')} (project scope).`
    );
  }

  // Check for existing configuration at the selected scope (not unconditionally
  // ~/.rith/.env) so the Add/Update/Fresh decision reflects the actual target.
  ...
  // Pi model ref lives in ~/.rith/config.yaml, not the .env file, because
  ...
  if (config.ai.pi && config.ai.piModel) {
    try {
      ...
      // ~/.rith/config.yaml later. Surface the error so it's not silent.
      const e = err as NodeJS.ErrnoException;
  ...
  // Offer to install the Rith Engine skill
  const shouldCopySkill = await confirm({
    message: 'Install the Rith Engine skill in your project? (recommended)',
    initialValue: true,
  });
  ...
  if (shouldCopySkill) {
    ...
    s.start('Installing Rith Engine skill...');
    try {
      await copyRithSkill(skillTargetRaw);
    } catch (err) {
      s.stop('Rith Engine skill installation failed');
      cancel(`Could not install skill: ${(err as NodeJS.ErrnoException).message}`);
      process.exit(1);
    }
    s.stop('Rith Engine skill installed');
    skillInstalledPath = join(skillTargetRaw, '.claude', 'skills', 'rith');
  ...
  if (!isCancel(wantsDocsPath) && wantsDocsPath) {
    ...
    if (!isCancel(docsPath) && typeof docsPath === 'string' && docsPath.trim()) {
      try {
        const rithDir = join(options.repoPath, '.rith');
        mkdirSync(rithDir, { recursive: true });
        const configPath = join(rithDir, 'config.yaml');
        const existing = existsSync(configPath) ? readFileSync(configPath, 'utf-8') : '';
  ...
  if (skillInstalledPath) {
    summaryLines.push('');
    summaryLines.push('Rith Engine skill installed:');
  ...
  note(
    'Other settings you can customize in ~/.rith/.env:\n' +
      '  - PORT (default: 3090)\n' +
  ...
  note(
    'To update Rith Engine:\n' +
      '  Homebrew:  brew upgrade artur-ciocanu/rith-engine/rith\n' +
      '  curl:      curl -fsSL https://raw.githubusercontent.com/artur-ciocanu/rith-engine/main/scripts/install.sh | bash\n' +
      '  Docker:    docker pull ghcr.io/artur-ciocanu/rith-engine:latest',
    'Update Instructions'
  );

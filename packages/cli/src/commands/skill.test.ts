...
import { copyRithSkill, skillInstallCommand } from './skill';

describe('copyRithSkill', () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'rith-skill-test-'));
  });
  ...
  it('writes every bundled skill file under .claude/skills/rith/', async () => {
    await copyRithSkill(tempDir);

    const skillRoot = join(tempDir, '.claude', 'skills', 'rith');
  ...
  it('overwrites pre-existing skill files with bundled content', async () => {
    ...
    // Pre-seed with stale content; copyRithSkill must overwrite it.
    await copyRithSkill(tempDir);
...
describe('skillInstallCommand', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'rith-skill-cmd-test-'));
  ...
  it('returns 0 and installs the skill into the target directory', async () => {
    ...
    expect(existsSync(join(tempDir, '.claude', 'skills', 'rith', 'SKILL.md'))).toBe(true);

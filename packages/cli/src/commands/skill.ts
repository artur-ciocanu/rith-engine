/**
 * Skill command - Install bundled Rith Engine skill files into a project
 *
 * Writes the bundled SKILL.md, guides, references and examples into
 * <targetPath>/.claude/skills/rith/ so Claude Code picks up the skill
 * the next time the project is opened.
 *
 * Always overwrites existing files to ensure the latest skill version
 * shipped with the current Rith Engine binary is installed.
 */
import { existsSync, mkdirSync, writeFileSync } from 'fs';
import { dirname, join, resolve } from 'path';

/**
 * Copy the bundled Rith Engine skill files to <targetPath>/.claude/skills/rith/
 *
 ...
 * `rith setup` or `rith skill install`. Without this indirection, every
 * `rith` invocation — including `rith --help` — fails at module load when
 * the source skill files are missing from disk.
 */
export async function copyRithSkill(targetPath: string): Promise<void> {
  const { BUNDLED_SKILL_FILES } = await import('../bundled-skill');
  const skillRoot = join(targetPath, '.claude', 'skills', 'rith');
...
/**
 * Install the bundled Rith Engine skill into a project directory.
 *
...
export async function skillInstallCommand(targetPath: string): Promise<number> {
  ...
  const skillRoot = join(absoluteTarget, '.claude', 'skills', 'rith');
  try {
    const { BUNDLED_SKILL_FILES } = await import('../bundled-skill');
    const fileCount = Object.keys(BUNDLED_SKILL_FILES).length;
    console.log(`Installing Rith Engine skill (${fileCount} files) into ${skillRoot}`);

    await copyRithSkill(absoluteTarget);

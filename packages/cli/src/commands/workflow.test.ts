...
import { makeTestWorkflowWithSource } from '@rith/workflows/test-utils';
import {
  workflowListCommand,
  workflowRunCommand,
...
mock.module('@rith/workflows/event-emitter', () => ({
...
describe('workflowListCommand', () => {
  ...
  it('calls discoverWorkflowsWithConfig with (cwd, loadConfig) — home scope is internal', async () => {
    ...
    // After the globalSearchPath refactor, discovery reads ~/.rith/workflows/
...
describe('workflowRunCommand', () => {
  ...
  it('should throw error when no workflows found', async () => {
    ...
    await expect(workflowRunCommand('/test/path', 'assist', 'hello')).rejects.toThrow(
      'No workflows found in .rith/workflows/'
    );
  });
  ...
  it('should resolve workflow by suffix match', async () => {
    ...
    (discoverWorkflowsWithConfig as ReturnType<typeof mock>).mockResolvedValueOnce({
      workflows: [
        makeTestWorkflowWithSource({ name: 'rith-assist', description: 'Help' }),
    ...
    // Should resolve successfully — "assist" suffix-matches "rith-assist"
    await workflowRunCommand('/test/path', 'assist', 'hello');

    // Verify suffix matching tier was used
    expect(mockLogger.info).toHaveBeenCalledWith(
      expect.objectContaining({ requested: 'assist', matched: 'rith-assist' }),
  ...
  it('should resolve workflow by substring match', async () => {
    ...
    (discoverWorkflowsWithConfig as ReturnType<typeof mock>).mockResolvedValueOnce({
      workflows: [
        makeTestWorkflowWithSource({ name: 'rith-smart-pr-review', description: 'Smart review' }),
        makeTestWorkflowWithSource({ name: 'rith-assist', description: 'Help' }),
      ],
      errors: [],
    });

    // "smart" substring-matches only "rith-smart-pr-review"
  ...
  it('should throw ambiguous error for multiple suffix matches', async () => {
    ...
    (discoverWorkflowsWithConfig as ReturnType<typeof mock>).mockResolvedValueOnce({
      workflows: [
        makeTestWorkflowWithSource({ name: 'rith-review', description: 'Review' }),
  ...
  it('should throw ambiguous error for multiple substring matches', async () => {
    ...
    (discoverWorkflowsWithConfig as ReturnType<typeof mock>).mockResolvedValueOnce({
      workflows: [
        makeTestWorkflowWithSource({
          name: 'rith-comprehensive-pr-review',
  ...
  it('falls back to generic workspace hint when registration error has an unrecognized shape', async () => {
    ...
    expect(error.message).toContain('Check your Rith Engine workspace registration under');
...
describe('workflowStatusCommand', () => {
  ...
  it('should show node summaries in verbose mode', async () => {
    const workflowDb = await import('@rith/core/db/workflows');
    const workflowEventsDb = await import('@rith/core/db/workflow-events');
...
describe('workflowRunCommand — progress rendering', () => {
  ...
  function setupWorkflowMocks(): void {
    ...
    const discoverMock = require('@rith/workflows/workflow-discovery')
  ...
  it('should write node_started event to stderr', async () => {
    ...
    const { executeWorkflow } = require('@rith/workflows/executor');
...
describe('extractStaleWorkspaceEntry', () => {
  it('extracts the workspace dir from a POSIX source-symlink error', async () => {
    ...
    expect(
      extractStaleWorkspaceEntry(
        'Source symlink at /home/user/.rith/workspaces/acme/widget/source already points to /other, expected /here'
      )
    ).toBe('/home/user/.rith/workspaces/acme/widget');
  });
  ...
  it('extracts the workspace dir from a Windows source-symlink error (backslash sep)', async () => {
    ...
    expect(
      extractStaleWorkspaceEntry(
        'Source symlink at C:\\Users\\me\\.rith\\workspaces\\acme\\widget\\source already points to D:\\x, expected D:\\y'
      )
    ).toBe('C:\\Users\\me\\.rith\\workspaces\\acme\\widget');
  });

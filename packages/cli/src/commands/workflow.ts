 ...
 * thrown by `createProjectSourceSymlink` in @rith/paths. Cross-package
...
export async function workflowInstallCommand(
  ...
  const { findRepoRoot } = await import('@rith/git');
  ...
  if (isDirectoryUrl(entry.sourceUrl)) {
    await installDirectory(entry, slug, rithDir, force, existsSync, mkdirSync, writeFileSync);
  } else {
    await installSingleFile(entry, slug, rithDir, force, existsSync, mkdirSync, writeFileSync);
  }

...
export async function getCodebase(id: string): Promise<Codebase | null> {
  const result = await pool.query<Codebase>('SELECT * FROM codebases WHERE id = $1', [id]);
...
export async function updateCodebaseCommands(
  ...
  await pool.query(
    `UPDATE codebases SET commands = $1, updated_at = ${dialect.now()} WHERE id = $2`,
...
export async function getCodebaseCommands(
  ...
  }>('SELECT commands FROM codebases WHERE id = $1', [id]);
    ...
    try {
      ...
      throw new Error(
        `Corrupt commands JSON for codebase ${id}: unable to parse stored data. ` +
          `Run UPDATE codebases SET commands = '{}' WHERE id = '${id}' to reset.`
      );
    }
  } else {
    parsed = raw ?? {};
...
export async function findCodebaseByRepoUrl(repoUrl: string): Promise<Codebase | null> {
  const result = await pool.query<Codebase>('SELECT * FROM codebases WHERE repository_url = $1', [
    repoUrl,
  ]);
  return result.rows[0] || null;
}
...
export async function updateCodebase(
  ...
  const result = await pool.query(
    `UPDATE codebases SET ${updates.join(', ')} WHERE id = $${paramIndex}`,
    values
  );
...
export async function listCodebases(): Promise<readonly Codebase[]> {
  const result = await pool.query<Codebase>('SELECT * FROM codebases ORDER BY name ASC');
  return result.rows;
}

export async function deleteCodebase(id: string): Promise<void> {
  getLog().debug({ codebaseId: id }, 'db.codebase_delete_cascade_started');
  await pool.query('DELETE FROM codebases WHERE id = $1', [id]);

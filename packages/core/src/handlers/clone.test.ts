 ...
 *
 * Strategy:
 * - mock.module() for DB modules and @rith/paths (safe — no standalone test files for these)
 * - spyOn() for @rith/git (execFileAsync) and fs/promises (access, rm)
 *   to avoid process-global mock.module pollution that would break git.test.ts
 * - Lazy logger pattern means @rith/paths mock must be set up before the module import
 */
...
// ── Spies for fs/promises and @rith/git ──────────────────────────────────
...
describe('normalizeRepoUrl (via cloneRepository)', () => {
  ...
  test('HTTPS URL produces expected project source path', async () => {
    ...
    expect(targetPath).toBe('/home/test/.rith/workspaces/myorg/myproject/source');
  });
...
describe('RegisterResult shape', () => {
  ...
  test('cloneRepository result contains all expected fields', async () => {
    ...
    expect(result).toMatchObject({
      ...
      defaultCwd: '/home/test/.rith/workspaces/owner/repo/source',
      commandCount: 0,

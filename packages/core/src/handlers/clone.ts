...
async function registerRepoAtPath(
  ...
  if (existing) {
    // Determine if the new path is "better" (local > rith-managed clone)
    const isNewPathLocal = !targetPath.includes('/.rith/workspaces/');
    const isExistingPathManaged = existing.default_cwd.includes('/.rith/workspaces/');

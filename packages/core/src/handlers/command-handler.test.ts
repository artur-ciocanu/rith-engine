...
import * as gitUtils from '@rith/git';
import * as pathValidation from '../utils/path-validation';
import * as workflowDiscovery from '@rith/workflows/workflow-discovery';
  ...
  describe('handleCommand', () => {
    ...
    describe('/help', () => {
      test('should return help message', async () => {
        ...
        expect(result.message).toContain('Rith Engine Orchestrator');
    ...
    describe('/workflow run with load errors', () => {
      ...
      test('should match workflow name via suffix match', async () => {
        spyDiscoverWorkflows.mockResolvedValueOnce({
          workflows: [
            makeTestWorkflowWithSource({ name: 'rith-assist', description: 'General assistant' }),
          ],
          errors: [],
        });
        ...
        expect(result.success).toBe(true);
        expect(result.workflow?.definition.name).toBe('rith-assist');
      });
      ...
      test('should match workflow name via substring match', async () => {
        ...
        expect(result.success).toBe(true);
        expect(result.workflow?.definition.name).toBe('rith-smart-pr-review');
      });
      ...
      test('should return failure with candidates on ambiguous suffix match', async () => {
        ...
        expect(result.message).toContain('rith-review');

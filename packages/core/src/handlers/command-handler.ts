...
import { listWorktrees, execFileAsync, toRepoPath } from '@rith/git';
...
function shortenPath(absolutePath: string, repoRoot?: string): string {
  ...
  const workspacePath = getRithWorkspacesPath();
  ...
  switch (subcommand) {
    ...
    case 'ls': {
      ...
      if (workflowEntries.length === 0 && errors.length === 0) {
        return {
          success: true,
          message: 'No workflows found.\n\nCreate workflows in `.rith/workflows/` as YAML files.',
        };
      }
...
export async function handleCommand(
  ...
  switch (command) {
    case 'help':
      return {
        success: true,
        message: `## Rith Engine Orchestrator
...
- For project setup, ask the orchestrator: "How do I add a new project?"`,
    ...
    case 'commands': {
      ...
      if (!Object.keys(commands).length) {
        return {
          success: true,
          message: 'No commands registered.\n\nAdd .md files to .rith/commands/ in your project.',
        };
      }
    ...
    case 'init': {
      // Create .rith structure in current repo
      if (!conversation.cwd) {
        return {
          success: false,
      ...
      const rithDir = join(conversation.cwd, '.rith');
      const commandsDir = join(rithDir, 'commands');
      const configPath = join(rithDir, 'config.yaml');

      try {
        // Check if .rith already exists
        try {
          await access(rithDir);
          return {
            success: false,
            message: '.rith directory already exists. Nothing to do.',
          };
        } catch {
        ...
        const defaultConfig = `# Rith Engine repository configuration
# See: https://github.com/artur-ciocanu/rith-engine
...
`;
        ...
        return {
          success: true,
          message: `Created .rith structure:
  .rith/
  ├── config.yaml
  └── commands/
      └── example.md

Commands are auto-discovered from .rith/commands/ — no registration needed.`,
        };
      } catch (error) {
        const err = error as Error;

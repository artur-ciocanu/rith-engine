...
const mockEnsureRithWorkspacesPath = mock(() => Promise.resolve('/home/test/.rith/workspaces'));
mock.module('@rith/paths', () => ({
  createLogger: mock(() => mockLogger),
  getRithWorkspacesPath: mock(() => '/home/test/.rith/workspaces'),
  ensureRithWorkspacesPath: mockEnsureRithWorkspacesPath,
...
mock.module('../utils/worktree-sync', () => ({
  syncRithToWorktree: mock(() => Promise.resolve()),
}));
...
describe('parseOrchestratorCommands', () => {
  ...
  const orgProject = makeCodebase('artur-ciocanu/rith-engine');
  ...
  describe('project name matching', () => {
    ...
    test('matches project by last path segment (partial match)', () => {
      // "artur-ciocanu/rith-engine" matched by "rith-engine" (last segment)
      const response = '/invoke-workflow assist --project rith-engine';
      const result = parseOrchestratorCommands(response, codebases, workflows);

      expect(result.workflowInvocation).not.toBeNull();
      expect(result.workflowInvocation?.projectName).toBe('artur-ciocanu/rith-engine');
    });
    ...
    test('uses matched codebase name (not the input name) in result', () => {
      // Input "rith-engine" should resolve to full name "artur-ciocanu/rith-engine"
      const response = '/invoke-workflow assist --project rith-engine';
      const result = parseOrchestratorCommands(response, codebases, workflows);
  ...
  describe('/register-project parsing', () => {
    ...
    test('strips markdown bold from /register-project with quoted path', () => {
      const response =
        '**/register-project SaberEngine "/.rith/workspaces/b1skit/SaberEngine/source"**';
      const result = parseOrchestratorCommands(response, codebases, workflows);
      ...
      expect(result.projectRegistration?.projectPath).toBe(
        '"/.rith/workspaces/b1skit/SaberEngine/source"'
      );
    });
...
describe('discoverAllWorkflows — remote sync', () => {
  beforeEach(() => {
    ...
    mockEnsureRithWorkspacesPath.mockClear();
  ...
  test('calls syncWorkspace with codebase.default_cwd when conversation has codebase_id', async () => {
    ...
    // /repos/test-repo is NOT under ~/.rith/workspaces/ so resetAfterFetch=false
    ...
    expect(mockEnsureRithWorkspacesPath).toHaveBeenCalled();
  });
  ...
  test('passes resetAfterFetch=true for managed clones', async () => {
    ...
    expect(mockSyncWorkspace).toHaveBeenCalledWith(
      '/home/test/.rith/workspaces/owner/repo/source',
      undefined,
...
describe('handleMessage — multi-chunk command accumulation (regression)', () => {
  ...
  test('stream mode — register-project split across 3 chunks', async () => {
    mockParseCommand.mockReturnValueOnce({
      ...
      args: ['ExampleProject', '/.rith/workspaces/owner/repo/source'],
    });
    mockSendQuery.mockImplementationOnce(async function* () {
      yield { type: 'assistant', content: "I'll register the project now.\n\n/register-project " };
      yield { type: 'assistant', content: 'ExampleProject ' };
      yield { type: 'assistant', content: '"/.rith/workspaces/owner/repo/source"' };
    ...
    expect(mockCreateCodebase).toHaveBeenCalledWith({
      name: 'ExampleProject',
      default_cwd: '/.rith/workspaces/owner/repo/source',
      ai_assistant_type: 'claude',
    });
    ...
    expect(allCalls.some(([, msg]) => msg.includes('/.rith/workspaces/owner/repo/source'))).toBe(
      true
    );
  });

...
import type { SendQueryOptions, TokenUsage } from '@rith/providers/types';
import { ConversationNotFoundError } from '../types';
import * as db from '../db/conversations';
import * as codebaseDb from '../db/codebases';
import * as sessionDb from '../db/sessions';
import * as commandHandler from '../handlers/command-handler';
import { formatToolCall } from '@rith/workflows/utils/tool-formatter';
import { classifyAndFormatError } from '../utils/error-formatter';
import { toError } from '../utils/error';
import { getAgentProvider, getProviderCapabilities } from '@rith/providers';
import { getRithWorkspacesPath, ensureRithWorkspacesPath } from '@rith/paths';
import { syncRithToWorktree } from '../utils/worktree-sync';
import { syncWorkspace, toRepoPath } from '@rith/git';
import type { WorkspaceSyncResult } from '@rith/git';
...
import type { ApprovalContext } from '@rith/workflows/schemas/workflow-run';
...
export function parseOrchestratorCommands(
  ...
  if (invokeMatch) {
    ...
    if (workflow) {
      ...
      // e.g., "Rith" matches "artur-ciocanu/rith-engine"
...
async function discoverAllWorkflows(conversation: Conversation): Promise<DiscoverResult> {
  ...
  try {
    ...
    const result = await discoverWorkflowsWithConfig(getRithWorkspacesPath(), loadConfig);
  ...
  if (conversation.codebase_id) {
    try {
      ...
      if (codebase) {
        ...
        await syncRithToWorktree(workflowCwd);
...
export async function handleMessage(
  ...
  try {
    ...
    const cwd = await ensureRithWorkspacesPath();
...
async function handleWorkflowRunCommand(
  ...
  if (codebases.length === 1) {
    ...
    try {
      ...
      await platform.sendMessage(
        conversationId,
        `Failed to load workflows: ${err.message}\n\nCheck .rith/workflows/ for YAML syntax issues.`
      );
      return;
    }

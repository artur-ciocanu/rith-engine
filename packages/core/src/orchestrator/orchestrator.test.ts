...
import { makeTestWorkflow, makeTestWorkflowList } from '@rith/workflows/test-utils';
...
mock.module('@rith/paths', () => ({
  ...
  ensureRithWorkspacesPath: mock(() => Promise.resolve('/home/test/.rith/workspaces')),
...
// Worktree sync mock
const mockSyncRithToWorktree = mock(() => Promise.resolve(false));

mock.module('../utils/worktree-sync', () => ({
  syncRithToWorktree: mockSyncRithToWorktree,
}));
...
mock.module('@rith/workflows/executor', () => ({
  executeWorkflow: mockExecuteWorkflow,
  hydrateResumableRun: mock(() => Promise.resolve(null)),
}));
mock.module('@rith/workflows/router', () => ({
  findWorkflow: mockFindWorkflow,
}));
mock.module('@rith/workflows/utils/tool-formatter', () => ({
...
const testWorkflowDefs = makeTestWorkflowList(['fix-bug', 'add-feature', 'rith-assist']);
...
function clearAllMocks(): void {
  ...
  mockSyncRithToWorktree.mockClear();
...
describe('parseOrchestratorCommands', () => {
  ...
  test('parses --prompt with double quotes', () => {
    const response =
      'I will analyze this.\n/invoke-workflow rith-assist --project test-project --prompt "Analyze the orchestrator module architecture"';
    const result = parseOrchestratorCommands(response, codebases, workflows);

    expect(result.workflowInvocation).not.toBeNull();
    expect(result.workflowInvocation?.workflowName).toBe('rith-assist');
    expect(result.workflowInvocation?.projectName).toBe('test-project');
  ...
  test('parses --prompt with spaces in the quoted value', () => {
    const response =
      '/invoke-workflow rith-assist --project test-project --prompt "Analyze the database schema and migration patterns in the project, focusing on table structure and relationships"';
    const result = parseOrchestratorCommands(response, codebases, workflows);
  ...
  test('parses --prompt with --project= equals syntax', () => {
    const response =
      '/invoke-workflow rith-assist --project=test-project --prompt "Summarize the README"';
    const result = parseOrchestratorCommands(response, codebases, workflows);
...
describe('orchestrator-agent handleMessage', () => {
  ...
  describe('workflow routing via AI', () => {
    ...
    test('passes synthesizedPrompt to workflow dispatch instead of original message', async () => {
      ...
      mockClient.sendQuery.mockImplementation(async function* () {
        yield {
          type: 'assistant',
          content: `Running analysis.\n/invoke-workflow rith-assist --project test-project --prompt "${synthesized}"`,
        };
        yield { type: 'result', sessionId: 'session-id' };
      });
    ...
    test('sends error when workflow found in parsing but not in dispatch', async () => {
      ...
      mockClient.sendQuery.mockImplementation(async function* () {
        yield {
          type: 'assistant',
          content: '/invoke-workflow rith-assist --project test-project',
        };
        yield { type: 'result', sessionId: 'session-id' };
      });
      ...
      expect(platform.sendMessage).toHaveBeenCalledWith(
        'chat-456',
        expect.stringContaining('rith-assist')
      );
    });
  });
  ...
  describe('workflow discovery', () => {
    test('discovers global workflows from workspaces path', async () => {
      ...
      // Home-scoped workflows (~/.rith/workflows/) are discovered internally.
      expect(mockDiscoverWorkflows).toHaveBeenCalledWith(
        '/home/test/.rith/workspaces',
        expect.any(Function)
      );
    });
    ...
    test('syncs .rith to worktree before repo workflow discovery', async () => {
      mockGetOrCreateConversation.mockResolvedValue(mockConversationWithProject);
      mockGetCodebase.mockResolvedValue(mockCodebase);
      mockClient.sendQuery.mockImplementation(async function* () {
        yield { type: 'assistant', content: 'Response' };
      ...
      mockSyncRithToWorktree.mockImplementation(async () => {
        callOrder.push('sync');
        return false;
      });
      ...
      expect(mockSyncRithToWorktree).toHaveBeenCalledWith('/workspace/project');
      expect(callOrder).toEqual(['sync', 'discover-repo']);
    });

    test('handles workflow discovery failure gracefully', async () => {
      mockDiscoverWorkflows.mockRejectedValue(new Error('No .rith/workflows directory'));
      mockClient.sendQuery.mockImplementation(async function* () {

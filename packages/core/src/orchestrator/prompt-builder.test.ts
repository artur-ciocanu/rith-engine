...
describe('buildOrchestratorSystemAppend', () => {
  ...
  const workflows = [
    {
      ...
      nodes: [{ id: 'step1', command: 'rith-assist', depends_on: [] }],
    },
  ] as unknown as import('@rith/workflows/schemas/workflow').WorkflowDefinition[];
  ...
  test('returns orchestrator prompt when no codebase is scoped', () => {
    ...
    expect(result).toContain('# Rith Engine Orchestrator');

...
export function formatWorkflowSection(workflows: readonly WorkflowDefinition[]): string {
  if (workflows.length === 0) {
    return 'No workflows available. Users can create workflows in `.rith/workflows/` as YAML files.\n';
  }
...
- If the user is asking a question or it's unclear whether they want a workflow → answer their question directly. You may suggest a workflow by name (e.g., "I can run the **rith-assist** workflow for this if you'd like"), but do NOT include /invoke-workflow in your response.

Example (clear intent):
I'll analyze the orchestrator module architecture for you.
/invoke-workflow rith-assist --project my-project --prompt "Analyze the orchestrator module architecture: explain how it routes messages, manages sessions, and dispatches workflows to AI clients"

Example (ambiguous — answer directly):
User: "What do you think about adding dark mode?"
Response: "Adding dark mode would involve... [answer the question]. If you'd like me to create a plan for this, I can run the **rith-idea-to-pr** workflow."

## Project Setup

When a user asks to add a new project:
1. Clone the repository into ~/.rith/workspaces/:
   git clone https://github.com/{owner}/{repo} ~/.rith/workspaces/{owner}/{repo}/source
2. Register it by emitting this command on its own line:
   /register-project {project-name} {path-to-source}

Example:
   /register-project my-new-app /home/user/.rith/workspaces/user/my-new-app/source
...
IMPORTANT: Always clone into ~/.rith/workspaces/{owner}/{repo}/source unless the user specifies a different location.`;
}
...
export function buildOrchestratorPrompt(
  ...
  let prompt = `# Rith Engine Orchestrator

You are Rith Engine, an intelligent coding assistant that manages multiple projects.
Your working directory is ~/.rith/workspaces/ where all projects live.

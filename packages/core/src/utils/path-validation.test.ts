...
async function importFresh() {
  ...
  const rithPathsModulePath = require.resolve('@rith/paths');
  delete require.cache[modulePath];
  delete require.cache[rithPathsModulePath];
  return import('./path-validation');
}

// Default rith workspaces path
function getDefaultWorkspacesPath(): string {
  return join(homedir(), '.rith', 'workspaces');
}
...
describe('path-validation', () => {
  ...
  describe('isPathWithinWorkspace', () => {
    test('should allow paths within default rith workspaces', async () => {
    ...
    test('should use RITH_HOME env var when set', async () => {
      process.env.RITH_HOME = '/custom/rith';
      const { isPathWithinWorkspace } = await importFresh();
      expect(isPathWithinWorkspace('/custom/rith/workspaces/repo')).toBe(true);
      const defaultPath = getDefaultWorkspacesPath();
  ...
  describe('validateAndResolvePath', () => {
    ...
    test('should use custom RITH_HOME for validation and error message', async () => {
      process.env.RITH_HOME = '/my/custom/rith';
      const { validateAndResolvePath } = await importFresh();
      const customWorkspace = resolve('/my/custom/rith/workspaces');
      // Valid path under custom workspace
      expect(validateAndResolvePath('/my/custom/rith/workspaces/repo')).toBe(
        resolve('/my/custom/rith/workspaces/repo')
      );

...
// Rith Engine logo — the official shield mark. Uses the source PNG as the
...
// replaces the PNG with a blob URL exposed at window.__resources.rithLogo
...
function getRithLogoSrc() {
  if (typeof window !== "undefined" && window.__resources?.rithLogo) {
    return window.__resources.rithLogo;
  }
  return "assets/rith-logo.png";
}

// Multi-color logo (the brand mark) — renders the PNG at any size.
function RithMark({ size = 22, alt = "Rith Engine" }) {
  return (
    <img
      src={getRithLogoSrc()}
      width={size}
...
// PNG is canon.
function RithGlyph({ size = 22, color = "currentColor" }) {
  return (
    <svg
      width={size}
      height={size}
...
function RithLockup({ size = 22, color = "currentColor", subtitle = null, useGlyph = false }) {
  const Mark = useGlyph ? RithGlyph : RithMark;
  return (
...
window.RithMark = RithMark;
window.RithGlyph = RithGlyph;
window.RithLockup = RithLockup;
window.getRithLogoSrc = getRithLogoSrc;

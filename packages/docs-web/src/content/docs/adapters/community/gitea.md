---
title: Gitea
description: Connect Rith Engine to a Gitea instance for issue and PR automation.
category: adapters
area: adapters
audience: [operator]
sidebar:
...
Connect Rith Engine to a self-hosted Gitea instance so you can interact with your AI coding assistant from Gitea issues and pull requests.

## Prerequisites

- Rith Engine server running (see [Getting Started](/getting-started/overview/))
...
# Custom @mention name (defaults to BOT_DISPLAY_NAME, then "Rith Engine")
GITEA_BOT_MENTION=rith
```
...
```
@rith can you analyze this bug?
@rith review this implementation
@rith /workflow run assist "explain the auth flow"
```

**First mention behavior:**
- Automatically clones the repository to `~/.rith/workspaces/`
- Detects and loads commands from `.rith/commands/` if present
...
- **Self-loop prevention**: Bot comments include a hidden HTML marker (`<!-- rith-bot-response -->`) to avoid re-triggering on its own messages
...
Add a webhook with the same secret to each repository you want the bot to monitor. The webhook secret must be identical across all repos pointing to the same Rith Engine instance.

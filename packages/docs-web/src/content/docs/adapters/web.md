---
title: Web UI
description: Built-in web interface for interacting with Rith Engine -- no tokens or external services required.
category: adapters
area: adapters
audience: [user]
status: current
...
The Web UI is the built-in interface for interacting with Rith Engine. It requires no tokens, API keys, or external services -- just start the server and open your browser.

## Prerequisites

- Rith Engine installed and dependencies resolved (`bun install`)
...
- **Save** -- Saves the workflow YAML to your project's `.rith/workflows/` directory
...
- Which `.rith/commands/` and `.rith/workflows/` are loaded

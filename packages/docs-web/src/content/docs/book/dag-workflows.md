...
Rith Engine computes topological layers automatically. You describe the *what* (which nodes, which dependencies); Rith Engine figures out the *when*. The workflow above has three layers:
...
Create `.rith/workflows/classify-and-route.yaml`:
...
```bash
rith workflow run classify-and-route --branch fix/auth-issue "Users can't log in after password reset"
```
...
```bash
rith workflow run classify-and-route --branch feature/dark-mode "Add dark mode support"
```
...
If the expression is invalid or can't be evaluated, Rith Engine fails open — the node runs rather than silently skipping.
...
`output_format` tells Rith Engine to enforce JSON output from an AI node. Pass a JSON Schema and Rith Engine will ensure the node returns data in that shape:
...
- id: analyze
  script: analyze-metrics        # Named script: .rith/scripts/analyze-metrics.py
  runtime: uv
...
**Let DAG resume handle failures.** If a long workflow fails partway through, run `rith workflow run <name> --resume` (or `rith workflow resume <id>`) to skip nodes that already completed and continue from where it left off. Plain `rith workflow run` always starts fresh.

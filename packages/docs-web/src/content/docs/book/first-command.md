...
description: Write your first Rith Engine command file — a focused markdown prompt that the AI executes as a single task.
category: book
part: customization
audience: [user]
sidebar:
...
You've seen commands do real work — investigating issues, writing code, posting reviews. In [Chapter 3](/book/how-it-works/), we traced how `rith-fix-github-issue` stitched seven of them together. Now you're going to write one yourself.
...
A **command** is a markdown file that tells the AI exactly what to do in one focused task. It's the atomic unit of Rith Engine — the smallest thing that can run independently or be wired into a workflow.

Commands live in your repository at `.rith/commands/`. When Rith Engine runs a step like `command: run-tests`, it finds `.rith/commands/run-tests.md`, substitutes any variables, and sends the whole document to the AI as its task instructions.

That's it. Commands are prompts, not code. You write what you want the AI to do, and it does it.

> **Where to put them**: Create a `.rith/commands/` directory in any git repository you're working with. Rith Engine finds commands there automatically alongside any bundled defaults.
...
**The frontmatter** (the `---` block at the top) is optional but recommended. The `description` field is what appears when someone runs `rith workflow list` or asks the AI which commands are available. The `argument-hint` tells users what they're expected to provide.
...
```bash
mkdir -p .rith/commands
touch .rith/commands/run-tests.md
```
...
You can invoke a command directly through `rith-assist`:

```bash
rith workflow run rith-assist "/command-invoke run-tests auth"
```

Rith Engine routes the `/command-invoke run-tests` instruction to the AI, which finds your `.rith/commands/run-tests.md`, substitutes `$ARGUMENTS` with `auth`, and runs the task.
...
| `$ARTIFACTS_DIR` | Absolute path to this run's artifact directory | `/home/user/.rith/workspaces/owner/repo/artifacts/runs/abc123/` |
...
```bash
rith workflow run rith-assist "/commands"
```

This lists every command available — your custom ones from `.rith/commands/` alongside Rith Engine's bundled defaults. The bundled commands (like `rith-investigate-issue` and `rith-fix-issue`) are good reference material when you're deciding how to structure your own.

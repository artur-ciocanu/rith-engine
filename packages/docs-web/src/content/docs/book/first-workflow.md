...
A **workflow** is a YAML file in `.rith/workflows/`. When you run `rith workflow run my-workflow "do something"`, Rith Engine finds the file, reads the nodes, and executes them in dependency order.
...
That's it. Three fields at the top, a list of nodes below. Each node needs a unique `id`. Rith Engine discovers workflow files recursively inside `.rith/workflows/`, so you can organize them in subdirectories if you want.

> **Where to put it**: Create `.rith/workflows/my-workflow.yaml` in your repository. Run `rith workflow list` to confirm Rith Engine found it.
...
Create `.rith/workflows/my-workflow.yaml`:
...
```bash
rith workflow run my-workflow --branch feature/auth-tokens "Add JWT refresh token support"
```

Rith Engine runs `rith-create-plan` with your input, waits for it to finish, then runs `rith-implement-tasks`. The AI carries its full conversation context from the planning node into the implementation node — it knows what it planned and can act on it immediately.
...
nodes:
  ...
  - id: code-review
    ...
    context: fresh
  - id: error-handling
    command: rith-error-handling-agent
    depends_on: [validate]
    context: fresh
  - id: test-coverage
    command: rith-test-coverage-agent
    depends_on: [validate]
    context: fresh
```

Nodes `code-review`, `error-handling`, and `test-coverage` all depend on `validate` but not on each other — Rith Engine runs them concurrently. Each agent gets its own fresh AI session. Rith Engine waits for all three to finish before moving to the next node.
...
nodes:
  ...
  - id: test-coverage
    ...
    context: fresh
  - id: self-fix
    command: rith-implement-review-fixes
    depends_on: [code-review, error-handling, test-coverage]
    context: fresh
```

The `rith-implement-review-fixes` command reads the artifacts written by all three review agents, synthesizes their findings, and implements the recommended changes. `context: fresh` keeps it focused on the review findings rather than the full implementation history.
...
You've just built a mini version of `rith-idea-to-pr` — the same structure, condensed. That bundled workflow adds a few more nodes (scope confirmation, PR creation, final summary), but the core pattern is identical to what you built here.

...
| `rith workflow run <name> "<prompt>"` | Run a workflow |
| `rith workflow run <name> --branch <name> "<prompt>"` | Run with an explicit branch |
| `rith workflow run <name> --no-worktree "<prompt>"` | Run in the live checkout (no isolation) |
| `rith workflow run <name> --cwd /path "<prompt>"` | Run against a specific directory |
| `rith workflow status` | Show status of active workflow runs |
| `rith workflow resume <run-id>` | Resume a failed workflow run |
| `rith workflow abandon <run-id>` | Abandon a non-terminal workflow run |
| `rith workflow cleanup [days]` | Delete old workflow run records (default: 7 days) |

### `rith isolation`

| Command | Description |
|---------|-------------|
| `rith isolation list` | List all active worktrees |
| `rith isolation cleanup` | Remove stale worktrees (older than 7 days) |
| `rith isolation cleanup <days>` | Remove stale worktrees older than N days |
| `rith isolation cleanup --merged` | Remove worktrees whose branches merged into main |
| `rith isolation cleanup --merged --include-closed` | Also remove worktrees with closed (abandoned) PRs |

### `rith complete`

| Command | Description |
|---------|-------------|
| `rith complete <branch>` | Remove worktree, local branch, and remote branch |
...
rith workflow run my-workflow "auth"
# $ARGUMENTS = "auth", $1 = "auth"

# Multi-argument
rith workflow run my-workflow "auth refresh-tokens"
...
**See what Rith Engine found:**
```bash
rith workflow list          # Are your workflows loaded?
rith validate workflows     # Any YAML errors?
rith isolation list         # Any stale worktrees?
```

**Enable verbose logging:**
```bash
rith --verbose workflow run my-workflow "..."
```

**Check execution logs** — each run writes a JSONL log:
```
~/.rith/workspaces/<owner>/<repo>/logs/
```

**Run without isolation** to simplify debugging:
```bash
rith workflow run my-workflow --no-worktree "..."
```

**Test a command directly** before embedding it in a workflow:
```bash
rith workflow run rith-assist "/command-invoke my-command some-arg"
```

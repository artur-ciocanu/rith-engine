---
title: CLI Internals
description: Technical reference for the Rith Engine CLI package — entry point flow, command routing, worktree logic, and adapter details.
...
│   [rith] stripped N keys from <cwd> (...) when N > 0.         │
└─────────────────────────────────┬───────────────────────────────┘
                                  │
                                  ▼
┌─────────────────────────────────────────────────────────────────┐
│ loadRithEnv(cwd)  — both loads use override: true             │
│   1. ~/.rith/.env        (home scope)                         │
│   2. <cwd>/.rith/.env    (repo scope, wins over home)         │
│   Emits one [rith] loaded N keys from <path> line per file    │
│   when N > 0 and RITH_VERBOSE_BOOT=1 or LOG_LEVEL=debug/trace.│
└─────────────────────────────────┬───────────────────────────────┘
                                  │
                                  ▼
┌─────────────────────────────────────────────────────────────────┐
...
│ - Searches .rith/workflows/ recursively                        │
...
```
┌─────────────────────────────────────────────────────────────────┐
│ rith workflow run <name> [message] [--branch X] [--from X] [--no-worktree]│
└─────────────────────────────────┬───────────────────────────────┘
                                  │
                                  ▼
┌─────────────────────────────────────────────────────────────────┐
...
```
┌──────────────────────────────────────────────────────────────────┐
│ rith workflow event emit --run-id <uuid> --type <type> [...]   │
└──────────────────────────────┬───────────────────────────────────┘
                               │
                               ▼
┌──────────────────────────────────────────────────────────────────┐
...
└─────────────────────────────────┬───────────────────────────────┘
                                  ...
                                  ▼
┌─────────────────────────────────────────────────────────────────┐
│ @rith/core isolationDb.listAllActiveWithCodebase()            │
...
|----------|---------|----------|---------|
| `discoverWorkflowsWithConfig(cwd, config)` | `@rith/workflows/workflow-discovery` | `workflows/src/workflow-discovery.ts` | Find and parse workflow YAML |
| `executeWorkflow(...)` | `@rith/workflows/executor` | `workflows/src/executor.ts` | Run workflow steps |
| `getIsolationProvider()` | `@rith/isolation` | `isolation/src/factory.ts` | Get WorktreeProvider singleton |
| `conversationDb.*` | `@rith/core` | `core/src/db/conversations.ts` | Conversation CRUD |
| `codebaseDb.*` | `@rith/core` | `core/src/db/codebases.ts` | Codebase CRUD |
| `isolationDb.*` | `@rith/core` | `core/src/db/isolation-environments.ts` | Worktree tracking |
| `git.*` | `@rith/git` | `packages/git/src/` | Git operations |
| `closeDatabase()` | `@rith/core` | `core/src/db/connection.ts` | Clean shutdown |
...
Worktrees stored at: `~/.rith/workspaces/<owner>/<repo>/worktrees/<branch-slug>/`

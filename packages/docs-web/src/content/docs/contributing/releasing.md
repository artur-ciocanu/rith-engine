---
title: Releasing
description: How to create a new release of the Rith Engine CLI — version management, release process, and troubleshooting.
...
This guide covers how to create a new release of the Rith Engine CLI.
...
# Review and commit
git diff homebrew/rith.rb
git add homebrew/rith.rb
git commit -m "chore: update Homebrew formula for vX.Y.Z"
git push origin main
```

If you maintain a Homebrew tap (`homebrew-rith`), copy the updated formula there.
...
> gh release download v0.2.0 --repo artur-ciocanu/rith-engine \
>   --pattern "rith-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/x86_64/x64/;s/aarch64/arm64/')" \
>   --dir /tmp/rith-install
>
> # Install the binary
> chmod +x /tmp/rith-install/rith-*
> sudo mv /tmp/rith-install/rith-* /usr/local/bin/rith
>
> # Verify
> rith version
> ```

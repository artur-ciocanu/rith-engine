...
description: Deploy Rith Engine to a cloud VPS with automatic HTTPS via Caddy and persistent uptime.
category: deployment
area: infra
audience: [operator]
status: current
...
Deploy Rith Engine to a cloud VPS for 24/7 operation with automatic HTTPS and persistent uptime.
...
- Domain name or subdomain (e.g., `rith.yourdomain.com`)
...
2. Create an **A Record**:
   - **Name:** `rith` (for `rith.yourdomain.com`) or `@` (for `yourdomain.com`)
...
sudo chown deploy:deploy /opt/rith
...
# Server settings
PORT=3090
RITH_HOME=/tmp/rith  # Override base directory (optional)
```
...
# Look for:
# [App] Starting Rith Engine
# [Database] Connected successfully
# [App] Rith Engine is ready!
```
...
```bash
# Basic health check
curl https://rith.yourdomain.com/api/health
# Expected: {"status":"ok"}

# Database connectivity
curl https://rith.yourdomain.com/api/health/db
# Expected: {"status":"ok","database":"connected"}

# Concurrency status
curl https://rith.yourdomain.com/api/health/concurrency
# Expected: {"status":"ok","active":0,"queued":0,"maxConcurrent":10}
```

### Check SSL Certificate

Visit `https://rith.yourdomain.com/api/health` in your browser:
...
| Field                | Value                                                                        |
| -------------------- | ---------------------------------------------------------------------------- |
| **Payload URL**      | `https://rith.yourdomain.com/webhooks/github`                              |
...
```bash
curl https://rith.yourdomain.com/webhooks/github

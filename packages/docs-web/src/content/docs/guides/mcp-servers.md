...
1. Create an MCP config file (e.g., `.rith/mcp/github.json`):
...
Rith Engine also accepts the common wrapper format `{ "mcpServers": { ... } }`; this
...
nodes:
  - id: fetch-context
    ...
    mcp: .rith/mcp/all-services.json
    allowed_tools: []
...
Some built-in workflows (like `rith-smart-pr-review`) include an optional
...
2. Open the app, tap "+", subscribe to a topic name (e.g. `rith-yourname-a8f3x`).
   Treat the topic name like a password — anyone who knows it can send you notifications.
3. Create `.rith/mcp/ntfy.json` in your repo:
...
```json
{
  "ntfy": {
    ...
    "args": ["-y", "ntfy-me-mcp"],
    "env": {
      "NTFY_TOPIC": "rith-yourname-a8f3x"
    }
  }
}
```

That's it. The file is gitignored (`.rith/mcp/` is in `.gitignore`), so your
topic stays local.
...
If `.rith/mcp/ntfy.json` doesn't exist, `check-ntfy` outputs `false`, the
...
curl -d "Hello from Rith Engine" ntfy.sh/YOUR_TOPIC_NAME

# Run a workflow with notifications
bun run cli workflow run rith-smart-pr-review "Review PR #123"
```
...
| `MCP config file is not valid JSON` | Syntax error in JSON | Validate with `cat .rith/mcp/config.json \| python3 -m json.tool` |

...
nodes:
  - id: fetch-pages
    script: fetch-github-pages   # resolves .rith/scripts/fetch-github-pages.ts
    runtime: bun
    timeout: 60000
```

The file `.rith/scripts/fetch-github-pages.ts` is loaded and executed with
...
|-------|------|----------|-------------|
| `script` | string | Yes | Inline code, or the basename (no extension) of a file in `.rith/scripts/` or `~/.rith/scripts/` |
...
1. `<repoRoot>/.rith/scripts/` — repo-local
2. `~/.rith/scripts/` — home-scoped (shared across every repo)

Each directory is walked one subfolder deep (e.g. `.rith/scripts/triage/foo.ts`
...
vars you've configured via the `env:` block in `.rith/config.yaml`. This is
the same injection surface used by Pi and bash nodes.
...
the script. Rith Engine-managed env (from `~/.rith/.env` and `<repo>/.rith/.env`)
...
`rith validate workflows <name>` checks script nodes for:

- **Script file exists** — for named scripts, the basename must exist in
  `.rith/scripts/` or `~/.rith/scripts/` with a matching extension for
...
### Reusable helper in `~/.rith/scripts/`

A helper you want available in every repo — say, a triage summary formatter —
lives at `~/.rith/scripts/triage-fmt.ts`:

```typescript
// ~/.rith/scripts/triage-fmt.ts
...
- [Global Workflows, Commands, and Scripts](/guides/global-workflows/) — home-scoped `~/.rith/scripts/`

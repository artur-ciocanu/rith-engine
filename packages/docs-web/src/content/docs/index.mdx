---
title: Rith Engine
description: AI workflow engine — package your coding workflows as YAML, run them anywhere.
template: splash
hero:
  title: Rith Engine
  image:
    file: ../../assets/logo.png
    alt: Rith Engine Logo
  tagline: Package your AI coding workflows as YAML. Run them from the CLI.
  actions:
    - text: Get Started
      link: /getting-started/installation/
      icon: right-arrow
      variant: primary
    - text: Roadmap
      link: /roadmap/
      icon: right-arrow
      variant: secondary
    - text: View on GitHub
      link: https://github.com/artur-ciocanu/rith-engine
      icon: external
      variant: minimal
---

import { Card, CardGrid } from '@astrojs/starlight/components';

## Install in seconds

:::code-group

```bash [macOS / Linux]
curl -fsSL https://github.com/artur-ciocanu/rith-engine/install | bash
```

```powershell [Windows]
irm https://github.com/artur-ciocanu/rith-engine/install.ps1 | iex
```

```bash [Homebrew]
brew install coleam00/rith/rith
```

```bash [Docker]
docker run --rm -v "$PWD:/workspace" ghcr.io/artur-ciocanu/rith-engine:latest workflow list
```

:::

## What is Rith Engine?

Rith Engine is a **workflow engine for AI coding agents**. Define multi-step development workflows in YAML — code review, bug fixes, feature implementation, testing — and run them with a single command.

<CardGrid>
  <Card title="Repeatable" icon="document">
    Package your best AI coding patterns as shareable YAML workflows
  </Card>
  <Card title="Isolated" icon="laptop">
    Each workflow runs in its own git worktree — no conflicts, no mess
  </Card>
  <Card title="Portable" icon="puzzle">
    Run from the CLI with full Git isolation and worktree management
  </Card>
  <Card title="Composable" icon="setting">
    Chain nodes into DAGs with dependencies, loops, and conditional logic
  </Card>
  <Card title="Multi-provider" icon="rocket">
    Works with ~20 LLM backends via Pi Coding Agent
  </Card>
</CardGrid>

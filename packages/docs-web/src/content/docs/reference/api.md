---
title: API Reference
description: REST API endpoints for programmatic access to Rith Engine.
category: reference
area: server
audience: [developer]
sidebar:
...
Rith Engine exposes a REST API via a [Hono](https://hono.dev/) server with OpenAPI spec generation. All endpoints are prefixed with `/api/`.
...
Override the port with the `PORT` environment variable or let Rith Engine auto-allocate when running inside a worktree (range 3190-4089).
...
None. Rith Engine is a single-developer tool -- there is no authentication on the API by default. If you expose Rith Engine on a network, use a reverse proxy or firewall to restrict access.
...
When `cwd` is omitted, Rith Engine returns bundled default workflows and any from `~/.rith/workflows/` (home-scoped). Project-specific workflows require either the `cwd` query param or a registered codebase, so the endpoint is useful on first launch before any project is registered.
...
```bash
curl http://localhost:3090/api/workflows/rith-assist
```

Query parameters:
- `cwd` (optional) -- Working directory for project-specific lookup

Returns `{ workflow, filename, source: "project" | "global" | "bundled" }`. The endpoint auto-discovers across all three scopes in order (project → home-scoped → bundled). `source: "global"` is returned when the workflow comes from `~/.rith/workflows/`.
...
Query parameters:
- `cwd` (optional) -- Target directory (must have `.rith/workflows/`)
- `source` (optional, enum: `project` \| `global`) -- Scope to write the workflow to. Defaults to `project` (writes to `<cwd>/.rith/workflows/`). Pass `source=global` to write to the home-scoped location (`~/.rith/workflows/`). Returns `400 "Invalid workflow source"` if any other value is supplied.
...
Query parameters:
- `cwd` (optional) -- Target directory (must have `.rith/workflows/`)
- `source` (optional, enum: `project` \| `global`) -- Scope to delete from. Defaults to `project`. Pass `source=global` to delete from `~/.rith/workflows/`. Returns `400 "Invalid workflow source"` if any other value is supplied.
...
```bash
# JSON (no attachments)
curl -X POST http://localhost:3090/api/workflows/rith-assist/run \
  -H "Content-Type: application/json" \
...
Resumes the workflow from where it left off, skipping already-completed nodes. Equivalent to `rith workflow resume <run-id>` from the CLI. Plain `rith workflow run <name>` invocations never resume implicitly.

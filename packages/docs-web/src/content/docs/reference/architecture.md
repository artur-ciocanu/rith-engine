---
title: Architecture
description: Comprehensive guide to Rith Engine's system architecture, packages, interfaces, and data flow.
category: reference
audience: [developer]
status: current
sidebar:
...
Comprehensive guide to understanding and extending Rith Engine.
...
```
PRIMARY: ~/.rith/workspaces/<owner>/<repo>/worktrees/<branch>/
LEGACY:  ~/.rith/worktrees/<owner>/<repo>/<branch>/   (fallback for repos not registered under workspaces/)
DOCKER:  /.rith/workspaces/<owner>/<repo>/worktrees/<branch>/
```

**Path resolution:**

1. Project registered under `workspaces/`? -> `~/.rith/workspaces/<owner>/<repo>/worktrees/<branch>/`
2. Legacy fallback -> `~/.rith/worktrees/<owner>/<repo>/<branch>/`
3. Docker detected? -> `/.rith/` prefix instead of `~/.rith/`
...
Orchestrator: Route to workflow via AI router
           |
Read command file: .rith/commands/plan.md
           |
...
{
  "prime": {
    "path": ".rith/commands/prime.md",
    "description": "Research codebase"
  },
  "plan": {
    "path": ".rith/commands/plan-feature.md",
...
```bash
/command-set analyze .rith/commands/analyze.md
```

**Bulk loading** (`/load-commands`):

```bash
/load-commands .rith/commands
...
**Reference:** `packages/paths/src/rith-paths.ts` (`@rith/paths`)
...
Rith Engine uses a 7-table schema with `remote_agent_` prefix. SQLite is the default (zero setup); PostgreSQL is optional for cloud/advanced deployments.
...
Command Handler: /clone
  ...
  - Detect .rith/commands/
         |
Send response: "Repository cloned! Found: .rith/commands/"
```
...
Orchestrator: Route via AI router
         |
Load command file: .rith/commands/prime.md
         |
...
```
User comments: @Rith Engine prime the codebase
         |
...
- [ ] Add example command file to `.rith/commands/`

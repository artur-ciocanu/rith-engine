...
description: Complete reference for all variable substitutions available in Rith Engine commands and workflows.
category: reference
area: workflows
audience: [user]
sidebar:
...
| `$ARTIFACTS_DIR` | Pre-created external artifacts directory (`~/.rith/workspaces/<owner>/<repo>/artifacts/runs/<id>/`) | Always exists before node execution; stored outside the repo to avoid polluting the working tree |
| `$BASE_BRANCH` | Base branch for git operations | Auto-detected from the repository's default branch, or set via `worktree.baseBranch` in `.rith/config.yaml`. Throws an error if referenced in a prompt but cannot be resolved |
| `$DOCS_DIR` | Documentation directory path | Configured via `docs.path` in `.rith/config.yaml`. Defaults to `docs/` when not set. Never throws |
...
- `worktree.baseBranch` is not set in `.rith/config.yaml`
...
These are standard environment variables read from `process.env` at clone time. They are **not** workflow-substituted variables — they must be set in your shell environment or `.env` file before Rith Engine starts.

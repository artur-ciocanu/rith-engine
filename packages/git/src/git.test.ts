...
// ---------------------------------------------------------------------------
// Mock @rith/paths: suppress logger, pass-through path functions
// ---------------------------------------------------------------------------
...
/** Mirror of @rith/paths getRithHome (reads env at call-time) */
function getRithHome(): string {
  ...
  return process.env.RITH_HOME ?? join(homedir(), '.rith');
}

mock.module('@rith/paths', () => ({
  createLogger: mock(() => mockLogger),
  getRithWorktreesPath: () => join(getRithHome(), 'worktrees'),
  getRithWorkspacesPath: () => join(getRithHome(), 'workspaces'),
  getProjectWorktreesPath: (owner: string, repo: string) =>
    join(getRithHome(), 'workspaces', owner, repo, 'worktrees'),
}));
...
describe('git utilities', () => {
  ...
  describe('getWorktreeBase', () => {
    ...
    test('returns workspace-scoped base for a local non-workspace repo (via path fallback)', () => {
      ...
      // living outside ~/.rith/workspaces/, owner/repo is derived from the last
      ...
      expect(result).toEqual({
        base: join(homedir(), '.rith', 'workspaces', 'workspace', 'my-repo', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });

    test('uses RITH_HOME for the workspace-scoped base (local non-Docker)', () => {
      delete process.env.WORKSPACE_PATH;
      delete process.env.WORKTREE_BASE;
      delete process.env.RITH_DOCKER;
      ...
      const result = git.getWorktreeBase('/workspace/my-repo');
      expect(result).toEqual({
        base: join('/custom/rith', 'workspaces', 'workspace', 'my-repo', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });

    test('uses the Docker rith home for the workspace-scoped base', () => {
      delete process.env.RITH_HOME;
      process.env.RITH_DOCKER = 'true';
      const result = git.getWorktreeBase('/workspace/my-repo');
      expect(result).toEqual({
        base: join('/', '.rith', 'workspaces', 'workspace', 'my-repo', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });
    ...
    test('returns workspace-scoped path when repo is already under workspaces/', () => {
      ...
      delete process.env.RITH_DOCKER;
      delete process.env.RITH_HOME;
      const workspacesPath = join(homedir(), '.rith', 'workspaces');
    ...
    test('workspace-scoped path honors RITH_HOME override', () => {
      delete process.env.WORKSPACE_PATH;
      delete process.env.RITH_DOCKER;
      process.env.RITH_HOME = join('/', 'custom', 'rith');
      const repoPath = join('/', 'custom', 'rith', 'workspaces', 'acme', 'widget', 'source');
      const result = git.getWorktreeBase(repoPath);
      expect(result).toEqual({
        base: join('/', 'custom', 'rith', 'workspaces', 'acme', 'widget', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });
    ...
    test('uses codebaseName to resolve workspace-scoped path for a local repo', () => {
      ...
      expect(result).toEqual({
        base: join(homedir(), '.rith', 'workspaces', 'Widinglabs', 'sasha-demo', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });
    ...
    test('ignores invalid codebaseName and falls back to path-derived owner/repo', () => {
      ...
      expect(result).toEqual({
        base: join(homedir(), '.rith', 'workspaces', 'local', 'repo', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });

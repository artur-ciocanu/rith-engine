...
import { createLogger, getRithWorkspacesPath, getProjectWorktreesPath } from '@rith/paths';
 ...
 * or inside the user's rith workspace area (default for every repo):
 *
 * - `repo-local`       — `<repoRoot>/<override.repoLocal>/`  (opt-in per repo config)
 * - `workspace-scoped` — `~/.rith/workspaces/<owner>/<repo>/worktrees/`  (default)
 *
...
/**
 * Resolve the `{ owner, repo }` identity used to scope rith-managed worktrees.
 *
 * Precedence:
 *   1. Explicit `codebaseName` in `owner/repo` format (from the database / web UI)
 *   2. Path segments when `repoPath` is already under `~/.rith/workspaces/owner/repo/`
 ...
 *   2. Otherwise             → `~/.rith/workspaces/<owner>/<repo>/worktrees/`
 ...
 * base; there is no `~/.rith/worktrees/owner/repo/` fallback layout.
 */

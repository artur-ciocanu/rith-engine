...
const TEST_RITH_HOME = '/test/.rith';

// Mock @rith/paths: provide getRithHome + workspaces path helpers so @rith/git (getWorktreeBase,
// isProjectScopedWorktreeBase) and worktree.ts resolve paths against TEST_RITH_HOME consistently.
mock.module('@rith/paths', () => ({
  ...
  getRithHome: () => TEST_RITH_HOME,
  getRithWorkspacesPath: () => join(TEST_RITH_HOME, 'workspaces'),
  getRithWorktreesPath: () => join(TEST_RITH_HOME, 'worktrees'),
  getProjectWorktreesPath: (owner: string, repo: string) =>
    join(TEST_RITH_HOME, 'workspaces', owner, repo, 'worktrees'),
  isDocker: () => false,
}));
...
describe('WorktreeProvider', () => {
  ...
  describe('generateBranchName', () => {
    test('generates issue-N for issue workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('rith/issue-42');
    });
    ...
    test('generates pr-N-review for fork PR workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('rith/pr-123-review');
    });
    ...
    test('generates review-N for review workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('rith/review-456');
    });
    ...
    test('generates thread-{hash} for thread workflows', () => {
      ...
      expect(name).toMatch(/^rith\/thread-[a-f0-9]{8}$/);
    });
    ...
    test('generates task-{slug} for task workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('rith/task-add-dark-mode');
    });
  ...
  describe('create', () => {
    ...
    test('creates worktree for issue workflow', async () => {
      ...
      expect(env.branchName).toBe('rith/issue-42');
    ...
    test('throws when branch already exists and fromBranch is specified', async () => {
      ...
      alreadyExistsError.stderr = "fatal: a branch named 'rith/task-test-adapters' already exists";
      ...
      await expect(provider.create(request)).rejects.toThrow(
        'Branch "rith/task-test-adapters" already exists. Cannot create it from "feature/extract-adapters".'
      );
    });
    ...
    test('resets and reuses existing branch when it already exists and no fromBranch', async () => {
      ...
      expect(execSpy).toHaveBeenCalledWith(
        'git',
        ['-C', '/workspace/repo', 'branch', '-f', 'rith/task-test-adapters', 'origin/main'],
        expect.any(Object)
      );
      ...
      expect(execSpy).toHaveBeenCalledWith(
        'git',
        ['-C', '/workspace/repo', 'worktree', 'add', expect.any(String), 'rith/task-test-adapters'],
        expect.any(Object)
      );
    });
    ...
    test('adopts existing worktree when repo ownership matches', async () => {
      ...
      mockReadFile.mockResolvedValue('gitdir: /workspace/repo/.git/worktrees/rith/issue-42\n');
    ...
    test('throws when worktree belongs to different repo root (cross-checkout)', async () => {
      worktreeExistsSpy.mockResolvedValue(true);
      mockReadFile.mockResolvedValue('gitdir: /different/repo/.git/worktrees/rith/issue-42\n');
    ...
    test('resets stale branch to start-point when it already exists', async () => {
      ...
      execSpy.mockImplementation(async (_cmd: string, args: string[]) => {
        ...
        if (callCount === 1 && args.includes('-b')) {
          const error = new Error(
            'fatal: A branch named rith/issue-42 already exists.'
          ) as Error & {
            stderr?: string;
          };
          error.stderr = 'fatal: A branch named rith/issue-42 already exists.';
          throw error;
        }
      ...
      expect(execSpy).toHaveBeenCalledWith(
        'git',
        ['-C', '/workspace/repo', 'branch', '-f', 'rith/issue-42', 'origin/main'],
        expect.any(Object)
      );
    ...
    test('propagates error if branch -f reset fails (protected branch, etc.)', async () => {
      execSpy.mockImplementation(async (_cmd: string, args: string[]) => {
        ...
        if (args.includes('branch') && args.includes('-f')) {
          ...
          error.stderr = "fatal: cannot force update the current branch 'rith/issue-42'";
          throw error;
        }
        return { stdout: '', stderr: '' };
      });
    ...
    test('creates worktree under project-scoped path for locally-registered repo', async () => {
      ...
      expect(env.workingPath).toBe(
        join(TEST_RITH_HOME, 'workspaces', 'Widinglabs', 'sasha-demo', 'worktrees', env.branchName)
      );

      // mkdir should be called with the project-scoped base (no owner/repo appended)
      expect(mkdirSpy).toHaveBeenCalledWith(
        join(TEST_RITH_HOME, 'workspaces', 'Widinglabs', 'sasha-demo', 'worktrees'),
        { recursive: true }
      );
    });
  ...
  describe('file copying', () => {
    ...
    const baseRequest: IsolationRequest = {
      codebaseId: 'cb-123',
      canonicalRepoPath: '/.rith/workspaces/owner/repo',
      workflowType: 'issue',
      identifier: '42',
    };
    ...
    test('copies configured files after worktree creation', async () => {
      ...
      // Should include default .rith plus user config
      expect(copyWorktreeFilesSpy).toHaveBeenCalledWith(
        '/.rith/workspaces/owner/repo',
        expect.stringContaining('issue-42'),
        expect.arrayContaining(['.rith', '.env.example -> .env', '.vscode/settings.json'])
      );
    });

    test('calls copyWorktreeFiles with default .rith when no copyFiles configured', async () => {
      copyWorktreeFilesSpy.mockResolvedValue([]);

      await provider.create(baseRequest);

      // Should still be called with default .rith
      expect(copyWorktreeFilesSpy).toHaveBeenCalledWith(
        '/.rith/workspaces/owner/repo',
        expect.stringContaining('issue-42'),
        ['.rith']
    ...
    test('calls copyWorktreeFiles with default .rith when copyFiles is empty', async () => {
      const configLoader: RepoConfigLoader = async () => ({
        baseBranch: 'main',
        copyFiles: [],
      });
    ...
    test('does not copy files when adopting existing worktree', async () => {
      ...
      mockReadFile.mockResolvedValue(
        'gitdir: /.rith/workspaces/owner/repo/.git/worktrees/rith/issue-42\n'
      );
      const configLoader: RepoConfigLoader = async () => ({
    ...
    test('should copy .rith directory by default (without config)', async () => {
      ...
      // Verify .rith was copied even without config
      expect(copyWorktreeFilesSpy).toHaveBeenCalledWith(
        '/.rith/workspaces/owner/repo',
        expect.stringContaining('issue-42'),
        ['.rith'] // Default only
    ...
    test('should merge .rith default with user copyFiles config', async () => {
      ...
      // Verify .rith + user files were copied
      expect(copyWorktreeFilesSpy).toHaveBeenCalledWith(
        '/.rith/workspaces/owner/repo',
        expect.stringContaining('issue-42'),
        expect.arrayContaining(['.rith', '.env', '.vscode'])
      );
    });

    test('should deduplicate .rith if user explicitly includes it', async () => {
      // Mock: User config explicitly includes .rith
      const configLoader: RepoConfigLoader = async () => ({
        baseBranch: 'main',
        copyFiles: ['.rith', '.env'],
      });
      ...
      // Verify .rith appears only once (deduplicated by Set)
      const copyFilesArg = copyWorktreeFilesSpy.mock.calls[0][2];
      const rithCount = copyFilesArg.filter((f: string) => f === '.rith').length;
      expect(rithCount).toBe(1);
    });
  ...
  describe('orphan directory handling', () => {
    ...
    test('does not remove directory if it is a valid worktree', async () => {
      ...
      mockReadFile.mockResolvedValue('gitdir: /workspace/repo/.git/worktrees/rith/issue-999\n');
  ...
  describe('workspace sync before worktree creation', () => {
    ...
    test('does not sync workspace when adopting existing worktree', async () => {
      ...
      worktreeExistsSpy.mockResolvedValue(true);
      mockReadFile.mockResolvedValue(
        'gitdir: /workspace/owner/repo/.git/worktrees/rith/issue-42\n'
      );
    ...
    test('auto-detects base branch when no baseBranch configured and no fromBranch', async () => {
      ...
      // resetAfterFetch: false because test path is not a managed clone under ~/.rith/workspaces
      ...
      syncWorkspaceSpy.mockRejectedValue(
        new Error(
          "Configured base branch 'does-not-exist' not found on remote. " +
            'Either create the branch, update worktree.baseBranch in .rith/config.yaml, ' +
  ...
  describe('cross-platform path handling', () => {
    test('getWorktreePath handles Unix-style paths', () => {
      const request: IsolationRequest = {
        codebaseId: 'cb-123',
        canonicalRepoPath: '/home/dev/.rith/workspaces/owner/repo',
        workflowType: 'issue',
        identifier: '42',
      };
    ...
    test('getWorktreePath handles Windows-style paths', () => {
      const request: IsolationRequest = {
        codebaseId: 'cb-123',
        canonicalRepoPath: 'C:\\Users\\dev\\.rith\\workspaces\\owner\\repo',
        workflowType: 'issue',
        identifier: '42',
      };
      const branchName = provider.generateBranchName(request);
    ...
    test('getWorktreePath uses codebaseName for locally-registered repo', () => {
      ...
      const path = provider.getWorktreePath(request, branchName);
      expect(path).toBe(
        join(TEST_RITH_HOME, 'workspaces', 'Widinglabs', 'sasha-demo', 'worktrees', branchName)
      );
    });
  });
  ...
  describe('worktree.path repo-local override', () => {
    ...
    test('null / undefined config falls back to workspace-scoped default', () => {
      const branch = provider.generateBranchName(baseRequest);
      const expected = join(TEST_RITH_HOME, 'workspaces', 'owner', 'myapp', 'worktrees', branch);
    ...
    test('override wins even when repo lives under ~/.rith/workspaces/', () => {
      ...
      const request: IsolationRequest = {
        ...
        canonicalRepoPath: join(TEST_RITH_HOME, 'workspaces', 'owner', 'repo'),
        workflowType: 'task',
        identifier: 'my-task',
      };
      ...
      expect(result).toBe(
        join(TEST_RITH_HOME, 'workspaces', 'owner', 'repo', 'worktrees-local', branch)
      );
    });
    ...
    test('accepts a nested relative path without `..`', () => {
      ...
      expect(result).toBe(join('/Users/dev/Projects/myapp', '.rith/worktrees', branch));
    });
  });

...
import type { RepoPath, BranchName, WorktreePath } from '@rith/git';
...
export class IsolationResolver {
  ...
  async resolve(request: ResolveRequest): Promise<IsolationResolution> {
    ...
    return this.createNewEnvironment(codebase, workflowType, workflowId, hints, canonicalPath);
  }
      ...
      if (!isValid) {
        ...
        return [
          `Worktree branch '${env.branch_name}' is not based on '${baseBranch}'. ` +
            `Recreate with: rith complete ${env.branch_name} --force`,
        ];
      }
    } catch (err) {
      getLog().warn(

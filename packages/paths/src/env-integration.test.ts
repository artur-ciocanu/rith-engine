 ...
 *   Bun auto-load (simulated) → stripCwdEnv() → ~/.rith/.env load → subprocess env
 *
 * Tests the full user scenario: what keys reach the Claude subprocess when the
 * user has various combinations of CWD .env, ~/.rith/.env, and shell env?
 *
...
describe('env isolation integration', () => {
  ...
  const rithDir = join(import.meta.dir, '__env-integration-rith__');
  ...
  afterEach(() => {
    ...
    rmSync(rithDir, { recursive: true, force: true });
  });
   ...
   * 3. Load ~/.rith/.env (dotenv.config)
   * 4. Return process.env snapshot (what buildSubprocessEnv would return)
   */
  function simulateEntryPointFlow(cwdEnv: string, rithEnv: string): NodeJS.ProcessEnv {
    ...
    // Step 3: Load ~/.rith/.env with override — user's Rith Engine config wins
    // over any shell-inherited vars (same as real entry point).
    writeFileSync(join(rithDir, '.env'), rithEnv);
    config({ path: join(rithDir, '.env'), override: true });
  ...
  it('scenario 2: user has OAuth token in rith env + random key in CWD .env — CWD stripped, rith kept', () => {
    const subprocessEnv = simulateEntryPointFlow(
  ...
  it('scenario 3: nothing from CWD .env leaks to subprocess', () => {
    ...
    expect(subprocessEnv.ANTHROPIC_API_KEY).toBeUndefined();
    // Rith key present
    expect(subprocessEnv.RITH_ONLY_KEY).toBe('trusted');
  ...
  it('scenario 4: same key in both CWD and rith env — rith value wins', () => {
    // User has ANTHROPIC_API_KEY in both places. CWD one is the target repo's,
    // rith one is the user's intentional config. Rith Engine must win.
    const subprocessEnv = simulateEntryPointFlow(
      'ANTHROPIC_API_KEY=sk-target-repo-WRONG\nSHARED_KEY=cwd-value\n',
      'ANTHROPIC_API_KEY=sk-my-real-key\nSHARED_KEY=rith-value\n'
    );

    // Rith value wins (CWD was stripped, then rith loaded)
    expect(subprocessEnv.ANTHROPIC_API_KEY).toBe('sk-my-real-key');
    expect(subprocessEnv.SHARED_KEY).toBe('rith-value');
  });
  ...
  it('scenario 5: DATABASE_URL in CWD .env does not reach Rith Engine — rith uses its own DB', () => {
    // Target repo has DATABASE_URL for its own PostgreSQL. Rith Engine must NOT
    // connect to the target app's database — it should use its own DB
    // (from ~/.rith/.env or default SQLite).
    const subprocessEnv = simulateEntryPointFlow(
      'DATABASE_URL=postgresql://target-app:5432/wrong_db\nREDIS_URL=redis://target:6379\n',
      'DATABASE_URL=sqlite:///Users/me/.rith/rith.db\n'
    );

    // CWD DATABASE_URL is stripped, rith's wins
    expect(subprocessEnv.DATABASE_URL).toBe('sqlite:///Users/me/.rith/rith.db');
    // Other CWD keys also stripped
    expect(subprocessEnv.REDIS_URL).toBeUndefined();
  });

  it('scenario 6: DATABASE_URL in CWD .env only (no rith env) — stripped entirely', () => {
    // User relies on default SQLite (no DATABASE_URL in ~/.rith/.env).
  ...
  it('CLAUDE_CODE_OAUTH_TOKEN from rith env survives marker strip', () => {

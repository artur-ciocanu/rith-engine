...
describe('stripCwdEnv — operator logging (#1302)', () => {
  ...
  it('emits [rith] stripped line with count and filename when keys are stripped', () => {
    ...
    stripCwdEnv(tmpDir);
    const line = stderrWrites.find(s => s.startsWith('[rith] stripped'));
  ...
  it('emits no [rith] stripped line when no CWD .env files exist', () => {
    stripCwdEnv(tmpDir);
    const line = stderrWrites.find(s => s.startsWith('[rith] stripped'));
    expect(line).toBeUndefined();
  });

  it('emits no [rith] stripped line when .env file is empty', () => {

 ...
 * (notably `@rith/paths/logger` which reads `LOG_LEVEL` during module load).
 *
 ...
 *    When `rith` is invoked from inside a target repo, that repo's env vars
 *    leak into the Rith Engine process. `override: true` in dotenv only fixes keys
 ...
 * Keys in ~/.rith/.env (loaded afterward by each entry point) are unaffected.
...
export function stripCwdEnv(cwd: string = process.cwd()): void {
    ...
    if (result.error) {
      ...
      if (code !== 'ENOENT') {
        process.stderr.write(
          `[rith] Warning: could not parse ${filepath} for CWD env stripping: ${result.error.message}\n`
        );
      }
  ...
  if (cwdKeys.size > 0) {
    process.stderr.write(
      `[rith] stripped ${cwdKeys.size} keys from ${cwd} (${strippedFiles.join(', ')}) to prevent target repo env from leaking into Rith Engine processes\n`
    );
  }
  ...
  if (process.env.CLAUDECODE === '1' && !process.env.RITH_SUPPRESS_NESTED_CLAUDE_WARNING) {
    process.stderr.write(

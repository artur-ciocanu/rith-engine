...
describe('telemetry opt-out detection', () => {
  ...
  test('RITH_TELEMETRY_DISABLED=1 disables telemetry', () => {
  ...
  test('RITH_TELEMETRY_DISABLED=0 does not disable (strict "1" match)', () => {
    process.env.RITH_TELEMETRY_DISABLED = '0';
    delete process.env.DO_NOT_TRACK;
    expect(isTelemetryDisabled()).toBe(false);
  });
...
describe('telemetry ID persistence', () => {
  ...
  beforeEach(() => {
    saved = saveEnv();
    tmpHome = mkdtempSync(join(tmpdir(), 'rith-telemetry-test-'));

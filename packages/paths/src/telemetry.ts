/**
 * Anonymous PostHog telemetry for Rith Engine.
 *
 * Emits one event — `workflow_invoked` — each time a workflow starts. No PII,
 * no user identity. A random UUID is persisted to `${RITH_HOME}/telemetry-id`
 ...
 * and swallowed. Capture must never crash Rith Engine.
 */
...
export function isTelemetryDisabled(): boolean {
  if (process.env.RITH_TELEMETRY_DISABLED === '1') return true;
...
/**
 * Load or create a stable anonymous install UUID at `${RITH_HOME}/telemetry-id`.
...
export function getOrCreateTelemetryId(): string {
  const idPath = join(getRithHome(), 'telemetry-id');
  try {
  ...
  const id = randomUUID();
  try {
    mkdirSync(getRithHome(), { recursive: true });
    ...
    try {
      ...
      client.capture({
        ...
        properties: {
          ...
          ...(props.rithVersion ? { rith_version: props.rithVersion } : {}),
        },
      });
    } catch (error) {

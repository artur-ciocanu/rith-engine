...
describe('PiCodingAgent', () => {
  ...
  test('sendQuery installs PI_PACKAGE_DIR shim before Pi SDK loads', async () => {
    ...
    // rith binaries. The shim writes a stub package.json to tmpdir and sets
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(
      {
        ...
        envHint: expect.stringContaining("not in the Rith Engine adapter's env-var table"),
  ...
  test('result chunk carries Pi sessionId (for Rith Engine to store and reuse)', async () => {
    process.env.GEMINI_API_KEY = 'sk-test';
    resetScript(scriptedAgentEnd());

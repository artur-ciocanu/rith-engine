...
// event-bridge). Pi's `@mariozechner/pi-coding-agent/dist/config.js`
// runs `readFileSync(getPackageJsonPath(), "utf-8")` at module load; inside a
// compiled Rith Engine binary `getPackageJsonPath()` resolves to
// `dirname(process.execPath) + "/package.json"` — a path that doesn't exist —
// and rith crashes at startup before any command runs (v0.3.7 symptom).
//
 ...
 * (which fails inside a compiled rith binary). Pi only reads three
...
function ensurePiPackageDirShim(): void {
  const shimDir = join(tmpdir(), 'rith-pi-shim');
 ...
 * scope — Rith Engine is a server-side platform and doesn't drive interactive
...
export class PiCodingAgent implements PiAgent {
  async *sendQuery(
    ...
    // mode (rith compiled binary crashes at startup when Pi's config.js
    ...
    const [
      ...
      { resolvePiSession },
      { createRithUIBridge, createRithUIContext },
    ] = await Promise.all([
    ...
    if (!modelRef) {
      throw new Error(
        'Pi provider requires a model. Set `model` on the workflow node, `model` on the workflow, or `pi.model` in .rith/config.yaml. ' +
        ...
        getLog().info(
          {
            ...
            envHint: `Provider '${parsed.provider}' is not in the Rith Engine adapter's env-var table — file an issue if you want a shortcut env var for it.`,
    ...
    // `pi.enableExtensions: false` in `.rith/config.yaml` — that's the trust
    // gate that suppresses Pi's native discovery of cwd `.pi/` extension code.
    const enableExtensions = piConfig.enableExtensions !== false;
    // Forwards extension `notify()` output into the chunk stream (see 4f).
    const uiBridge = enableExtensions ? createRithUIBridge() : undefined;
      ...
      if (!model) {
        ...
        throw new Error(
          `Pi model not found: provider='${parsed.provider}' model='${parsed.modelId}'. ` +
            ...
            'and `enableExtensions: true` is set in .rith/config.yaml.'
        );
      }
      try {

 ...
 * Rith Engine's common surface includes 'off' (from Codex's modelReasoningEffort)
 * and 'max' (from Claude's EffortLevel enum). Map into Pi's vocabulary:
 *  - 'off'    → undefined (no explicit thinkingLevel; Pi's implicit off)
 *  - 'max'    → 'xhigh'  (Rith Engine's EffortLevel doesn't have xhigh)
 *  - others pass through if they're already Pi-native
 *
 * See packages/workflows/src/schemas/dag-node.ts#effortLevelSchema for
 * the Rith Engine schema enum (`low | medium | high | max`). Workflow YAML can
 * only carry Rith Engine-enum values; Pi-native `minimal` / `xhigh` are accepted
...
/**
 * Resolve Rith Engine's `effort` / `thinking` node fields to Pi's `ThinkingLevel`.
 *
...
/**
 * Filter Pi's built-in tool set against Rith Engine's `allowed_tools`` /
 ...
 *   - allowed_tools: [] → return [] (explicit no-tools; valid Rith Engine idiom)
...
export { resolveSkillDirectories as resolvePiSkills } from './skills';
export type { ResolvedSkills } from './skills';

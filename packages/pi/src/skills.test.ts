...
function makeFakeWorld(): FakeWorld {
  const root = mkdtempSync(join(tmpdir(), 'rith-skills-test-'));
...
describe('resolveSkillDirectories', () => {
  ...
  test('resolves a skill staged under cwd/.rith/skills (highest priority)', () => {
    const dir = fake.stageSkill('cwd', '.rith', 'rith-impl');
    const result = resolveSkillDirectories(fake.cwd, ['rith-impl']);
    expect(result.paths).toEqual([dir]);
    expect(result.missing).toEqual([]);
  });

  test('prefers cwd/.rith/skills over cwd/.claude/skills', () => {
    const rithDir = fake.stageSkill('cwd', '.rith', 'overlap');
    fake.stageSkill('cwd', '.claude', 'overlap');
    const result = resolveSkillDirectories(fake.cwd, ['overlap']);
    expect(result.paths).toEqual([rithDir]);
    expect(result.missing).toEqual([]);
  });

  test('falls back to home/.rith/skills when cwd/.rith misses', () => {
    const dir = fake.stageSkill('home', '.rith', 'home-rith');
    const result = resolveSkillDirectories(fake.cwd, ['home-rith']);
    expect(result.paths).toEqual([dir]);
    expect(result.missing).toEqual([]);
  });

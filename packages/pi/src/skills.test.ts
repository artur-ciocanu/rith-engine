...
describe('resolveSkillDirectories', () => {
  ...
  test('resolves a skill staged under cwd/.rith/skills (highest priority)', () => {
    const dir = fake.stageSkill('cwd', '.rith', 'alpha');
    fake.stageSkill('cwd', '.agents', 'alpha');
    const result = resolveSkillDirectories(fake.cwd, ['alpha']);
    expect(result.paths).toEqual([dir]);
    expect(result.missing).toEqual([]);
  });

  test('resolves a skill staged under home/.rith/skills', () => {
    const dir = fake.stageSkill('home', '.rith', 'beta-rith');
    const result = resolveSkillDirectories(fake.cwd, ['beta-rith']);
    expect(result.paths).toEqual([dir]);
    expect(result.missing).toEqual([]);
  });

  test('prefers cwd/.rith over home/.rith', () => {
    const cwdDir = fake.stageSkill('cwd', '.rith', 'gamma');
    fake.stageSkill('home', '.rith', 'gamma');

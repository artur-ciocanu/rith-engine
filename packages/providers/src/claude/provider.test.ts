...
describe('ClaudeProvider', () => {
  ...
  describe('sendQuery', () => {
    ...
    test("honors explicit settingSources: ['project'] to opt out of user scope", async () => {
      ...
      // .rith/config.yaml must NOT be silently widened to the new default.
...
describe('sendQuery decomposition behaviors', () => {
  ...
  test('preserves first-event timeout error instead of generic abort', async () => {
    ...
    const consumeGenerator = async (): Promise<void> => {
      ...
      const original = process.env.RITH_CLAUDE_FIRST_EVENT_TIMEOUT_MS;
      process.env.RITH_CLAUDE_FIRST_EVENT_TIMEOUT_MS = '50';
      try {
        for await (const _ of client.sendQuery('test', '/workspace')) {
          // consume
        }
      } finally {
        if (original !== undefined) process.env.RITH_CLAUDE_FIRST_EVENT_TIMEOUT_MS = original;
        else delete process.env.RITH_CLAUDE_FIRST_EVENT_TIMEOUT_MS;
      }
    };

 ...
 * - ~/.rith/.env loaded with override:true as the trusted source
 */
...
function getFirstEventTimeoutMs(): number {
  const raw = process.env.RITH_CLAUDE_FIRST_EVENT_TIMEOUT_MS;
  if (raw) {
...
/** YAML hook matcher shape (matches @rith/workflows/schemas/dag-node WorkflowNodeHooks) */
 ...
 * operator pointed Rith Engine at a legacy Bun/Node-runnable cli script.
 * Otherwise return `false`.
 *
 * Safety: target-repo `.env` leaks are prevented by `stripCwdEnv()` in
 * `@rith/paths` (#1067), which deletes CWD `.env` keys from
 * `process.env` at every Rith Engine entry point before any subprocess is
...
/**
 * Normalize raw Claude SDK events into Rith Engine MessageChunks.

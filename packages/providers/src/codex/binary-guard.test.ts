 ...
 *
 * Separate file because mock.module('@rith/paths') with BUNDLED_IS_BINARY=true
...
// Mock @rith/paths with BUNDLED_IS_BINARY = true (simulates compiled binary)
mock.module('@rith/paths', () => ({
  createLogger: mock(() => mockLogger),
  BUNDLED_IS_BINARY: true,
  getRithHome: mock(() => '/tmp/test-rith'),
}));
...
const mockResolveCodexBinaryPath = mock(
  (_configPath?: string): Promise<string | undefined> =>
    Promise.resolve('/tmp/test-rith/vendor/codex/codex')
);
...
describe('CodexProvider binary mode resolution', () => {
  ...
  test('propagates resolver errors as clear failures', async () => {
    mockResolveCodexBinaryPath.mockRejectedValueOnce(
      new Error('Codex native binary not found at /tmp/test-rith/vendor/codex/codex')
    );
  ...
  test('retries initialization after first failure (rejected promise not cached)', async () => {
    mockResolveCodexBinaryPath
      ...
      .mockResolvedValueOnce('/tmp/test-rith/vendor/codex/codex');

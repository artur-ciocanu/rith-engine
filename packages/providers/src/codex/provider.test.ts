...
describe('CodexProvider', () => {
  ...
  describe('sendQuery', () => {
    ...
    test('builds env by preserving process vars and letting request env win on collisions', async () => {
      ...
      const originalRithEnv = process.env.RITH_CODEX_TEST_ENV;
      process.env.PATH = 'from-process';
      process.env.RITH_CODEX_TEST_ENV = 'kept-from-process';
      ...
      try {
        ...
        expect(MockCodex).toHaveBeenCalledWith(
          expect.objectContaining({
            env: expect.objectContaining({
              PATH: 'from-request',
              RITH_CODEX_TEST_ENV: 'kept-from-process',
              MY_SECRET: 'abc123',
            }),
          })
        );
        ...
        if (originalRithEnv === undefined) {
          delete process.env.RITH_CODEX_TEST_ENV;
        } else {
          process.env.RITH_CODEX_TEST_ENV = originalRithEnv;
        }
      }
    });
    ...
    test('passes workflow MCP config as Codex mcp_servers overrides', async () => {
      ...
      const originalToken = process.env.RITH_CODEX_MCP_TOKEN;
      process.env.RITH_CODEX_MCP_TOKEN = 'token-from-process';
      ...
      try {
        await writeFile(
          ...
          JSON.stringify({
            figma: {
              ...
              headers: { Authorization: 'Bearer $RITH_CODEX_MCP_TOKEN' },
            ...
            local: {
              ...
              command: 'npx',
              args: ['-y', 'figma-mcp'],
              env: { TOKEN: '$RITH_CODEX_MCP_TOKEN' },
            },
          })
        );
        ...
        if (originalToken === undefined) {
          delete process.env.RITH_CODEX_MCP_TOKEN;
        } else {
          process.env.RITH_CODEX_MCP_TOKEN = originalToken;
        }
    ...
    test('prefixes workflow MCP warnings for workflow forwarding', async () => {
      ...
      delete process.env.RITH_CODEX_MISSING_TOKEN;
      ...
      try {
        await writeFile(
          ...
          JSON.stringify({
            figma: {
              command: 'figma-mcp',
              env: { TOKEN: '$RITH_CODEX_MISSING_TOKEN' },
            },
          })
        );
        mockRunStreamed.mockResolvedValue({
        ...
        expect(chunks[0]).toEqual({
          type: 'system',
          content:
            '⚠️ MCP config references undefined env vars: RITH_CODEX_MISSING_TOKEN. These will be empty strings - MCP servers may fail to authenticate.',
        });
      } finally {
        await rm(testDir, { recursive: true, force: true });
      }
    ...
    test('uses generic dashboard guidance when fallback mapping is unknown', async () => {
      ...
      await expect(consumeGenerator()).rejects.toThrow('update your model in ~/.rith/config.yaml');
    });

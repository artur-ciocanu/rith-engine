...
function convertMcpServerConfigForCodex(
  ...
  // Rith Engine's MCP JSON format uses `headers`; Codex config uses `http_headers`.
...
function buildModelAccessMessage(model?: string): string {
  ...
  const fixLine = suggested
    ? `To fix: update your model in ~/.rith/config.yaml:\n  assistants:\n    codex:\n      model: ${suggested}`
    : 'To fix: update your model in ~/.rith/config.yaml to one your account can access.';
...
/**
 * Normalize raw Codex SDK events into Rith Engine MessageChunks.

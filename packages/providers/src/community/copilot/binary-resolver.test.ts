 ...
 *
 * Must run in its own bun test invocation because it mocks @rith/paths
...
mock.module('@rith/paths', () => ({
  ...
  getRithHome: mock(() => '/tmp/test-rith-home'),
}));
...
describe('resolveCopilotBinaryPath (binary mode)', () => {
  ...
  test('checks vendor directory when no env or config path', async () => {
    ...
    expect(normalized).toContain('/tmp/test-rith-home/vendor/copilot/');
  });
...
describe('isExecutableFile', () => {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'rith-copilot-resolver-'));

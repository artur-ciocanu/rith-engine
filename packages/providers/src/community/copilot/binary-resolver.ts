/**
 * Copilot CLI binary resolver for compiled (bun --compile) rith binaries.
 *
 ...
 * via `import.meta.url`. In compiled rith binaries that path is frozen to
 ...
 *  3. `~/.rith/vendor/copilot/<platform-binary>` (user-placed)
...
import { BUNDLED_IS_BINARY, getRithHome, createLogger } from '@rith/paths';
...
export async function resolveCopilotBinaryPath(
    ...
    if (!isExecutableFile(configCliPath)) {
      throw new Error(
        `assistants.copilot.copilotCliPath is set to "${configCliPath}" but it is not an executable file.\n` +
          'Please verify the path in .rith/config.yaml points to the Copilot CLI executable (chmod +x if needed).'
      );
    }
  ...
  if (binaryName) {
    ...
    const vendorBinaryPath = join(rithHome, COPILOT_VENDOR_DIR, binaryName);
  ...
  const vendorPath = `~/.rith/${COPILOT_VENDOR_DIR}/`;
  throw new Error(

 ...
 * binary path + reasoning config, translates Rith Engine workflow options
...
/**
 * Resolve Copilot's `reasoningEffort` from Rith Engine's workflow inputs.
 * Precedence:
 *   nodeConfig.thinking > nodeConfig.effort > config.modelReasoningEffort
 *
 * Rith Engine's `effort` schema is `'low' | 'medium' | 'high' | 'max'` — we map
...
/**
 * Translate Rith Engine's per-node `allowed_tools` / `denied_tools` to Copilot's
...
/**
 * Translate Rith Engine's `nodeConfig.mcp` (JSON-file path) to Copilot's
...
/**
 * Translate Rith Engine's `nodeConfig.skills` (string names) to Copilot's
...
/**
 * Translate Rith Engine's `nodeConfig.agents` (Record<name, AgentDef>) to
 ...
 * Rith Engine agent fields Copilot cannot represent (`model`, `disallowedTools`,
 * `skills`, `maxTurns`) surface as one consolidated warning per agent.
 *
 * We do NOT set `SessionConfig.agent` — Rith Engine's workflow model invokes
  ...
  if (isModelAccessError(combined)) {
    return new Error(
      `Copilot model access error: ${combined}\n\n` +
        'Try a different model in the workflow node or set assistants.copilot.model in .rith/config.yaml.'
    );
  }
  ...
  if (
    ...
    return new Error(
      `Copilot authentication failed: ${combined}\n\n` +
        ...
        '`useLoggedInUser: false` in `.rith/config.yaml` to use GH_TOKEN / GITHUB_TOKEN.'
    );
  }
 ...
 * `@github/copilot-sdk`, translating Rith Engine workflow options (tools, MCP,
...
export class CopilotProvider implements IAgentProvider {
  ...
  async *sendQuery(
    ...
    // Rith Engine's dag-executor sets `forkSession: true` on every reuse so retries

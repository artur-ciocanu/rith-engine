...
export async function materializeAgents(
  ...
  // Remove stale rith-owned agent files that aren't in the current request
  const currentRithFiles = new Set(Object.keys(agents).map(key => `rith-${toKebabCase(key)}.md`));
  try {
    const existing = await readdir(agentsDir);
    await Promise.all(
      existing
        .filter(f => f.startsWith('rith-') && !currentRithFiles.has(f))
  ...
  await Promise.all(
    Object.entries(agents).map(([key, config]) => {
      const filename = `rith-${toKebabCase(key)}.md`;

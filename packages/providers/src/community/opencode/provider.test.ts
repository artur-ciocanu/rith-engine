...
async function createTempProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'rith-opencode-provider-'));
  tempDirs.add(dir);
  return dir;
}
...
describe('OpencodeProvider', () => {
  ...
  test('agent config injects rith-prefixed kebab-case name into promptAsync body', async () => {
  ...
  test('materializes workflow agents under project .opencode/agents with mapped content', async () => {
    ...
    expect(error).toBeUndefined();
    const agentPath = join(cwd, '.opencode', 'agents', 'rith-reviewer.md');
  ...
  test('materialization preserves user-authored files and only replaces rith-owned files for current request scope', async () => {
    const cwd = await createTempProjectDir();
    const agentsDir = join(cwd, '.opencode', 'agents');
    await mkdir(agentsDir, { recursive: true });
    await writeFile(join(agentsDir, 'custom-agent.md'), '# user agent\n', 'utf8');
    await writeFile(join(agentsDir, 'rith-stale-agent.md'), 'old stale content\n', 'utf8');
    await writeFile(join(agentsDir, 'rith-keep-agent.md'), 'old keep content\n', 'utf8');
    ...
    expect(await readFile(join(agentsDir, 'rith-keep-agent.md'), 'utf8')).toContain('Fresh prompt');
    await expect(readFile(join(agentsDir, 'rith-stale-agent.md'), 'utf8')).rejects.toMatchObject({
      code: 'ENOENT',
    });
  });
    ...
    const runtime = makeRuntime({
      promptAsync: mock(async () => {
        const content = await readFile(
          join(cwd, '.opencode', 'agents', 'rith-order-check.md'),
          'utf8'
        );
  ...
  test('disposes cached OpenCode instance after agent materialization and before prompt execution', async () => {
    ...
    expect(runtime.client.instance.dispose).toHaveBeenCalledWith({
      query: { directory: join(cwd, '.rith-opencode', 'node-1') },
    });
  ...
  test('retries once when first attempt fails with agent-not-found for inline agents', async () => {
    ...
    const failingRuntime = makeRuntime({
      promptAsync: mock(async () => {
        throw new Error("Agent not found: 'rith-reviewer'");
      }),
    });
    const successRuntime = makeRuntime();
    ...
    expect(mockCreateOpencode).toHaveBeenCalledTimes(2);
    expect(mockLogger.info).toHaveBeenCalledWith(
      { attempt: 0, sessionCwd: join(cwd, '.rith-opencode', 'node-2') },
  ...
  test('external baseUrl mode is rejected even when pre-generated agent files exist', async () => {
    ...
    await mkdir(agentsDir, { recursive: true });
    await writeFile(
      join(agentsDir, 'rith-reviewer.md'),
      ['---', 'name: rith-reviewer', 'description: "Review agent"', '---', '', 'Review'].join('\n'),
      'utf8'
    );
  ...
  test('external baseUrl mode rejects multi-agent execution with same deprecation error', async () => {
    ...
    const agentsDir = join(cwd, '.opencode', 'agents');
    await mkdir(agentsDir, { recursive: true });
    await writeFile(join(agentsDir, 'rith-agent-a.md'), '---\nmode: subagent\n---\nA\n', 'utf8');
    await writeFile(join(agentsDir, 'rith-agent-b.md'), '---\nmode: subagent\n---\nB\n', 'utf8');

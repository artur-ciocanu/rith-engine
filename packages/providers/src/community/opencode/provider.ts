    ...
    if (usingExternalBaseUrl) {
      throw new Error(
        'OpenCode external baseUrl mode is no longer supported. ' +
          'Rith Engine now requires managed embedded OpenCode runtime for fully controlled agent lifecycle.'
      );
    }

    const sessionCwd =
      hasAgentConfig && nodeId && !usingExternalBaseUrl ? join(cwd, '.rith-opencode', nodeId) : cwd;

...
describe('createRithUIBridge', () => {
  test('drops notifications when no emitter is set', () => {
    const bridge = createRithUIBridge();
...
describe('createRithUIContext', () => {
  function mk() {
    const bridge = createRithUIBridge();
    const chunks: MessageChunk[] = [];
    bridge.setEmitter(c => chunks.push(c));
    const ui = createRithUIContext(bridge);
    return { ui, chunks };
  }
  ...
  test('theme getter returns a proxy that throws on property access', () => {
    ...
    expect(() => themeRef.fg('accent', 'text')).toThrow(/Rith Engine's remote UI stub/);
  });

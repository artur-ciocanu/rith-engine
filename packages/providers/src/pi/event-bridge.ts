...
/**
 * Extract Rith Engine TokenUsage from Pi's Usage struct.
...
/**
 * Pure mapper from Pi's `AgentSessionEvent` → zero-or-more Rith Engine `MessageChunk`s.
 *
 ...
 *  - compaction_start / compaction_end (auto-compaction opaque to Rith Engine)
...
/** Lets the UI stub push extension `notify()` output into the session's chunk queue. */
export interface BridgeNotifier {
  setEmitter(fn: ((chunk: MessageChunk) => void) | undefined): void;
}

/**
 * Bridge a Pi `AgentSession` into Rith Engine's `AsyncGenerator<MessageChunk>` contract.
 *
 * Behavior:
...
export async function* bridgeSession(
  ...
  try {
    for await (const item of queue) {
      ...
      // Annotate the terminal result chunk with Pi's session UUID so Rith Engine's

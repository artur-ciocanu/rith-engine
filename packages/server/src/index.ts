...
// into the server process. stripCwdEnv() removes them before ~/.rith/.env loads.
...
import { BUNDLED_IS_BINARY, getRithEnvPath } from '@rith/paths';
...
// Load rith-owned env from ~/.rith/.env (user scope) and <cwd>/.rith/.env
...
import { GiteaAdapter } from '@rith/adapters/community/forge/gitea';
import { GitLabAdapter } from '@rith/adapters/community/forge/gitlab';
...
export async function startServer(opts: ServerOptions = {}): Promise<void> {
    ...
    getLog().fatal(
      {
        ...
        envFile: BUNDLED_IS_BINARY ? getRithEnvPath() : envPath,
      },
      'no_ai_credentials'
    );
    process.exit(1);
  ...
  // buttons in the Web UI dashboard, or `rith workflow abandon <run-id>`.
  // (`rith workflow cleanup` is a separate command that deletes OLD terminal
  // rows for disk hygiene — it does not handle stuck `running` rows.)
  // See #1216.

  // Log Rith Engine paths configuration
  logRithPaths();
  ...
  if (!opts.skipPlatformAdapters) {
    ...
    if (process.env.DISCORD_BOT_TOKEN) {
      ...
      // `rith serve` for the web UI shouldn't lose it because of an
...
async function checkGhAuth(): Promise<void> {
  const { execFileAsync } = await import('@rith/git');
  try {

...
describe('DELETE /api/codebases/:id', () => {
  ...
  test('does not delete disk directory for external (non-Rith Engine-managed) repos', async () => {
    // The codebase's default_cwd is outside the Rith Engine workspaces root
    mockGetCodebase.mockImplementationOnce(async () => ({
      ...MOCK_CODEBASE,

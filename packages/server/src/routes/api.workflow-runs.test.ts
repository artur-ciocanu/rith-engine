...
mock.module('@rith/paths', () => ({
  ...
  getRithWorkspacesPath: () => '/tmp/.rith/workspaces',
  getRithHome: () => '/tmp/.rith',
  getRunArtifactsPath: (owner: string, repo: string, runId: string): string =>
    `/tmp/.rith/workspaces/${owner}/${repo}/artifacts/runs/${runId}`,
}));
...
mock.module('@rith/core/utils/commands', () => ({
...
describe('POST /api/workflows/:name/run', () => {
  ...
  test('returns 400 when isValidCommandName rejects the name', async () => {
    const { isValidCommandName } = await import('@rith/workflows/command-validation');
...
describe('POST /api/workflows/runs/:runId/resume', () => {
  ...
  test('returns 400 with CLI hint when run has no parent_conversation_id', async () => {
    ...
    expect(response.status).toBe(400);
    const body = (await response.json()) as { error: string };
    expect(body.error).toContain('rith workflow resume run-uuid-4');
...
describe('approve/reject auto-resume', () => {
  ...
  test('approve: skips dispatch when parent_conversation_id is null (CLI-dispatched run)', async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as { message: string };
    expect(body.message).toContain('rith workflow resume run-paused-1');
    expect(mockHandleMessage).not.toHaveBeenCalled();
  ...
  test('reject: surfaces CLI resume hint when on_reject configured but parent is non-web', async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as { message: string };
    expect(body.message).toContain('rith workflow resume run-reject-non-web');
    expect(mockHandleMessage).not.toHaveBeenCalled();
  });
...
describe('GET /api/runs/:runId/artifacts', () => {
  ...
  // would, after the join, resolve to a directory outside RITH_HOME. The
  // mocked getRunArtifactsPath above naively joins inputs, so passing
  // `'..'` as the owner produces a path that normalises outside /tmp/.rith.
  test('returns 400 when the resolved artifact dir escapes RITH_HOME', async () => {
    mockGetWorkflowRun.mockImplementationOnce(async () => ({
      ...MOCK_RUNNING_RUN,
      id: 'run-escape',

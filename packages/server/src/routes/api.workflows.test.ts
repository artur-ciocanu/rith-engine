...
import { makeTestWorkflow, makeTestWorkflowWithSource } from '@rith/workflows/test-utils';
...
const mockDiscoverWorkflows = mock(async (_cwd: string | null) => ({
  ...
  errors: [{ filename: '/tmp/.rith/workflows/bad.md', error: 'invalid', errorType: 'parse_error' }],
}));
...
mock.module('@rith/core', () => ({
  handleMessage: mock(async () => {}),
  getDatabaseType: () => 'sqlite',
  loadConfig: mock(async () => ({})),
  getWorkflowFolderSearchPaths: mock(() => ['.rith/workflows']),
  getCommandFolderSearchPaths: mock(() => ['.rith/commands', '.rith/commands/defaults']),
  getDefaultCommandsPath: mock(() => '/tmp/.rith-test-nonexistent/commands/defaults'),
  getDefaultWorkflowsPath: mock(() => '/tmp/.rith-test-nonexistent/workflows/defaults'),
  ...
  getRithWorkspacesPath: () => '/tmp/.rith/workspaces',
  createLogger: () => ({
...
mock.module('@rith/workflows/workflow-discovery', () => ({
  discoverWorkflowsWithConfig: mockDiscoverWorkflows,
}));
mock.module('@rith/workflows/loader', () => ({
  parseWorkflow: mockParseWorkflow,
}));
mock.module('@rith/workflows/command-validation', () => ({
...
mock.module('@rith/workflows/defaults', () => ({
  BUNDLED_WORKFLOWS: {
    'rith-assist': 'name: rith-assist\ndescription: Rith Engine Assist\nnodes: []',
  },
  BUNDLED_COMMANDS: {
    'rith-assist': '# rith-assist command',
  },
  isBinaryBuild: mock(() => false),
}));

// Note: @rith/core/defaults/bundled-defaults and @rith/core/utils/commands are NOT mocked.
...
mock.module('@rith/core/db/workflows', () => ({}));
mock.module('@rith/core/db/workflow-events', () => ({}));
mock.module('@rith/core/db/messages', () => ({}));
...
describe('GET /api/workflows/:name', () => {
  ...
  test('returns bundled workflow with source:bundled', async () => {
    ...
    // No cwd → no readFile attempt → checks BUNDLED_WORKFLOWS → rith-assist found
    mockListCodebases.mockImplementationOnce(async () => []);

    const response = await app.request('/api/workflows/rith-assist');
    expect(response.status).toBe(200);
    const body = (await response.json()) as { source: string; filename: string; workflow: unknown };
    expect(body.source).toBe('bundled');
    expect(body.filename).toBe('rith-assist.yaml');
  ...
  test('returns home-scoped workflow with source:global when project/bundled miss', async () => {
    ...
    const prevRithHome = process.env.RITH_HOME;
    process.env.RITH_HOME = tmpHome;
    try {
      const app = createTestApp();
      registerApiRoutes(app, {} as WebAdapter, {} as ConversationLockManager);
      ...
      if (prevRithHome === undefined) {
        delete process.env.RITH_HOME;
      } else {
        process.env.RITH_HOME = prevRithHome;
      }
  ...
  test('project-scope shadows home-scope when same filename exists in both', async () => {
    ...
    const projectDir = join(testDir, '.rith', 'workflows');
...
describe('GET /api/workflows/:name - cwd validation', () => {
  test('returns 400 when cwd is not a registered codebase path', async () => {
    ...
    const response = await app.request('/api/workflows/rith-assist?cwd=/etc/secrets');
    expect(response.status).toBe(400);
    const body = (await response.json()) as { error: string };
...
describe('PUT /api/workflows/:name', () => {
  ...
  test('falls back to getRithHome() when no cwd and no codebases registered', async () => {
    const testRithHome = join(tmpdir(), `rith-home-test-${Date.now()}`);
    process.env.RITH_HOME = testRithHome;
    ...
    try {
      ...
      delete process.env.RITH_HOME;
      await rm(testRithHome, { recursive: true, force: true });
    }
  });
  ...
  test('saves valid workflow to RITH_HOME workflows when source=global', async () => {
    const testRithHome = join(tmpdir(), `rith-home-put-global-${Date.now()}`);
    process.env.RITH_HOME = testRithHome;
    ...
    try {
      ...
      const saved = await readFile(
        join(testRithHome, 'workflows', 'global-workflow.yaml'),
        'utf-8'
      );
...
describe('DELETE /api/workflows/:name', () => {
  test('returns 400 for bundled default name', async () => {
    ...
    // rith-assist is in the real BUNDLED_WORKFLOWS
    const response = await app.request('/api/workflows/rith-assist', { method: 'DELETE' });
    expect(response.status).toBe(400);
    const body = (await response.json()) as { error: string };
    expect(body.error).toContain('rith-assist');
  });
  ...
  test('falls back to getRithHome() when no cwd and no codebases, returns 404 for missing file', async () => {
    const app = createTestApp();
    registerApiRoutes(app, {} as WebAdapter, {} as ConversationLockManager);
  ...
  test('removes home-scoped workflow file when source=global', async () => {
    const testRithHome = join(tmpdir(), `rith-home-del-global-${Date.now()}`);
    const workflowDir = join(testRithHome, 'workflows');
    await mkdir(workflowDir, { recursive: true });
    await writeFile(
...
describe('GET /api/commands', () => {
  ...
  test('includes bundled commands with source:bundled', async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as { commands: Array<{ name: string; source: string }> };
    // rith-assist is in the real BUNDLED_COMMANDS
    const rithAssist = body.commands.find(c => c.name === 'rith-assist');
    expect(rithAssist).toBeDefined();
    expect(rithAssist?.source).toBe('bundled');
  });
});

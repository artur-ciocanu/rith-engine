...
/** Prefix-to-category mapping. Checked after stripping the `rith-` prefix. */
...
function stripRithPrefix(name: string): string {
  return name.startsWith('rith-') ? name.slice('rith-'.length) : name;
}

function findCategory(name: string): string {
  const stripped = stripRithPrefix(name);

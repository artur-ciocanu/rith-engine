...
NOT for: Feature requests, enhancements, or non-bug work. Only for bugs/problems.`;
  ...
  test('handles "Handles:" and "Capability:" fallbacks (rith-assist style)', () => {
...
describe('getWorkflowDisplayName', () => {
  test('strips rith- prefix and converts to title case', () => {
    expect(getWorkflowDisplayName('rith-create-issue')).toBe('Create Issue');
    expect(getWorkflowDisplayName('rith-feature-development')).toBe('Feature Development');
  });

  test('preserves known acronyms (PR, CI, DAG)', () => {
    expect(getWorkflowDisplayName('rith-comprehensive-pr-review')).toBe('Comprehensive PR Review');
    expect(getWorkflowDisplayName('rith-ralph-dag')).toBe('Ralph DAG');
    expect(getWorkflowDisplayName('rith-interactive-prd')).toBe('Interactive PRD');
  });

  test('handles names without rith- prefix', () => {
    expect(getWorkflowDisplayName('my-custom-workflow')).toBe('My Custom Workflow');
  });

  test('handles single-word names', () => {
    expect(getWorkflowDisplayName('rith-assist')).toBe('Assist');
  });
});

describe('getWorkflowCategory', () => {
  test('categorizes review workflows', () => {
    expect(getWorkflowCategory('rith-comprehensive-pr-review', 'Review a PR')).toBe('Code Review');
    expect(getWorkflowCategory('rith-smart-pr-review', 'Smart PR review')).toBe('Code Review');
  });

  test('categorizes automation workflows', () => {
    expect(getWorkflowCategory('rith-create-issue', 'Create GitHub issue')).toBe('Automation');
    expect(getWorkflowCategory('rith-ralph-dag', 'Ralph implementation loop')).toBe('Automation');
    expect(getWorkflowCategory('rith-refactor-safely', 'Refactor code safely')).toBe('Automation');
  });

  test('categorizes CI/CD workflows', () => {
    expect(getWorkflowCategory('rith-validate-pr', 'Validate PR checks')).toBe('CI/CD');
    expect(getWorkflowCategory('rith-test-loop-dag', 'Run test loop')).toBe('CI/CD');
  });

  test('does not miscategorize workflows with "ci" as substring', () => {
    expect(getWorkflowCategory('rith-decision-tree', 'Routes decisions')).toBe('Development');
  ...
  test('categorizes development workflows', () => {
    expect(getWorkflowCategory('rith-feature-development', 'Implement a feature')).toBe(
      'Development'
    );
    expect(getWorkflowCategory('rith-assist', 'General help')).toBe('Development');
    expect(getWorkflowCategory('rith-idea-to-pr', 'From idea to PR')).toBe('Development');
  });
});
...
describe('getWorkflowTags', () => {
  test('derives tags from name and description', () => {
    ...
    const tags = getWorkflowTags('rith-comprehensive-pr-review', parsed);
  ...
  test('deduplicates tags', () => {
    ...
    const tags = getWorkflowTags('rith-pr-review', parsed);
  ...
  test('falls back to inference when no explicit tags', () => {
    ...
    const tags = getWorkflowTags('rith-pr-review', parsed, undefined);
    expect(tags).toContain('GitHub');
    expect(tags).toContain('Review');
  });
...
describe('getWorkflowIconName', () => {
  test('maps issue/bug workflows to Bug icon', () => {
    expect(getWorkflowIconName('rith-create-issue', 'Automation')).toBe('Bug');
    expect(getWorkflowIconName('rith-fix-github-issue', 'Automation')).toBe('Bug');
  });

  test('maps review workflows to Eye icon', () => {
    expect(getWorkflowIconName('rith-comprehensive-pr-review', 'Code Review')).toBe('Eye');
  });

  test('maps conflict workflows to GitMerge icon', () => {
    expect(getWorkflowIconName('rith-resolve-conflicts', 'Automation')).toBe('GitMerge');
  });

  test('maps feature workflows to Rocket icon', () => {
    expect(getWorkflowIconName('rith-feature-development', 'Development')).toBe('Rocket');
  });

  test('maps ralph to Bot icon', () => {
    expect(getWorkflowIconName('rith-ralph-dag', 'Automation')).toBe('Bot');
  });

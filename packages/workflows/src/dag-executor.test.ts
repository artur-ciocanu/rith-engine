...
describe('substituteNodeOutputRefs -- large output file substitution', () => {
  ...
  beforeEach(async () => {
    tempDir = join(tmpdir(), `rith-test-large-output-${Date.now()}`);
...
describe('executeDagWorkflow -- approval node', () => {
  ...
  it('approval message substitutes $nodeId.output.field references from upstream structured output', async () => {
    ...
    // (Per CodeRabbit review of PR artur-ciocanu/rith-engine#1426.)
...
describe('executeDagWorkflow -- script nodes', () => {
  ...
  it('named bun script executes from .rith/scripts/', async () => {
    const mockDeps = createMockDeps();
    const platform = createMockPlatform();
    ...
    const scriptsDir = join(testDir, '.rith', 'scripts');
  ...
  it('named script not found at runtime results in failed state and platform message', async () => {
    ...
    // Do NOT create .rith/scripts/missing.ts — the script should fail to resolve
    const scriptNode: ScriptNode = {
      id: 'gone-script',
      script: 'missing',
      runtime: 'bun',
    ...
    const sendMessage = platform.sendMessage as ReturnType<typeof mock>;
    const messages = sendMessage.mock.calls.map((call: unknown[]) => call[1] as string);
    const notFoundMsg = messages.find((m: string) => m.includes('not found in .rith/scripts/'));
...
describe('bundled opus nodes -- provider annotation invariant (#1610)', () => {
  it('every bundled node with an opus model has provider: claude at the node or workflow level', async () => {
    // Resolve the defaults directory relative to this package (same logic as getAppRithBasePath).
    // import.meta.dir = packages/workflows/src → go up 3 levels to repo root → .rith/workflows/defaults
    const repoRoot = join(import.meta.dir, '..', '..', '..');
    const defaultsDir = join(repoRoot, '.rith', 'workflows', 'defaults');

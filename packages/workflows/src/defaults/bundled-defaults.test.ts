...
// four levels to the repo root, then into .rith/.
const REPO_ROOT = join(import.meta.dir, '..', '..', '..', '..');
const COMMANDS_DIR = join(REPO_ROOT, '.rith/commands/defaults');
const WORKFLOWS_DIR = join(REPO_ROOT, '.rith/workflows/defaults');
...
describe('bundled-defaults', () => {
  describe('isBinaryBuild', () => {
    it('should return false in dev/test mode', () => {
      ...
      // `@rith/paths`. In dev/test mode it is `false`. It is only rewritten to
  ...
  describe('bundle completeness', () => {
    ...
    it('BUNDLED_COMMANDS contains every .md file in .rith/commands/defaults/', () => {
    ...
    it('BUNDLED_WORKFLOWS contains every .yaml/.yml file in .rith/workflows/defaults/', () => {
  ...
  describe('BUNDLED_COMMANDS', () => {
    ...
    it('rith-pr-review-scope should read .pr-number before other discovery', () => {
      const content = BUNDLED_COMMANDS['rith-pr-review-scope'];
      expect(content).toContain('$ARTIFACTS_DIR/.pr-number');
      expect(content).toContain('PR_NUMBER=$(cat $ARTIFACTS_DIR/.pr-number');
    });

    it('rith-create-pr should write .pr-number to artifacts', () => {
      const content = BUNDLED_COMMANDS['rith-create-pr'];
  ...
  describe('BUNDLED_WORKFLOWS', () => {
    ...
    it('rith-workflow-builder should have validate-before-save node ordering and key constraints', () => {
      const content = BUNDLED_WORKFLOWS['rith-workflow-builder'];
    ...
    it('rith-adversarial-dev init-workspace should avoid non-portable sed -i', () => {
      const content = BUNDLED_WORKFLOWS['rith-adversarial-dev'];

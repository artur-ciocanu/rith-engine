...
export async function loadCommandPrompt(
    ...
    getLog().warn(
      {
        err,
        cwd,
        note: 'Default commands will be loaded. Check your .rith/config.yaml if this is unexpected.',
      },
  ...
  // precedence: repo > home (~/.rith/commands/) > bundled/app defaults.
  const searchPaths = rithPaths.getCommandFolderSearchPaths(configuredFolder);
  const resolvedSearchPaths: string[] = [
    ...searchPaths.map(folder => join(cwd, folder)),
    rithPaths.getHomeCommandsPath(),
  ];

  for (const dir of resolvedSearchPaths) {
    const entries = await rithPaths.findMarkdownFilesRecursive(dir, '', { maxDepth: 1 });
  ...
  // If not found in repo/home, search app defaults (from filesystem)
...
export function substituteWorkflowVariables(
  ...
  if (!baseBranch && prompt.includes('$BASE_BRANCH')) {
    throw new Error(
      'No base branch could be resolved. Auto-detection failed and `worktree.baseBranch` is not set in .rith/config.yaml. ' +

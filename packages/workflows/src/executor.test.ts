...
describe('executeWorkflow', () => {
  ...
  describe('concurrent-run guard', () => {
    ...
    it('uses the actionable "in use" message format with workflow name, duration, and short id', async () => {
      ...
      expect(sentMessage).toContain('rith-implement');
  ...
  describe('model/provider resolution', () => {
    ...
    it('passes workflow.model through unchanged', async () => {
      const store = makeStore();
      const deps = makeDeps(store);
      // Model is forwarded verbatim. The SDK is the source of truth for
      // what model strings work.
      await executeWorkflow(
        deps,
        makePlatform(),
        'conv-1',

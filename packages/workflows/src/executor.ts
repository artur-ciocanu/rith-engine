...
import { createLogger, captureWorkflowInvoked, BUNDLED_VERSION } from '@rith/paths';
import { getDefaultBranch, toRepoPath } from '@rith/git';
...
async function resolveProjectPaths(
  ...
  if (codebaseId) {
    try {
      ...
      if (codebase) {
        const parsed = rithPaths.parseOwnerRepo(codebase.name);
        if (parsed) {
          return {
            artifactsDir: rithPaths.getRunArtifactsPath(parsed.owner, parsed.repo, workflowRunId),
            logDir: rithPaths.getProjectLogsPath(parsed.owner, parsed.repo),
          };
        }
        getLog().warn({ codebaseName: codebase.name }, 'codebase_name_not_owner_repo_format');
      }
    } catch (error) {
      const fallbackArtifactsDir = join(cwd, '.rith', 'artifacts', 'runs', workflowRunId);
      getLog().error(
  ...
  return {
    artifactsDir: join(cwd, '.rith', 'artifacts', 'runs', workflowRunId),
    logDir: join(cwd, '.rith', 'logs'),
  };
}
...
export async function executeWorkflow(
  ...
  try {
    ...
    // Opt out via RITH_TELEMETRY_DISABLED=1 or DO_NOT_TRACK=1.
    captureWorkflowInvoked({
      workflowName: workflow.name,
      workflowDescription: workflow.description,
      platform: platform.getPlatformType(),
      rithVersion: BUNDLED_VERSION,
    });
    deps.store
      .createWorkflowEvent({

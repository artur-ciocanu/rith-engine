...
describe('Workflow Loader', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.RITH_HOME = join(testDir, 'home');
...
`;
  ...
  describe('discoverWorkflows', () => {
    it('should discover workflows from .rith/workflows/', async () => {
      const workflowDir = join(testDir, '.rith', 'workflows');
      await mkdir(workflowDir, { recursive: true });
...
`;
  ...
  describe('multi-source loading', () => {
    it('should load real app defaults when enabled', async () => {
      // Test dir has no .rith/workflows/
      const result = await discoverWorkflows(testDir, { loadDefaults: true });
      const workflows = result.workflows.map(ws => ws.workflow);

      // Should load the real rith-** prefixed app defaults
...
`;
      ...
      const assistWorkflow = workflows.find(
        w => w.name === 'my-custom-assist' || w.name === 'rith-assist'
      );
      expect(assistWorkflow).toBeDefined();
      // Repo version should win (has custom name)
      expect(assistWorkflow?.name).toBe('my-custom-assist');
      expect(assistWorkflow?.description).toBe('My custom assist (overrides rith-assist)');
    });
    ...
    it('should skip app defaults when loadDefaults is false', async () => {
      ...
      // Should NOT find any rith-* workflows since app defaults are disabled
...
`;
  ...
  describe('home-scoped workflows (~/.rith/workflows/)', () => {
    // Home-scope is read unconditionally by discovery — no caller option. Tests
    // redirect `getRithHome()` to a temp dir via the `RITH_HOME` env var so
    // they don't touch the user's real `~/.rith/`.
    let homeDir: string;
    const originalRithHome = process.env.RITH_HOME;
    const originalRithDocker = process.env.RITH_DOCKER;
    ...
    it('loads home-scoped workflows from ~/.rith/workflows/ and merges with repo', async () => {
    ...
    it('silently skips when ~/.rith/workflows/ does not exist', async () => {
      // homeDir exists but no workflows/ subdirectory — should not error.
      const result = await discoverWorkflows(testDir, { loadDefaults: false });
      expect(result.errors).toEqual([]);
    });

    it('supports 1-level subfolders under ~/.rith/workflows/ (e.g. triage/foo.yaml)', async () => {
  ...
  describe('legacy ~/.rith/.rith/workflows/ deprecation warning', () => {
    let homeDir: string;
    const originalRithHome = process.env.RITH_HOME;
    const originalRithDocker = process.env.RITH_DOCKER;
    ...
    it('emits a WARN with the migration command when the legacy path exists', async () => {
      const legacyDir = join(homeDir, '.rith', 'workflows');
  ...
  describe('discoverWorkflowsWithConfig', () => {
    it('should pass loadDefaults from config to discoverWorkflows', async () => {
      ...
      // With loadDefaults: false, no rith-** defaults should appear
      const rithWorkflow = result.workflows.find(w => w.workflow.name.startsWith('rith-'));
      expect(rithWorkflow).toBeUndefined();
    ...
    it('should default to loadDefaults: true when config load fails', async () => {
      ...
      // With config failure, defaults to true, so rith-* should appear
      const rithWorkflow = result.workflows.find(w => w.workflow.name === 'rith-assist');
      expect(rithWorkflow).toBeDefined();
    });

    it('surfaces home-scoped workflows without any option — discovery reads ~/.rith/workflows/ internally', async () => {

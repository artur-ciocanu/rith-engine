 ...
 * 3. Suffix match (e.g. "assist" → "rith-assist")
 * 4. Substring match (e.g. "smart" → "rith-smart-pr-review")
 *
...
export function resolveWorkflowName(
  ...
  return (
    ...
    // Tier 3: Suffix match (e.g. "assist" matches "rith-assist")
    checkTier(
      workflows.filter(w => w.name.toLowerCase().endsWith(`-${lowerName}`)),
      'workflow.resolve_suffix_match'
    ) ??
    // Tier 4: Substring match (e.g. "smart" matches "rith-smart-pr-review")
    checkTier(

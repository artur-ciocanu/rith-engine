 ...
 * initSubmodules, path, baseBranch) live in repo-level `.rith/config.yaml`
...
/**
 * Workflow origin:
 * - `bundled` — embedded in the Rith Engine binary / bundled defaults
 * - `global`  — user-level, discovered at `~/.rith/workflows/` (applies to every repo)
 * - `project` — repo-local, discovered at `<repoRoot>/.rith/workflows/`
 *

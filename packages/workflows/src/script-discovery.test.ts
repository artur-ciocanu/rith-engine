...
describe('discoverScriptsForCwd — merge repo + home with repo winning', () => {
  ...
  test('merges scripts from ~/.rith/scripts and <cwd>/.rith/scripts', async () => {
    mockReaddir.mockImplementation(async (path: string) => {
      const p = norm(path);
      if (p === '/home/scripts') return ['home-only.ts'];
      if (p === '/repo/.rith/scripts') return ['repo-only.py'];
      return [];
    });
  ...
  test('repo-scoped script overrides same-named home script', async () => {
    mockReaddir.mockImplementation(async (path: string) => {
      ...
      if (p === '/repo/.rith/scripts') return ['shared.ts'];
      return [];
    });
    mockStat.mockResolvedValue({ isDirectory: () => false });
    ...
    expect(result.get('shared')!.path).toBe('/repo/.rith/scripts/shared.ts');
  });
  ...
  test('tolerates missing home dir (new user, no personal scripts yet)', async () => {
    mockReaddir.mockImplementation(async (path: string) => {
      ...
      if (p === '/repo/.rith/scripts') return ['only-repo.ts'];
      return [];
    });
    mockStat.mockResolvedValue({ isDirectory: () => false });

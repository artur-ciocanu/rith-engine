/**
 * Script discovery - finds and loads script files from .rith/scripts/.
 *
...
import { createLogger, getHomeScriptsPath } from '@rith/paths';
 ...
 * grouping (e.g. `.rith/scripts/triage/foo.ts`) but no nested folders.
...
async function scanScriptDir(
  ...
  for (const entry of entries) {
    ...
    if (entryStat.isDirectory()) {
      // 1-depth cap: allow one level of grouping (e.g. `.rith/scripts/triage/foo.ts`)
...
/**
 * Discover scripts from a directory (expected to be .rith/scripts/ or equivalent).
 ...
 *   1. `<cwd>/.rith/scripts/` — repo-scoped (`source: 'project'` equivalent)
 *   2. `~/.rith/scripts/`    — home-scoped (`source: 'global'` equivalent)
 *
...
export async function discoverScriptsForCwd(cwd: string): Promise<Map<string, ScriptDefinition>> {
  ...
  const repoScripts = await discoverScripts(join(cwd, '.rith', 'scripts'));

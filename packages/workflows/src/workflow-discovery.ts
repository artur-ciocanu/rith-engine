 ...
 *
 *   1. `bundled` — read from the app's defaults folder on disk.
...
function getLog(): ReturnType<typeof createLogger> {
  return createLogger('workflow.discovery');
}

/**
 * One-time deprecation warning for the pre-refactor `~/.rith/.rith/workflows/`
...
async function maybeWarnLegacyHomePath(): Promise<void> {
  ...
  const legacyPath = rithPaths.getLegacyHomeWorkflowsPath();
  const newPath = rithPaths.getHomeWorkflowsPath();
  try {
  ...
  const moveCommand = `mv "${legacyPath}" "${newPath}" && rmdir "${join(rithPaths.getRithHome(), '.rith')}"`;
 ...
 *
 * `1` allows one level of grouping (e.g. `.rith/workflows/defaults/foo.yaml`);
 ...
 *   1. App defaults from disk (unless `options.loadDefaults === false`).
...
export async function discoverWorkflows(
  ...
  // 1. Load from app's defaults on disk (unless opted out)
  ...
  const [workflowFolder] = rithPaths.getWorkflowFolderSearchPaths();
  ...
  try {
    ...
    try {
      ...
      const oldDefaults = defaultEntries.filter(
        f => (f.endsWith('.yaml') || f.endsWith('.yml')) && !f.startsWith('rith-')
      );

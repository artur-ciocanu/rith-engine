 ...
 * the on-disk defaults in .rith/commands/defaults/ and .rith/workflows/defaults/.
 *
  ...
  try {
    ...
    throw new Error(
      `${label} directory not found: ${dir}\n` +
        ...
        'or verify the .rith/ tree exists.'
    );
  }
}
...
function renderFile(commands: BundledFile[], workflows: BundledFile[]): string {
  const header = [
    ...
    ' *',
    ' * Source of truth:',
    ' *   .rith/commands/defaults/*.md',
    ' *   .rith/workflows/defaults/*.{yaml,yml}',
    ' *',

# scripts/install.ps1
# Install Rith Engine CLI from GitHub Releases on Windows
#
# Usage: irm https://github.com/artur-ciocanu/rith-engine | iex
#
# Options (via environment variables):
#   $env:VERSION       - Specific version to install (default: latest)
#   $env:INSTALL_DIR   - Installation directory (default: $env:USERPROFILE\.rith\bin)
#   $env:SKIP_CHECKSUM - Set to "true" to skip checksum verification (not recommended)
#
# Examples:
#   # Install latest
#   irm https://github.com/artur-ciocanu/rith-engine | iex
#
#   # Install specific version
#   $env:VERSION = "v0.2.0"; irm https://github.com/artur-ciocanu/rith-engine | iex
#
#   # Install to custom directory
#   $env:INSTALL_DIR = "C:\tools\rith"; irm https://github.com/artur-ciocanu/rith-engine | iex
...
$INSTALL_DIR  = if ($env:INSTALL_DIR) { $env:INSTALL_DIR } else { "$env:USERPROFILE\.rith\bin" }
...
function Show-Banner {
    ...
    Write-Host "  |      Rith Engine CLI Installer             |" -ForegroundColor Cyan
...
function Get-Arch {
    ...
    switch ($procArch.ToUpper()) {
        "ARM64"  {
            ...
            Write-Warn "Windows ARM64 binaries are not yet available for Rith Engine."
...
function Main {
    ...
    try {
        ...
        Write-Host "    rith workflow list"
        Write-Host "    rith workflow run assist `"What workflows are available?`""
        Write-Host ""

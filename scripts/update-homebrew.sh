...
REPO="artur-ciocanu/rith-engine"
FORMULA_FILE="homebrew/rith.rb"
...
SHA_DARWIN_ARM64=$(echo "$CHECKSUMS" | grep "rith-darwin-arm64" | awk '{print $1}')
SHA_DARWIN_X64=$(echo "$CHECKSUMS" | grep "rith-darwin-x64" | awk '{print $1}')
SHA_LINUX_ARM64=$(echo "$CHECKSUMS" | grep "rith-linux-arm64" | awk '{print $1}')
SHA_LINUX_X64=$(echo "$CHECKSUMS" | grep "rith-linux-x64" | awk '{print $1}')
...
validate_checksum "rith-darwin-arm64" "$SHA_DARWIN_ARM64"
validate_checksum "rith-darwin-x64" "$SHA_DARWIN_X64"
validate_checksum "rith-linux-arm64" "$SHA_LINUX_ARM64"
validate_checksum "rith-linux-x64" "$SHA_LINUX_X64"
...
# Darwin ARM64: line after rith-darwin-arm64 URL
sed -i.bak '/rith-darwin-arm64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_DARWIN_ARM64}"'"/;}' "$FORMULA_FILE"
# Darwin x64: line after rith-darwin-x64 URL
sed -i.bak '/rith-darwin-x64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_DARWIN_X64}"'"/;}' "$FORMULA_FILE"
# Linux ARM64: line after rith-linux-arm64 URL
sed -i.bak '/rith-linux-arm64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_LINUX_ARM64}"'"/;}' "$FORMULA_FILE"
# Linux x64: line after rith-linux-x64 URL
sed -i.bak '/rith-linux-x64/{n;s/sha256 "[a-f0-9]\{64\}"/sha256 "'"${SHA_LINUX_X64}"'"/;}' "$FORMULA_FILE"
